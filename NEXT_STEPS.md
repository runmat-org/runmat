...
- Keep true Turbine cell/comma-list expanded call instructions on interpreter fallback until the JIT value stack can represent non-scalar values; non-expanding scalar semantic expanded calls now use the tagged `TurbineValue[]` host ABI.
...
- Turbine direct and named semantic multi-output calls now compile through a semantic host callback that writes numeric outputs into JIT result slots; non-expanding scalar semantic expanded calls compile through the tagged `TurbineValue[]` host ABI, while true cell/comma-list expansion still requires widening JIT value representation beyond f64 lanes.
...
- Current generic call counts: VM legacy compiler generic calls are centralized behind 3 explicit helper methods (`emit_legacy_user_call`, `emit_legacy_user_call_multi`, `emit_legacy_user_call_expand_multi`) instead of 9 scattered `CallFunction*` emissions; Turbine production has semantic-first named-call lowering centralized behind 2 helper methods, named-call hashing centralized behind 1 helper, plus 1 explicit expanded-call ABI blocker helper; Turbine now has a durable tagged `TurbineValue` foundation and semantic host callbacks for value-shaped host calls, and non-expanding scalar semantic expanded calls use that ABI; Turbine JIT tests have 16 `CallFunction*` instructions, split into 11 semantic-registry named-call ratchets and 5 fallback/error ratchets.
...
- Semantic-first, replaceable next: runtime producers that can be upgraded to semantic handles before crossing the VM/runtime ABI, plus true expanded JIT call shapes after Turbine can pass non-scalar cell/comma-list arguments.
...
- Turbine JIT tests now include a source-level ratchet that exactly 4 `LegacyUserFunction` fixtures remain and that each one has an intentional fallback-boundary note.
