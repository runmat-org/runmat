...
- Current generic call counts: VM legacy compiler generic calls are centralized behind 3 explicit helper methods (`emit_legacy_user_call`, `emit_legacy_user_call_multi`, `emit_legacy_user_call_expand_multi`) instead of 9 scattered `CallFunction*` emissions; Turbine production has semantic-first named-call lowering centralized behind 2 helper methods, named-call hashing centralized behind 1 helper, plus 1 explicit expanded-call ABI blocker helper; Turbine now has a durable tagged `TurbineValue` foundation and semantic host callbacks for non-scalar/cell/comma-list host calls; Turbine JIT tests have 16 `CallFunction*` instructions, split into 11 semantic-registry named-call ratchets and 5 fallback/error ratchets.
