...
- Several VM functions success-path and arity tests now run semantic bytecode, including nested user-function calls, `nargin`/`nargout`, fixed-arity input errors, shared input/output names, inline `fprintf`/`sprintf` cast arguments, root/function-output struct materialization, and nested member assignment.
