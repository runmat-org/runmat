...
- `{count} call outputs`: semantic user-function, `feval`, `size`, `min`/`max`, and `cummin`/`cummax` multi-assign are ratcheted through multi-output bytecode/runtime output context; generic rvalue call outputs and broader builtin output splitting remain call ABI/output-list policy, so avoid ad hoc bytecode variants until call descriptor design is settled.
