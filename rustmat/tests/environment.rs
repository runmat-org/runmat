...
fn run_rustmat_with_env(args: &[&str], env_vars: HashMap<&str, &str>) -> std::process::Output {
...
#[test]
fn test_rustmat_log_level_env_var() {
...
#[test]
fn test_rustmat_timeout_env_var() {
    let mut env = HashMap::new();
...
#[test]
fn test_rustmat_jit_disable_env_var() {
    let mut env = HashMap::new();
...
#[test]
fn test_rustmat_jit_threshold_env_var() {
    let mut env = HashMap::new();
...
#[test]
fn test_rustmat_jit_opt_level_env_var() {
    let opt_levels = [
        ("none", "None"),
        ("size", "Size"), 
        ("speed", "Speed"),
...
#[test]
fn test_rustmat_gc_preset_env_var() {
    let presets = [
...
#[test]
fn test_rustmat_gc_young_size_env_var() {
    let mut env = HashMap::new();
...
#[test]
fn test_rustmat_gc_threads_env_var() {
    let mut env = HashMap::new();
...
#[test]
fn test_rustmat_gc_stats_env_var() {
    let mut env = HashMap::new();
...
fn test_env_vars_override_defaults() {
    ...
    let default_output = run_rustmat_with_env(&["info"], HashMap::new());
    ...
    let override_output = run_rustmat_with_env(&["info"], env);
...
fn test_cli_args_override_env_vars() {
    ...
    let output = run_rustmat_with_env(&["--jit-threshold", "20", "info"], env);
    assert!(output.status.success());
...
fn test_env_var_case_sensitivity() {
    ...
    let mut env = HashMap::new();
    env.insert("rustmat_debug", "1"); // lowercase should not work
...
fn test_port_env_vars_for_kernel() {
    ...
    for port_var in &port_vars {
        ...
        let output = run_rustmat_with_env(&["kernel", "--help"], env);
        assert!(output.status.success(), "Failed with port env var: {}", port_var);
    }
}
...
fn test_boolean_env_var_variations() {
    ...
    for (value, _expected) in &boolean_variations {
        ...
        assert!(output.status.success(), "Failed with RUSTMAT_GC_STATS={}", value);
    }
}