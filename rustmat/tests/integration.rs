...
"#).unwrap();
    ...
    let jit_output = run_rustmat(&[
        "--jit-opt-level", "speed",
        "benchmark", script_path.to_str().unwrap(),
        "--iterations", "5",
        "--jit"
    ]);
    
    // Test with JIT disabled  
    let interp_output = run_rustmat(&[
        "--no-jit",
        "run", script_path.to_str().unwrap()
    ]);
    
    assert!(jit_output.status.success(), "JIT benchmark failed");
    assert!(interp_output.status.success(), "Interpreter execution failed");
...
"#).unwrap();
    ...
    let output = run_rustmat(&[
        "--gc-preset", "high-throughput",
        "--jit-opt-level", "aggressive",
        "--verbose",
        "run", script_path.to_str().unwrap()
    ]);
...
fn test_version_information() {
    ...
    for component in &required_components {
        assert!(stdout.contains(component), "Version info missing component: {}", component);
    }
}
...
fn test_gc_commands() {
    ...
    for cmd in &gc_commands {
        ...
        assert!(output.status.success() || output.status.success(), // Some might not be implemented yet
