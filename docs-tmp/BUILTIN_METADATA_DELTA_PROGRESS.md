...
| Wave 4 (Special Constructors) | `empty`, `magic` | Done | _fill_ | `cargo fmt`; `cargo test -p runmat-builtins`; `cargo test -p runmat-lsp`; targeted runtime filters `empty_`, `magic_` | Attached descriptors + LSP signature-help coverage for constructor edge cases |
