...
- RM-378: complete nested qualified external-handle `feval(...)` output-shape matrix
  - Extended nested qualified unresolved external-handle `feval` coverage in [functions.rs](/Users/nallana/Source/runmat-acc-2/runmat/crates/runmat-vm/tests/functions.rs) to close missing output-shape variants:
    - `unresolved_nested_qualified_external_handle_zero_output_feval_uses_typed_instruction`
    - `unresolved_nested_qualified_external_handle_expand_zero_output_feval_uses_typed_instruction`
    - `unresolved_nested_qualified_external_handle_expand_single_output_feval_uses_typed_instruction`
  - Combined with existing nested qualified `feval` tests, `h = @pkg.sub.remote` now has explicit fixed/expanded output-count matrix coverage:
    - fixed args via `CallFevalMulti(1, out_count)` for `out_count` `0/1/2`
    - expanded args via `CallFevalExpandMultiOutput(..., out_count)` for `out_count` `0/1/2`
    - unresolved runtime identifier pinned to `RunMat:UndefinedFunction`.
  - This removes the remaining nested external-handle `feval` output-policy gap and keeps direct-handle vs `feval` callable ABI contracts aligned.
  - Validation:
    - `cargo test -p runmat-vm --test functions unresolved_nested_qualified_external_handle_ -- --nocapture`
    - `cargo fmt --all --check`
    - `git diff --check`
...
- RM-378: ratchet invalid RHS identifier contracts for numeric slice assignment
  - Added source-level runtime contracts in [basics.rs](/Users/nallana/Source/runmat-acc-2/runmat/crates/runmat-vm/tests/basics.rs) for unsupported RHS type assignment into numeric slice stores:
    - `numeric_linear_slice_assignment_with_string_rhs_reports_invalid_rhs_identifier`
    - `numeric_nd_slice_assignment_with_string_rhs_reports_invalid_rhs_identifier`
  - These tests pin `RunMat:InvalidSliceAssignmentRhs` for both linear and N-D numeric slice assignment paths when RHS is a string scalar, preventing regression back to message-only unsupported RHS behavior.
  - This strengthens selector/assignment ABI closure by ratcheting typed runtime identifier behavior at the source-execution boundary for unsupported slice RHS values.
  - Validation:
    - `cargo test -p runmat-vm --test basics numeric_linear_slice_assignment_with_string_rhs_reports_invalid_rhs_identifier -- --nocapture`
    - `cargo test -p runmat-vm --test basics numeric_nd_slice_assignment_with_string_rhs_reports_invalid_rhs_identifier -- --nocapture`
    - `cargo fmt --all --check`
    - `git diff --check`
...
- RM-378: add invalid slice-RHS identifier contracts for string-array stores
  - Added source-level runtime contracts in [basics.rs](/Users/nallana/Source/runmat-acc-2/runmat/crates/runmat-vm/tests/basics.rs) for unsupported numeric RHS assignment into string-array slice stores:
    - `string_linear_slice_assignment_with_numeric_rhs_reports_invalid_rhs_identifier`
    - `string_nd_slice_assignment_with_numeric_rhs_reports_invalid_rhs_identifier`
  - These tests pin `RunMat:InvalidSliceAssignmentRhs` for both linear and N-D string-array slice assignment paths, aligning string-path source behavior with existing real/complex invalid-RHS identifier surfaces.
  - This further closes assignment ABI error-surface drift by keeping unsupported slice-RHS contracts typed and stable across value-lane store variants.
  - Validation:
    - `cargo test -p runmat-vm --test basics string_linear_slice_assignment_with_numeric_rhs_reports_invalid_rhs_identifier -- --nocapture`
    - `cargo test -p runmat-vm --test basics string_nd_slice_assignment_with_numeric_rhs_reports_invalid_rhs_identifier -- --nocapture`
    - `cargo fmt --all --check`
    - `git diff --check`
...
- RM-378: complete unresolved qualified `@string` feval failure matrix
  - Extended unresolved qualified string-handle `feval('@pkg.remote_inc', ...)` contracts in [closures.rs](/Users/nallana/Source/runmat-acc-2/runmat/crates/runmat-vm/tests/closures.rs):
    - fixed-arg lanes:
      - `feval_with_unresolved_qualified_string_handle_errors`
      - `feval_with_unresolved_qualified_string_handle_zero_output_errors`
      - `feval_with_unresolved_qualified_string_handle_multi_output_errors`
    - expanded-arg lanes:
      - `feval_with_unresolved_qualified_string_handle_expand_errors`
      - `feval_with_unresolved_qualified_string_handle_expand_zero_output_errors`
      - `feval_with_unresolved_qualified_string_handle_expand_multi_output_errors`
  - This closes unresolved qualified `@string` callback output-count `0/1/2` failure coverage across fixed and expanded argument shapes.
  - All lanes are pinned to stable `RunMat:UndefinedFunction`, tightening text-handle callable ABI execution contracts at the qualified-name boundary.
  - Validation:
    - `cargo test -p runmat-vm --test closures feval_with_unresolved_qualified_string_handle_ -- --nocapture`
    - `cargo fmt --all --check`
    - `git diff --check`
...
- RM-378: make object selector masks safe beyond u32 mask width
  - Object selector materialization in [shared.rs](/Users/nallana/Source/runmat-acc-2/runmat/crates/runmat-vm/src/call/shared.rs) now uses a safe mask-bit helper (`object_selector_mask_has_dim`) instead of raw `1u32 << d` shifts inside `for d in 0..dims`.
  - This closes a high-dimension selector-plan seam where `dims >= 33` could panic during selector serialization for object paren and expr-paren paths.
  - Added ratchets:
  ...
  - Validation:
    - `cargo test -p runmat-vm object_paren_selector_values_support_dims_beyond_mask_width -- --nocapture`
    - `cargo test -p runmat-vm object_paren_expr_selector_values_support_dims_beyond_mask_width -- --nocapture`
    - `cargo fmt --all --check`
    - `cargo test -p runmat-core --test semicolon_suppression -- --nocapture`
    - `cargo check --workspace`
    - `git diff --check`
