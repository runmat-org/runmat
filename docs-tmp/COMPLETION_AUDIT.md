...
- Latest result: still partial; non-builtin semantic gap inventory remains open (recent progress includes compile-stage selector-plan identifier ratchets, explicit compile identifiers for unsupported MIR operators (unary+binary)/unknown MIR builtin ids/MIR aggregate-shape invariant violations/unsupported fallback-policy invariants, and early aggregate-shape semantic invariant checks, but residual aggregate/selector design gaps remain).
