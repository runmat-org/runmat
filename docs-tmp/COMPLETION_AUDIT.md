...
- Latest result: still partial; non-builtin semantic gap inventory remains open (recent progress includes compile-stage selector-plan invariant identifier ratchets including cell-selector plan cases, and early aggregate-shape invariant checks, but residual aggregate/selector design gaps remain).
