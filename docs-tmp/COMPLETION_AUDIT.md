...
- Incremental update: object expr-slice selector descriptor serialization now preserves mixed non-numeric selector payloads (`string`, `char`, `cell`, `logical`) for non-range dimensions when any selector dimension carries range/end expression metadata, ratcheted by `object_paren_expr_selector_values_accept_string_selector_in_mixed_plan` and `object_paren_expr_selector_values_accept_cell_selector_in_mixed_plan`; unsupported payload rejection remains explicit via `object_paren_expr_selector_values_reject_unsupported_numeric_selector_type` (`RunMat:ObjectSelectorTypeUnsupported` on `Struct`).
