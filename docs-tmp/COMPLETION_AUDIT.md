...
- Incremental update: VM compile boundaries now enforce explicit delete-assignment RHS semantics (`RunMat:MirDeleteAssignmentRhsInvalid`), rejecting MIR delete markers with non-empty tensor RHS values (`primary_compile_rejects_nonempty_delete_rhs_with_identifier`) so delete intent remains compiler-product driven instead of runtime-inferred.
