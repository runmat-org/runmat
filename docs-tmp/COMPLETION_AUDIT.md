...
- Incremental update: VM compile boundaries now enforce explicit delete-assignment invariants with stable identifiers: non-empty delete RHS rejection (`RunMat:MirDeleteAssignmentRhsInvalid`, `primary_compile_rejects_nonempty_delete_rhs_with_identifier`) and delete mutation/assign target mismatch rejection (`RunMat:MirDeleteAssignmentPlaceMismatch`, `primary_compile_rejects_delete_place_mismatch_with_identifier`), keeping delete intent compiler-product driven instead of runtime-inferred or silently dropped.
