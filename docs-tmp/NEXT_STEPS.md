...
- Continue widening Turbine value-lane coverage beyond semantic cell expansion; semantic expanded calls, expanded `feval` calls, semantic-resolvable named expanded calls, expanded builtin calls, semantic-resolvable method/member expanded calls, and unresolved method/member expanded object/member dispatch now cross the JIT boundary through typed host bridges with tagged variables + `TurbineArgSpec` (no compile-time fallback gate for unresolved method/member expanded calls).
...
- Turbine named multi-output JIT target resolution now derives semantic lookup names through shared fallback-policy semantics (`CallableFallbackPolicy::semantic_resolution_name_for(...)`) instead of generic `CallableIdentity::display_name()` reconstruction, enabling well-formed `DynamicName`/`ExternalName`/`Imported`/`Method` policy-owned lookup shapes while keeping malformed identities off the semantic JIT path.
...
- Imported callable fallback/handle invariants now also require `DefPath` module/item leaf alignment (`module` leaf == `item` leaf) before semantic resolver lookup, VM fallback name derivation, or imported function-handle emission.
- Static non-builtin call lowering now also rejects malformed callee identity name shapes at compile boundaries (`RunMat:MirCallTargetNameInvalid`) instead of deferring those identity-shape failures to runtime.
- Multi-assign output-target lowering now also rejects malformed top-level indexed target contexts at compile boundaries (`RunMat:MirIndexContextInvalid`) instead of deferring invalid assignment-place metadata to runtime store paths.
- Direct indexed assignment lowering now also rejects malformed non-delete assignment-place index contexts at compile boundaries (`RunMat:MirIndexContextInvalid`) instead of deferring invalid assignment metadata to runtime store paths.
- Brace index helper lowering now also enforces assignment/deletion-compatible context invariants on helper read/store-back paths (`RunMat:MirIndexContextInvalid`) instead of bypassing indexed helper context checks in member-chain store-back compilation.
...
- `runmat-config` now exposes a typed project source-index scanner over manifest source roots (`build_project_source_index`) that records discovered `.m` files with qualified identities and explicit MATLAB layout buckets for `+pkg`, `@ClassName`, and `private` directories.
