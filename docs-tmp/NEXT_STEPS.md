...
- Continue widening Turbine value-lane coverage beyond semantic cell expansion; semantic expanded calls, expanded `feval` calls, and semantic-resolvable named expanded calls can now cross the JIT boundary with tagged variables and `TurbineArgSpec`, while external/object expansion paths still need semantic descriptor work.
