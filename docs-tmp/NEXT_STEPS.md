...
- Continue widening Turbine value-lane coverage beyond semantic cell expansion; semantic expanded calls and expanded `feval` calls can now cross the JIT boundary with tagged variables and `TurbineArgSpec`, while external/object expansion paths still need semantic descriptor work.
