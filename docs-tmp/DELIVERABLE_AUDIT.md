...
- Evidence:
  ...
  - semantic fusion-window node mapping in [compile.rs](/Users/nallana/Source/runmat-acc-2/runmat/crates/runmat-vm/src/bytecode/compile.rs) now also accepts accel nodes whose instruction spans fully cover semantic window spans (not only strict contained-by-window spans), reducing fragility to accel-graph span widening while preserving stricter-than-overlap matching.
  - compile-level coverage now includes this boundary via `semantic_windows_map_accel_nodes_that_cover_window_span`.
  ...
  - this extends release-semantics evidence beyond direct `GpuTensor` payloads to nested closure-capture, output-list, and struct task payloads.
