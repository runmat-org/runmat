...
- Additional contract-hardening ratchet:
  ...
  - VM runtime fusion-plan preparation in [state.rs](/Users/nallana/Source/runmat-acc-2/runmat/crates/runmat-vm/src/interpreter/state.rs) now falls back to semantic-window-derived fusion-group scaffolds when compile-populated bytecode groups are absent, reducing runtime dependence on compile group materialization while preserving existing-group precedence.
