...
- Blocking gap:
  ...
  - spawned-task provider-handle policy now has explicit provider declarations plus VM enforcement, and provider-backed release semantics are covered for residency-clear/drop paths (including `Pop`) plus spawn/await completion/cancellation and compiled semantic spawn-overwrite lifecycle flows; semantic async spawn/await lifecycle coverage now includes unaliased helper/callee release behavior via semantic invoker path for direct and struct-nested payload helpers. Remaining evidence gap is broader async spawned-workload release/cleanup semantics across additional nested output-list/cell and multi-output helper shapes.
