...
- Blocking gap:
  ...
  - spawned-task provider-handle policy now has explicit provider declarations plus VM enforcement, and cancellation/completion-path residency cleanup is now covered, but there is not yet full end-to-end evidence for provider-level drop/release semantics across real provider-backed spawned workloads.
