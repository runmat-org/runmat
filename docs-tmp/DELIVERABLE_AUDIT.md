...
- Evidence:
  ...
  - VM dispatch drop boundaries now retire spawn-task IDs for dropped task-handle values in [mod.rs](/Users/nallana/Source/runmat-acc-2/runmat/crates/runmat-vm/src/interpreter/dispatch/mod.rs) (including `Instr::Pop` and `Instr::ExitScope`), reducing stale task-ID registry growth when handles are dropped without await.
