...
- Evidence:
  ...
  - `cov` rows/normalization validation now exposes stable identifier contracts in [cov.rs](/Users/nallana/Source/runmat-acc-2/runmat/crates/runmat-runtime/src/builtins/stats/summary/cov.rs), with rejection tests asserting `RuntimeError.identifier()` for `RunMat:cov:RowsMismatch` and `RunMat:cov:NormalizationInvalid` instead of message-fragment checks.
