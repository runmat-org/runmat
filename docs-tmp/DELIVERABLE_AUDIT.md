...
- Evidence:
  ...
  - VM interpreter cancellation path now clears residency marks for live stack/variable GPU-handle values before returning `ExecutionCancelled` in [runner.rs](/Users/nallana/Source/runmat-acc-2/runmat/crates/runmat-vm/src/interpreter/runner.rs), with direct coverage in `cancellation_clears_gpu_residency_for_live_values`.
...
- Blocking gap:
  - executable fusion groups now use semantic/bytecode instruction-window boundaries and semantic kind classification first, but fusion realization still depends on accel-graph node mapping and graph-derived shape artifacts after boundary derivation in [compile.rs](/Users/nallana/Source/runmat-acc-2/runmat/crates/runmat-vm/src/bytecode/compile.rs).
  - spawned-task provider-handle policy now has explicit provider declarations plus VM enforcement, and cancellation-path residency cleanup is now covered, but there is not yet full end-to-end evidence across completion/drop/release phases for real provider-backed spawned workloads.
