...
- Additional contract-hardening ratchet:
  ...
  - VM runtime fusion planning in [state.rs](/Users/nallana/Source/runmat-acc-2/runmat/crates/runmat-vm/src/interpreter/state.rs) now materializes an accel graph on demand when compile graph artifacts are missing but semantic runtime groups are present, reducing remaining compile-time graph dependence in the runtime planning boundary.
  - VM bytecode runtime fusion helper in [program.rs](/Users/nallana/Source/runmat-acc-2/runmat/crates/runmat-vm/src/bytecode/program.rs) now provides graph-aware group preparation (`runtime_fusion_groups_for_graph`) that applies stack-layout annotation to semantic fallback groups, and interpreter fusion-plan prep consumes that helper in [state.rs](/Users/nallana/Source/runmat-acc-2/runmat/crates/runmat-vm/src/interpreter/state.rs).
