...
- Evidence:
  ...
  - VM compile now requires semantic candidate overlap with accel-capable bytecode instructions before accel-graph construction in [compile.rs](/Users/nallana/Source/runmat-acc-2/runmat/crates/runmat-vm/src/bytecode/compile.rs), reducing no-op graph construction when candidate spans map only to non-accelerable instructions.
  ...
  - pre-gate coverage now includes builtin-call classification in [compile.rs](/Users/nallana/Source/runmat-acc-2/runmat/crates/runmat-vm/src/bytecode/compile.rs), asserting accel-capable builtin overlap acceptance (`sum`) and control/sink non-accelerable builtin overlap rejection (`assert`).
