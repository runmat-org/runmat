...
- Evidence:
  ...
  - VM slice-plan compile invariants now also reject range/end selector operands on `MirIndexPlan::Slice` (which must lower through `IndexSliceExpr`) in [core.rs](/Users/nallana/Source/runmat-acc-2/runmat/crates/runmat-vm/src/compiler/core.rs), preserving explicit selector-plan normalization contracts under `RunMat:MirSliceIndexPlanInvalid`.
