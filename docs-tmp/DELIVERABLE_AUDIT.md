...
- Gap:
  - designed gaps still open (aggregate edge behavior; selector-plan normalization gap has narrowed further with explicit compile-stage selector-plan invariant identifiers and ratchets). Async/future/spawn runtime behavior is now explicit as a lazy future-descriptor lane with spawn/await boundary materialization, but broader cancellation/suspension model work remains out of scope for this slice.
