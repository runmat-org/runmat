...
function createRunnerHtml(timeoutMs, concurrency, logIntervalMs) {
    const workerSource = [
        ...
        "      self.process.env.HOME = \"/home/user\";",
        "    }",
        ...
        "      const cwdBase = \"/home/user\";",
        "      fsProvider.createDirAll(\"/home/user\");",
        ...
        "    try {",
        "      try {",
        "        await Promise.resolve(session.execute(\"cd('/home/user')\"));",
        "      } catch (err) {}",
...
function normalizeOutput(text) {
    ...
    const stripped = lines.map((line) =>
        line.replace(/^\s*\w+(?:\s*[x×]\s*\w+)+\s+\w+(?:\s+\w+)*(?:\s+array)?\s*$/i, "")
