...
pub static PASS_ORDER: &[PassSpec] = &[PACKAGING, WGPU, COMPLETION];

fn packaging_extra() -> String {
    let mut s = String::new();
    s.push_str("Re-check the implementation against the Builtin Packaging template. ");
    s.push_str("Paste the entire template and ensure all required sections (GPU/Fusion specs, tests, error semantics) are present and correct. ");
...
fn wgpu_extra() -> String {
    ...
    s.push_str("(4) keep CPU semantics identical; (5) re-run tests until all pass. Apply changes via apply_patch.\n\n");
...
fn completion_extra(ctx: &AuthoringContext) -> String {
    ...
    s.push_str("(4) keep CPU/GPU parity. Apply changes via apply_patch, then stop.\n\n");
