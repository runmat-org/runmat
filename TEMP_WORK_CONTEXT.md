...
- [ ] **`hist` / `histogram`**
  ...
  - ✅ GPU histogram path using compute shaders is in place; bin centers + normalization literals/name-value pairs and `'BinEdges'` now parse on both CPU and GPU (GPU path still limited to uniform bins/count mode). Style parsing mirrors `bar`, so `'FaceColor'`, `'EdgeColor'`, `'FaceAlpha'`, and labels work uniformly. Follow-up work: histogram-style weighting + exposing the richer `histogram` builtin API.
- [ ] **`surf` / `mesh` / variants**
