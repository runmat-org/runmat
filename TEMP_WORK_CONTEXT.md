...
- Async stdin is now plumbed end-to-end: the VM can suspend mid-execution, `RunMatSession` tracks pending frames via UUIDs, `resume_input` revives the interpreter, and wasm/TS expose `stdinRequested`, `resumeInput(id, value)` and `pendingStdinRequests()` so hosts can bridge prompt UIs without polling stdout. JS input handlers can still answer synchronously, or return `null`/`{ pending: true }`/Promises to opt into the async flow. The actual MATLAB-compatible `input` builtin now targets this path (default prompt, `'s'` flag, numeric parsing via `str2double`), the struct we send over now includes `waitingMs` so shells can warn when a prompt has been idle for a while, and the single-threaded `async_stdin` tests exercise both numeric/text prompts plus `pause`/keypress flows so regressions surface quickly.
- Telemetry consent is now wired end-to-end: `initRunMat({ telemetryConsent })` feeds the session flag, the CLI mirrors the user’s `telemetry.enabled` setting into the same field, and analytics emitters can check `RunMatSession::telemetry_consent()` before sending events. Profiling data still flows regardless so local dashboards remain useful; the docs (`docs/TELEMETRY.md`, bindings README, WASM_UP_PLAN) call out the distinction explicitly.
- Telemetry consent strictly controls analytics now: profiling continues to flow even when consent is `false`, but collectors must check `RunMatSession::telemetry_consent()` before emitting events. Hosts can pass their existing analytics CID via `telemetryId` (TS) / `telemetry_id` (wasm), the runtime stores it (`RunMatSession::set_telemetry_client_id`), and `session.telemetryClientId()` exposes the resolved value for JS.
- Session lifecycle surface is complete: `session.memoryUsage()` taps the wasm memory handle (when available) so hosts can monitor `memory.grow`, the README documents every `initRunMat` option (snapshot, fsProvider, telemetry, plotting, LOD), and `docs/wasm/WASM_UP_PLAN.md` now marks section A as complete.
- Canvas + FS prereqs now return structured init errors: `registerFsProvider`/`registerPlotCanvas`/`registerFigureCanvas` all wrap failures in the new `InitErrorCode::{FilesystemProvider,PlotCanvas}` payload, matching the errors coming out of `initRunMat`. Hosts no longer have to parse free-form strings to learn why boot failed.
- Vitest coverage now exercises the default-filesystem fallback, plotCanvas registration ordering/error propagation, and the `stdinRequested.waitingMs` payload. We spy on `createDefaultFsProvider` so the tests stay deterministic without IndexedDB, and the mock native module override lets us assert the new structured error shapes.
- Lifecycle cleanup landed: `RunMatWasm` exports `dispose()` (cancels executions, clears stdin handlers), the TS wrapper surfaces `session.dispose()` and prevents any further method calls once it runs, and docs now explain when hosts should tear sessions down.
...
- 2025-??-??: `#[runmat_macros::register_*]` proc-macros compile again. The error (“macro expansion ignores ';'…”) traced back to us emitting `inventory::submit! {... };` from the attribute expansion. Removing the stray semicolons lets the submit macro remain a proper item, so `cargo check -p runmat-runtime` succeeds and we can keep pushing on the wasm build. No other behavior changes.
- 2025-??-??: Added `cfg_attr(target_arch = "wasm32", allow(dead_code))` to the runtime builtin/spec macros plus wasm-only registries so `cargo check -p runmat-runtime --target wasm32-unknown-unknown` runs clean. The wasm check now fails one layer deeper in `runmat-accelerate`: the WGPU provider’s global `OnceCell<Box<WgpuProvider>>` requires `Send + Sync`, but WebGPU contexts on wasm aren’t thread-safe (wgpu exposes them as `JsValue`). Need to decide whether to stub the provider on wasm (and rely on the JS bridge) or introduce a wasm-specific implementation that drops the `Send + Sync` bounds by keeping the provider in thread-local storage. This is the current blocker for finishing WASM_UP Plan section A.
