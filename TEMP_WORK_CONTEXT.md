...
- _Status 2024-05-05_: `scatter` now parses positional + name/value args via the shared parser (size/color vectors, `'filled'`, `'Marker*'`), feeds CPU plots with per-point metadata, and keeps the GPU path zero-copy when styles stay uniform. `scatter3` understands the same argument surface and can ingest per-point RGB matrices on the CPU path, but still errors when callers supply per-point marker sizes because runmat-plot lacks size attributes for the 3-D renderer. GPU shaders continue to reject markers/attribute-heavy cases until follow-up work lands.
