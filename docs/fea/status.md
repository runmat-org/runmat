...
| Thermal standalone | Thermal run path with temperature, element-domain gradient/heat-flux/source/boundary-flux fields, explicit source/boundary modeling, prep-backed recovery topology, heat-balance diagnostics, invalid authoring coverage, and known-answer gates. | Thermal operation tests, stability/constitutive diagnostics, CPU/GPU fixture coverage, prep-aware fixtures, external-reference checks, and report-validator gates. | L2. | L3-L5 still need broader mesh-convergence studies and independent thermal comparisons. |
