...
V&V levels are defined in [Verification & Validation](/docs/fea/validation). The table below separates current L2 family support from families that still have L2 evidence in progress. L2 means the currently exposed family contract has solver, field, diagnostic, invalid-case, and backend parity/fallback coverage. It is not a production-readiness claim; L3-L5 require broader convergence, independent-reference, and governed production evidence.
...
| Are artifacts configurable? | Yes. `[runtime.fea]` configures FEA run, study, and thermo-field artifact roots. | Environment configuration uses the documented `RUNMAT_FEA_*` variables. |
...
| --- | --- | --- | --- | --- |
| Linear static structural | Dedicated structural linear-static run path, study support, canonical solver-mesh consumption, stress/strain/reaction/energy fields, moment/rotation support for rotational-DOF elements, known-answer work-energy and closed-form structural gates, and result persistence. | Runtime contracts, solver tests, persisted result tests, GPU fallback/parity fixtures, external-reference checks, report-validator gates, moment invalid-case coverage, and beam/shell reference fixtures. | L2. | L3-L5 still need broader analytic/patch families, mesh-convergence studies, and independent-reference evidence. |
...
| Thermal standalone | Thermal run path with temperature, element-domain gradient/heat-flux/source/boundary-flux fields, explicit source/boundary modeling, heat-balance diagnostics, invalid authoring coverage, and known-answer gates. | Thermal operation tests, stability/constitutive diagnostics, CPU/GPU fixture coverage, external-reference checks, and report-validator gates. | L2. | L3-L5 still need broader mesh-convergence studies and independent thermal comparisons. |
...
| Nonlinear structural | Dedicated nonlinear path with increment, line-search, plasticity/contact fields, rotation snapshots for rotational-DOF moment cases, contact/plastic known-answer gates, and typed invalid-case coverage. | Nonlinear contracts, reference fixtures, policy divergence tests, governance scripts, external-reference checks, report-validator gates, and rotational snapshot tests. | L2. | L3-L5 still need true contact-surface maps when geometry carries them, broader nonlinear-law coverage, and independent nonlinear/plasticity/contact references. |
...
| Electro-thermal | Electrical conductance topology with electric potential, E/J, Joule heat, temperature/residual fields, conservation diagnostics, invalid-case coverage, and governed Joule fixtures. | Domain validation, coupled diagnostics, external-reference checks, and release-readiness thresholds. | L2. | L3-L5 still need broader coupled electrical/thermal reference cases, mesh-convergence evidence, and independent references. |
| Electromagnetic | Frequency-domain Maxwell edge curl-curl harmonic path with oriented edge DOFs, gauge handling, complex vector/flux/E/H/J/loss/energy/Poynting fields, source/boundary/material validation, sweep/resonance metrics, and known-answer gates. | EM contracts, option validation, edge-topology/gauge/residual diagnostics, source/boundary/sweep known-answer gates, external-reference checks, governance thresholds, and public guards against proxy/placeholder field ids. | L2. | L3-L5 still need broader Maxwell validation cases, convergence studies, larger workload hardening, and independent references. |
| CFD | Finite-volume incompressible velocity-pressure path with authored inlet/outlet/wall boundary handling, cell-centered public fields, residuals, transient evolution, invalid-case coverage, and known-answer gates. | `fea.run_cfd/v1` contracts, CFD fixtures, external-reference checks, transient evolution gates, invalid-boundary coverage, and report-validator gates. | L2. | L3-L5 still need broader canonical CFD benchmarks, conservation studies, independent fluid references, and backend parity beyond explicit fallback. |
| CHT | Coupled CFD plus thermal operation with authored CHT interfaces, separated fluid/solid temperatures, interface heat flux/jump fields, interface closure diagnostics, and known-answer gates. | `fea.run_cht/v1` contracts, CFD/thermal validation, coupled diagnostics, interface closure checks, and external-reference checks. | L2. | L3-L5 still need broader conjugate heat-transfer benchmarks, mesh-convergence evidence, and independent CHT references. |
| FSI | Partitioned fluid-structure path with authored FSI interfaces, two-way pressure/displacement feedback, structural traction updates, face-domain pressure/traction, closure diagnostics, and known-answer gates. | `fea.run_fsi/v1` contracts, FSI fixture coverage, interface work/energy checks, and external-reference checks. | L2. | L3-L5 still need broader two-way FSI benchmarks, mesh-convergence evidence, and independent FSI references. |
