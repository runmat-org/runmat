...
| Acoustic harmonic | Acoustic operation with damped Helmholtz domain-graph pressure solve, typed acoustic fields, frequency-response fields, residual diagnostics, material/source/boundary validation, and known-answer gates. | `fea.run_acoustic/v1` contracts, acoustic fixture coverage, external-reference checks, and report-validator gates. | L2 governed path. | Needs broader impedance/radiation boundary depth, mesh-convergence evidence, and external acoustic references before production claims. |
...
| Electromagnetic | Electromagnetic run path with source, boundary, material, harmonic, and sweep payloads. | EM contracts, option validation, sweep/resonance metrics, governance thresholds. | L1-L2. | Full Maxwell vector/edge FE field recovery, known-case validation, and larger workload hardening remain limited. |
| CFD | Finite-volume incompressible velocity-pressure path with authored inlet/outlet/wall boundary handling, cell-centered public fields, residuals, and known-answer gates. | `fea.run_cfd/v1` contracts, CFD fixtures, external-reference checks, and report-validator gates. | L1-L2 governed path. | Needs true fluid mesh/control-volume connectivity, broader CFD benchmarks, conservation studies, and backend parity beyond explicit fallback before production claims. |
| CHT | Coupled CFD plus thermal operation with separated fluid/solid temperatures, interface heat flux/jump fields, interface closure diagnostics, and known-answer gates. | `fea.run_cht/v1` contracts, CFD/thermal validation, coupled diagnostics, external-reference checks, and report-validator gates. | L1-L2 governed path. | Needs full fluid/solid mesh assembly and broader conjugate heat-transfer references before production claims. |
| FSI | Partitioned fluid-structure projection path with fluid fields, structural/interface displacement, face-domain interface pressure/traction, closure diagnostics, and known-answer gates. | `fea.run_fsi/v1` contracts, FSI fixture coverage, external-reference checks, and report-validator gates. | L1-L2 governed path. | Needs full fluid/structure interface iteration over solved structural traction updates plus broader two-way FSI references before production claims. |
