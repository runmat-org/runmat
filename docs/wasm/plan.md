...
- **Histogram weighting + normalization** – `'Weights'` now rides both CPU and GPU paths. Host tensors are uploaded (typed as `single`/`double`), gpuArray weights reuse their provider buffers, the histogram compute shader performs float atomic adds per bin, and a follow-up pass scales by the requested normalization (`'count'`, `'probability'`, `'pdf'`). Degenerate bins and empty histograms reuse the resolved totals so probability/PDF plots match MATLAB numerically even when everything stays on the GPU (`crates/runmat-runtime/src/builtins/plotting/ops/hist.rs`, `crates/runmat-plot/src/gpu/histogram.rs`, `docs/PLOTTING.md`).
- **Scatter label parity** – `scatter`/`scatter3` now honor `'DisplayName'`/`'Label'` name-value pairs via the shared parser, so legends and future UI integrations pick up MATLAB-style series names on both CPU and zero-copy GPU paths.
...
- **Histogram multi-output + bin options** – `hist` now mirrors MATLAB’s `[N, X] = hist(...)` behavior via a dedicated VM branch, and the runtime parser accepts `NumBins`, `BinWidth`, `BinLimits`, and the `'sqrt'`, `'sturges'`, or `'integers'` `BinMethod` tokens alongside `'Weights'`/`'Normalization'`. Uniform bin descriptions stay zero-copy on the GPU; options that introduce non-uniform edges fall back to the CPU path automatically. New unit tests cover the parser additions.
- **Scatter marker-edge parity** – `MarkerEdgeColor` now accepts `'flat'` just like MATLAB. The runtime validates that per-point color data exists, pushes the resolved metadata through the zero-copy scatter packers, and the renderer mixes per-vertex edge colors directly in the point shader so GPU plots stay on-device while honoring per-point edge colors. Tests cover the new parser path and runtime wiring (`crates/runmat-runtime/src/builtins/plotting/ops/scatter.rs`, `crates/runmat-plot/src/plots/scatter.rs`, `crates/runmat-plot/src/gpu/shaders/vertex/point_direct.rs`).
- **Grouped/stacked bar GPU path** – `bar` now handles matrix inputs by emitting one `BarChart` per series. Grouped layouts keep each series in its own offset slot (CPU + zero-copy GPU) while stacked layouts accumulate positive/negative bases per category, pass them into the compute shader, and expand bars directly on the GPU. Crosstalk with legends/labels works because parser + style helpers now understand the `'stacked'` token, and histogram builds reuse the updated GPU structs. Docs/tests cover the new argument surface and matrix behavior (`crates/runmat-runtime/src/builtins/plotting/ops/bar.rs`, `crates/runmat-plot/src/gpu/bar.rs`, `crates/runmat-plot/src/gpu/shaders/bar.rs`, `crates/runmat-runtime/src/builtins/plotting/ops/hist.rs`).
...
- **Documentation** – Filesystem strategy captured in `docs/FILESYSTEM.md` (providers, REST contract, test approach), and this plan stays the canonical overview.
