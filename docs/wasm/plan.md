...
- **Figure lifecycle API gaps** – Expose the remaining registry controls (`close`, `clf`, `gcf/gca`, subplot bounds) plus structured error reporting through the wasm bindings so host apps can surface missing-handle/subplot violations without parsing MATLAB text output.
