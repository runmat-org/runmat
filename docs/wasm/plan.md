...
- **Renderer detach signals** – `FigureEventKind::Closed` now causes the wasm bridge to drop any registered WebGPU canvas for the closed handle (`runmat-runtime/src/builtins/plotting/core/web.rs`, `crates/runmat-wasm/src/lib.rs`), so browser/Tauri hosts no longer need to poll before tearing down their DOM nodes.
...
- **Figure lifecycle API gaps** – (partially addressed) Closed events now detach wasm canvases immediately; the remaining work is wiring the same signal into the native GUI windowing layer so host apps can destroy their surfaces without parsing MATLAB output.
