...
- 2026-05-16: Advanced Maxwell EM to Phase 5 by adding explicit EM material coefficients to analysis-core (`MaterialElectricalModel.relative_permittivity`, `relative_permeability`) and upgrading the EM weak-form proxy assembly path to consume material-driven means for reluctivity and frequency mass-like terms (`omega*sigma + omega^2*epsilon`), with expanded `FEA_EM_STATIC` diagnostic posture fields for material means/spread ratios.
