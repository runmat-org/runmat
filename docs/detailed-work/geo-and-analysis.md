...
- CI emits a compact nonlinear summary from `analysis_benchmark_report.json` into the job summary using `scripts/analysis/reporting/summarize_analysis_report.py`.
...
- CI also evaluates rolling nonlinear trends with `scripts/analysis/reporting/analyze_nonlinear_trends.py`:
...
- CI evaluates final nonlinear release readiness via `scripts/analysis/governance/release_readiness_nonlinear.py`, producing:
...
- CI schema validation for nonlinear artifacts (`scripts/analysis/governance/validate_analysis_report_nonlinear.py`) now also verifies thermo fixture threshold assertions and thermo summary-field presence on thermo-enabled records.
...
- CI includes prep artifact SLO summary via `scripts/analysis/prep_calibration/summarize_prep_artifacts.py` with configurable warn/fail thresholds:
...
- 2026-05-16: Advanced Maxwell EM to Phase 5 by completing region/material-weighted EM coefficient assembly sourced from `material_assignments`, adding deterministic fallback/coverage accounting, surfacing EM readiness metrics (`assignment_coverage_ratio`, `fallback_coefficient_ratio`, `region_coefficient_contrast_index`, `solver_conditioning_proxy`) in `analysis.results` + `analysis.trends`, and introducing a four-fixture EM governance reference family (homogeneous, heterogeneous, sparse-assignment, fallback-heavy) with threshold-enforced publishability splits.
- 2026-05-16: Advanced Maxwell EM to Phase 6 by extracting shared runtime thermo/EM policy thresholds into `analysis/policy.rs`, hardening baseline + rolling drift gates with explicit EM readiness metric checks, and adding a dedicated EM contract snapshot-shape test (`electromagnetic_contract_snapshot.json`) to lock typed payload/summary interface stability.
- 2026-05-16: Advanced Maxwell EM to Phase 7 by adding EM-specific source/boundary schema primitives (`LoadKind::{CurrentDensity, CoilCurrent}`, `BoundaryConditionKind::{MagneticInsulation, VectorPotentialGround}`), wiring them into EM assembly/solve behavior, and promoting `source_realization_ratio` + `boundary_anchor_ratio` to first-class readiness signals with policy thresholds, trend breach rates, and conformance/baseline tracking.
- 2026-05-16: Advanced Maxwell EM to Phase 8 by adding EM field-consistency governance metrics (`flux_divergence_proxy`, `energy_imbalance_ratio`, `boundary_energy_ratio`), recalibrating their formulas to be assembly/material-aware for stable constrained-boundary behavior, and wiring the new signals through runtime policy gates, conformance thresholds, and baseline drift tracking.
- 2026-05-16: Advanced Maxwell EM to Phase 9 by implementing region-localized EM source deposition from `LoadCase.region_id` to assignment-backed coefficient regions (with explicit unmapped fallback accounting), and promoting source-locality readiness metrics (`source_region_coverage_ratio`, `source_material_alignment_ratio`, `source_localization_ratio`) into runtime quality policy, results/trends surfaces, and EM conformance/baseline gates.
- 2026-05-16: Advanced Maxwell EM to Phase 10 by introducing region-weighted source kernels and signed deposition polarity for EM load coupling, adding multi-source overlap/interference diagnostics (`source_overlap_ratio`, `source_interference_index`), wiring overlap/interference thresholds into runtime quality/trend governance, and adding a dedicated overlap-interference EM reference fixture to conformance/baseline gates.
- 2026-05-16: Advanced Maxwell EM to Phase 11 by adding boundary-kernel governance diagnostics (`boundary_condition_localization_ratio`, `ground_anchor_effectiveness_ratio`, `insulation_leakage_proxy`), integrating new policy thresholds/quality-reason mappings and trend breach-rate surfaces in runtime, and adding a dedicated boundary-kernel stress reference fixture with baseline/rolling drift coverage.
- 2026-05-16: Advanced Maxwell EM to Phase 12 by replacing the scalar EM static pass with a coupled harmonic iteration path (real/imag component solves with conductivity coupling), adding normalized coupled-residual + harmonic-coupling diagnostics, and recalibrating top-level EM solver-quality signaling so contract-level homogeneous controls remain publishable while stress fixtures stay governed by readiness thresholds.
- 2026-05-16: Advanced Maxwell EM to Phase 13 by replacing staged harmonic passes with a single block-coupled real/imag solve core (`[K -C; C K]`) using block BiCGStab + block-Jacobi preconditioning, retaining runtime contract/result stability while improving EM solver-fidelity posture and coupled residual behavior.
- 2026-05-16: Advanced Maxwell EM to Phase 14 by adding region-consistent boundary/source governance metrics (`boundary_penalty_conditioning_contribution`, `source_region_energy_consistency_ratio`) and split harmonic residual posture (`real_residual_norm`, `imag_residual_norm`) to `FEA_EM_STATIC`, promoting all four metrics to first-class runtime summary/trend/policy signals with typed quality reasons, and extending EM conformance with two new reference fixtures (`boundary_penalty_stress`, `multi_region_phased_source`) plus baseline/rolling drift checks.
- 2026-05-16: Advanced Maxwell EM to Phase 15 by adding explicit source phasing controls (`phase_rad`, `amplitude_scale`) to EM load schema (`current_density`, `coil_current`), driving harmonic solve with true complex source RHS vectors (`rhs_real`, `rhs_imag`) and complex-vector overlap/interference posture metrics, and applying boundary-penalty terms directly in assembled operator diagonals so boundary-penalty contribution is solver-realized rather than diagnostics-only.
- 2026-05-16: Advanced Maxwell EM to Phase 16 by adding optional runtime frequency-sweep controls (`sweep_enabled`, `sweep_frequency_hz`), surfacing typed sweep/resonance payload and summary/trend posture metrics (sweep count, resonance peak/bandwidth/Q/flux-gain), and introducing policy-governed sweep-coverage/resonance-sharpness quality reasons while preserving default single-frequency EM publishability behavior.
- 2026-03-13: Started Maxwell EM bring-up Phase 0 by adding `electromagnetic` step/schema support and `electromagnetic_static` create-model profile templates, plus a versioned runtime operation contract placeholder `analysis.run_electromagnetic/v1` that performs shape validation and returns deterministic unsupported status until EM solver kernels are implemented.
- 2026-03-13: Advanced Maxwell EM to Phase 1 by adding model-owned `electromagnetic` domain schema in analysis-core and converting `analysis.run_electromagnetic/v1` from unsupported hard-fail to deterministic placeholder run payload mode (`FEA_EM_PLACEHOLDER`), with explicit EM domain validation and non-publishable degraded posture to keep results/trends pipelines active while EM solver kernels are still pending.
- 2026-03-13: Advanced Maxwell EM to Phase 2 by adding typed EM placeholder results payloads (`electromagnetic_results/v1`) on `AnalysisRunResult`, surfacing EM placeholder posture metrics in `analysis.results` summary, and wiring EM run-kind/placeholder warning-rate signals into `analysis.trends` as initial governance skeleton coverage.
- 2026-03-13: Advanced Maxwell EM to Phase 3 by introducing a first FEA-side static EM starter solve pipeline and switching `analysis.run_electromagnetic/v1` from runtime-synthetic payload generation to delegated analysis-fea execution, while preserving typed EM results, summary posture fields, and trend/governance integration.
- 2026-03-13: Advanced Maxwell EM to Phase 4 by upgrading the EM static path from synthetic mapping to a first weak-form style assembled solve proxy (curl-curl stiffness + frequency-conductivity mass terms, matrix-free PCG solve, and flux-density gradient-derived post field), while keeping runtime contracts and result/trend interfaces stable.
- 2026-03-13: Closed the in-scope deepening scope (thermal + thermo/electro + plastic/contact realism/reference hardening baseline) by surfacing additional constitutive posture fields in `analysis.results` (`thermo_field_clamp_ratio`, electro transient/nonlinear time-scale means, plastic/contact severity means), tightening thermo/electro nonlinear conformance assertions, extending nonlinear readiness governance with thermo field-clamp and electro time-scale threshold/breach/trend policy, and calibrating these additions against rolling artifacts.
- 2026-03-13: Added a dedicated physics over/under and rival-gap planning document (`docs/detailed-work/fea-rival-gap-matrix.md`) capturing current implemented vs missing domain coverage (including explicit Maxwell/CFD/FSI/acoustics gaps) and a domain-by-domain build matrix for contracts, schema, solver, diagnostics, benchmarks, and governance gates required for credible ANSYS/COMSOL-class code-first positioning.
- 2026-03-12: Added the initial code-reviewed simulation-package workflow spec (`docs/detailed-work/code-reviewed-fea-workflow.md`) for an `.m`-first authoring/review/execution/inspection loop (model-authored setup, engineer-reviewed code/data diffs, operation-contract execution, desktop-first result inspection, and script/agent-driven downstream analysis).
- 2026-03-12: Revised the workflow spec to enforce a single control plane where operation sequencing is authored only in `.m` scripts, with JSON reserved for data inputs plus generated lock/evidence artifacts; added concrete v1 examples for `simulation_manifest.json`, `simulation.lock.json`, `run_manifest.json`, `analysis_results.json`, and reference `run_simulation.m`/`inspect_results.m` patterns.
- 2026-03-12: Reframed the workflow as a Study Document Model (`study.yaml` as primary authored input with optional JSON equivalent) with composable includes/imports, proposed high-level builtins (`analysis.validate_study`, `analysis.plan_study`, `analysis.run_study`) backed by `serde` + type registry, and a runtime-owned canonical op expansion that emits generated lock/evidence artifacts.
- 2026-03-12: Expanded the study-workflow spec with a whole-schema example aligned to currently implemented analysis interfaces (`AnalysisCreateModelIntentSpec`, `AnalysisModel`, profile-matched run options, and `AnalysisResultsQuery`) so materials/assignments/BCs/loads/steps and execution options mirror real runtime contract fields.
- 2026-03-12: Clarified results-inspection contract in the study-workflow spec by making `run.data` the canonical script-facing inspection surface (via `data.open`/`Dataset`/`DataArray` builtins), removing non-contract summary assumptions like `max_displacement`, and documenting expected dataset attrs/arrays alongside governance JSON envelopes.
- 2026-03-12: Updated artifact topology to root-scoped storage at `<workspace-root>/.artifacts/analysis/` with collision-safe discovery/indexing (`by-id/<analysis_id>`, `by-study/<study_fingerprint>`, `index/*.jsonl`) and immutable run roots under `.artifacts/analysis/by-id/<analysis_id>/runs/<run_id>/...`.
- 2026-03-12: Tightened the workflow spec with explicit import merge keys/conflict rules, planned-vs-current helper API status, deterministic study-fingerprint canonicalization rules, and run-kind-specific required `run.data` array contracts to remove ambiguity before implementation.
- 2026-03-12: Added explicit `Required` semantics to additional workflow tables (Rust type mapping, runtime operation bindings, and run-kind data-array requirements) so implementers can distinguish mandatory vs conditional/defaulted fields at a glance.
- 2026-03-12: Added a physics-domain coverage + migration plan to the study-workflow spec (v1 -> v2 -> v3) that moves thermo/electro/plastic/contact definitions from run-option proxy fields into model-owned constitutive/interface blocks, with a field-by-field mapping and staged validation/deprecation policy.
- 2026-03-12: Split physics-domain coverage/migration planning into a dedicated peer design doc (`docs/detailed-work/analysis-physics-domain-coverage-and-migration.md`) and kept the workflow doc focused on study authoring/execution/discovery contracts.
- 2026-03-12: Expanded the dedicated physics-domain doc with a plain-language simulation status matrix (implemented vs partial vs not implemented), an explicit "most engineers" target capability set, and a phased completion plan with concrete exit criteria from schema migration through governance hardening.
- 2026-03-12: Converted the dedicated physics-domain doc from migration-staged language to a hard-cutover target-shape spec (no legacy compatibility track), making model-owned constitutive/interface physics the canonical `runmat-analysis-study/v1` contract direction.
- 2026-03-12: Implemented the first code-level ownership shift in core analysis model types by introducing `MaterialThermalModel`, `MaterialElectricalModel`, `MaterialPlasticModel`, and `AnalysisInterface`/`AnalysisInterfaceKind`/`ContactInterfaceModel`, then updating core/runtime/fixture model construction paths and thermo constitutive assembly usage to read from `material.thermal.*` fields.
- 2026-03-10: Reorganized the scripts tree into domain-scoped directories (`scripts/analysis/{governance,reporting,prep_calibration,thermo_artifacts,reference_data}`, `scripts/release`, `scripts/runtime`, `scripts/metadata`) and removed top-level backward-compat wrappers to keep `scripts/` clean and reduce path ambiguity in CI/tooling.
...
- 2026-03-12: Continued hard-cutover cleanup by removing plastic/contact proxy framing from active runtime/FEA internals (constitutive/interface option/context naming now canonical), while retaining legacy fixture IDs where needed for benchmark continuity.
- 2026-03-12: Added first standalone thermal vertical slice with new core step kind (`AnalysisStepKind::Thermal`), runtime thermal operation contract (`analysis.run_thermal` + thermal payload), FEA thermal solve path (`run_thermal_with_options`), and benchmark/conformance fixture coverage (`thermal_standalone_ramp_gpu_provider`) integrated into runtime gates.
- 2026-03-12: Extended standalone thermal into governance posture by adding thermal constitutive diagnostics (`FEA_THERMAL_CONSTITUTIVE`), thermal quality policy reasons (`ThermalConstitutiveSpreadHigh`), thermal summary fields in `analysis.results`, thermal trend posture fields in `analysis.trends`, and release-readiness policy thresholds/reasons for thermal residual/temperature/spread and thermal spread drift.
- 2026-03-12: Calibrated profile-default thermal readiness thresholds against current standalone thermal benchmark evidence (`thermal_standalone_ramp_gpu_provider`) by tightening release/development/feature thermal residual/temperature/spread defaults to observed-safe envelopes and separating thermal posture from thermo-coupling severity diagnostics to avoid cross-domain metric pollution.
- 2026-03-12: Extended threshold-ratchet governance reporting to include thermal drift policy (`RUNMAT_RELEASE_READINESS_THERMAL_MAX_SPREAD_TREND_RATIO`) so thermal trend tightening is tracked and validated alongside plastic/contact in ratchet artifacts.
- 2026-03-12: Refactored `runmat-analysis-fea` crate structure to split oversized `fea/src/lib.rs` into explicit module boundaries: `contracts/` (public FEA contracts/types), `pipeline/` (per-run orchestration modules for linear/modal/transient/thermal/nonlinear), `diagnostics/builders.rs` (cross-cutting diagnostic builders), `physics/thermal/` (thermal constitutive metric helpers), and `fea/src/tests.rs` (crate integration tests moved out of root facade).
- 2026-03-12: Continued module-boundary cleanup by introducing `fea/src/physics/coupling/*` and `fea/src/physics/structural/*` helper modules, extracting thermo/electro/plastic/contact severity and electro time-profile logic out of solver implementation files, and wiring transient/nonlinear solvers to consume the new physics-layer helpers.
- 2026-03-12: Added second-pass coupling extraction by moving thermo time-profile sampling and thermo convergence-policy shaping (transient/nonlinear) into `physics/coupling/thermo_mechanical.rs`, switching coupling helpers to borrowed-context APIs to reduce clone-heavy plumbing, and simplifying solver files to policy application orchestration only.
- 2026-03-12: Added third-pass scalar-policy extraction by moving transient timestep recommendation and nonlinear damping/trial-residual/spike-threshold scalar helpers into `physics/structural/mod.rs` policy helpers, then rewiring transient/nonlinear solver loops to consume typed policy inputs instead of embedding these formulas inline.
- 2026-03-12: Completed thermo module boundary cleanup by deleting root `fea/src/thermo.rs`, moving its time-profile sampling/variation logic into `physics/coupling/thermo_mechanical.rs`, and updating assembly/pipeline/solver callsites to consume the coupling module directly.
- 2026-03-12: Completed Slice A baseline (standalone thermal fidelity hardening + acceptance envelopes) by upgrading the thermal solver path to response/diffusion-style progression and adding explicit outcome diagnostics (`FEA_THERMAL_OUTCOME`: spatial gradient index, monotonic response fraction, response-realization ratio), then wiring these metrics through conformance thresholds, runtime summary fields, and release-readiness thermal posture policy.
- 2026-03-12: Completed Slice B baseline (thermo/electro constitutive outcome assertion expansion) by extending benchmark fixture threshold assertions to include additional thermo constitutive indicators (`constitutive_temperature_factor`, `effective_modulus_scale`) and electro constitutive temporal indicators (`time_scale_mean`) alongside existing severity/spread/Joule checks.
...
- 2026-03-10: Added second reference-backed frictionless contact scenario (`nonlinear_contact_frictionless_reference_complex_gpu_provider`) and calibrated contact drift governance against rolling reference evidence by introducing contact-reference trend checks (`CONTACT_REFERENCE_TREND_WORSENING`) with profile thresholds and CI summary surfacing.
- 2026-03-10: Added first reference-backed frictionless contact benchmark path via `nonlinear_contact_frictionless_reference_gpu_provider` using verified assignment fixture backing, frictionless contact proxy configuration, and deterministic low-severity threshold envelopes/validator coverage to start bridging from proxy-only contact stress tests toward reference-calibrated evidence.
- 2026-03-10: Completed Phase-4 contact Layer-3 governance parity by adding contact nonlinear severity summary/trend surfaces, branch-profiled readiness drift policy (`CONTACT_*` severity/breach/trend reasons), strict report validation/CI summary hooks for contact posture, and rolling trend/report script coverage alongside thermo/electro/plastic signals.
- 2026-03-10: Added Phase-4 contact Layer-2 nonlinear diagnostics by introducing `FEA_CONTACT_NONLINEAR` severity signaling from contact-proxy context, mapping runtime quality reason `ContactNonlinearStress` into balanced/strict publishability policy, and adding `nonlinear_contact_proxy_gpu_provider` conformance thresholds plus validator coverage.
- 2026-03-10: Completed Phase-4 kickoff items 1-2 by extending nonlinear governance/trend posture to include plasticity drift metrics (severity max, breach rate, trend ratio with policy reasons/thresholds and CI summary surfacing) and by introducing contact kickoff contract surface (`contact_proxy`) with runtime validation and FEA context plumbing for nonlinear runs.
...
- 2026-03-10: Advanced Phase-3 electro-thermal Layer-2 conformance by adding provider-backed benign/pathological electro-thermal Joule fixtures to the benchmark manifest, wiring fixture-scoped electro-thermal coupling inputs through transient/nonlinear runtime harness paths, and enforcing new threshold assertions for `FEA_ET_COUPLING`, `FEA_ET_TRANSIENT`, and `FEA_ET_NONLINEAR` diagnostics on electro-thermal and mixed nonlinear paths.
- 2026-03-10: Started Phase-3 electro-thermal family layering (Layer 1 contracts/assembly/runtime wiring) by adding additive electro-thermal run-option contracts (`electro_thermal_coupling`) across linear/modal/transient/nonlinear operations, introducing FEA electro-thermal context and deterministic coupling diagnostics (`FEA_ET_COUPLING`), and wiring runtime validation/mapping into all solve entry points while preserving existing thermo governance/test stability.
- 2026-03-10: Added thin production rollout guardrails by enforcing non-default thermo signing-key policy on release branches in CI/readiness (`THERMO_FIELD_SIGNING_KEY_UNSAFE`) while keeping feature/development flexibility, to reduce trust-chain risk without expanding governance complexity.
- 2026-03-10: Delivered Phase-3A thermo artifact trust-chain hardening by adding payload-hash/signature verification at runtime for approved artifact-backed thermo fields, approver authorization checks, readiness reasons for artifact-backed/approval/staleness/provenance + promotion-blocked status, drift-aware promotion blocking with override-token support, artifact-backed conformance fixture splits, and CI thermo trust-report publication.
- 2026-03-10: Completed Phase-2D thermo field artifact lifecycle governance by adding artifact generation/validation/promotion scripts, wiring runtime `field_artifact_id` resolution with typed failure mapping, splitting conformance with artifact-backed smooth/oscillatory thermo fixtures, adding deterministic inline-vs-artifact replay parity coverage, and extending readiness/CI strict gating for artifact-backed thermo policy and thermo field trend controls.
- 2026-03-10: Extended Phase-2C with artifact-backed thermo field retrieval (`RUNMAT_THERMO_FIELD_ARTIFACT_ROOT` + `field_artifact_id`) and typed failure mapping (`ANALYSIS_RUN_THERMO_FIELD_NOT_FOUND`/`INVALID`/`SCHEMA_UNSUPPORTED`/`MISMATCH`), added per-fixture field-quality trend gating in release readiness (`THERMO_FIELD_COVERAGE_TREND_WORSENING`, `THERMO_FIELD_EXTRAPOLATION_TREND_WORSENING`), and introduced strict CI validator enforcement for thermo field summary presence on thermo-gated branches.
- 2026-03-10: Delivered Phase-2C thermo field-source guardrails and observability slice by adding thermo field-source contract fields (`source_id`/`revision`/interpolation mode/expected regions), runtime option validation for thermo profile monotonicity and expected-region consistency, shared thermo time-profile interpolation utilities in FEA, new field-quality diagnostics (`spatial_coverage_ratio`, `field_extrapolation_ratio`, clamp ratios), policy reasons for low coverage/high extrapolation, conformance thresholds on smooth/oscillatory thermo fixtures, and release-readiness/CI summary visibility for thermo field coverage/extrapolation posture.
- 2026-03-10: Updated CI Tier-7.5 governance summary/runbook posture to surface thermo spread ratio thresholds explicitly and annotate feature-profile intent (pathological thermo fixtures tolerated up to spread ratio 1.60) to reduce reviewer ambiguity in readiness triage.
- 2026-03-10: Tuned Phase-2B feature-branch thermo posture by raising default release-readiness `RUNMAT_RELEASE_READINESS_THERMO_MAX_SPREAD_RATIO` from 1.50 to 1.60 to treat the pathological thermo fixture spread as expected exploratory baseline (while keeping release/development tighter), and added unit coverage for this profile-default behavior.
- 2026-03-10: Completed Phase-2B thermo field realism stabilization by switching runtime thermo severity extraction to `severity_peak` (with legacy `severity` fallback) so conformance/readiness summaries remain populated after peak/mean diagnostic key migration, and documented spatial/temporal gradient policy thresholds plus new smooth/oscillatory fixture posture in prep-aware solve docs.
...
- 2026-03-09: Extended nonlinear release-readiness governance to consume thermo summary/trend metrics directly from conformance report records by adding thermo posture fields to harness report records (`thermo_coupling_enabled`/fingerprint/severity metrics) and adding branch-profile thermo thresholds/reason codes in `scripts/analysis/governance/release_readiness_nonlinear.py` (`THERMO_*`) with unit-test coverage.
...
- 2026-03-08: Completed Tier-6 evidence-anchored calibration drift control by adding a versioned prep calibration evidence artifact (`scripts/analysis/prep_calibration/evidence/prep_calibration_evidence.json`), implementing reusable drift evaluation utilities (`scripts/analysis/prep_calibration/evaluate_prep_calibration_drift.py`), extending benchmark records with structured prep calibration/acceptance fields, and wiring nonlinear release-readiness drift policy (`PREP_CALIBRATION_EVIDENCE_MISSING`, `PREP_CALIBRATION_DRIFT_HIGH`) plus rolling trend/summary visibility for acceptance-score posture.
...
- 2026-03-08: Added CI prep artifact SLO summary (`scripts/analysis/prep_calibration/summarize_prep_artifacts.py`) and workflow integration with configurable warn/fail thresholds for artifact count and p95 artifact age, including summary publication in CI step output.
...
- 2026-03-13: Extended nonlinear constitutive diagnostics for plastic/contact with load-path outcome metrics (`load_realization_ratio`, `load_amplification_ratio`) in `FEA_PLASTIC_NONLINEAR` and `FEA_CONTACT_NONLINEAR`, tightened runtime conformance fixtures to enforce deterministic threshold envelopes for these metrics across stress/reference fixture families, surfaced the metrics in `analysis.results` summary contracts, and added release-readiness governance checks for load-path envelopes, breach rates, and rolling trend drift.
- 2026-03-13: Calibrated branch-profile default thresholds for new plastic/contact load-path governance checks (`*_LOAD_REALIZATION_*`, `*_LOAD_AMPLIFICATION_*`) using rolling benchmark artifacts, loosening pre-hardening envelopes/trend ratios where needed to reduce false positives while preserving release-policy signal quality.
- 2026-03-13: Refined nonlinear release-readiness trend-ratio computation for thermo/electro/plastic/contact posture metrics to use fixture-aligned rolling medians (per-fixture baseline pairing) instead of mixed-fixture global medians, eliminating noisy cross-fixture trend alerts during stable runs while preserving true per-fixture drift detection.
...
- 2026-03-08: Added release-readiness governance for nonlinear analysis via `scripts/analysis/governance/release_readiness_nonlinear.py` with typed verdict output (`analysis-release-readiness/v1`: `pass`/`warn`/`fail`), reason codes, machine-readable artifact emission, and protected-branch fail semantics driven by conformance/trend/artifact verification signals.
...
- 2026-03-08: Added CI nonlinear trend intelligence via `scripts/analysis/reporting/analyze_nonlinear_trends.py`, appending trend summaries to job output and enforcing branch-aware slowdown thresholds (`RUNMAT_ANALYSIS_TREND_MAX_SLOWDOWN_RATIO`) with protected-branch fail semantics.
...
- 2026-03-08: Added consumer-side nonlinear report utility validation (`scripts/analysis/governance/validate_analysis_report_nonlinear.py`) and wired it into nonlinear CI governance so `analysis_benchmark_report.json` must include expected nonlinear fixture threshold keys before summary/artifact publication.
