...
`sortrows` accepts every integer class for matrix data and nonzero integer column selectors. Sorted values preserve exact input class, size, and stable equal-row order; optional indices are one-based double. Interactive GPU input follows the same all-class contract: compatible plain-real calls may use the owning provider's row-sort hook, while integer, logical, complex, and unsupported option forms use typed host fallback and restore every requested output to that provider.
