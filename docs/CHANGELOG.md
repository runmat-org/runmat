...
#### Added
* Added `runmat compile` for producing standalone host-native executables through the canonical project/compiler pipeline, verified Native IR and relocatable objects, an ABI/schema/catalog-bound runtime archive embedded in the single RunMat binary, and safe response-file system linking with explicit optimization, policy, linker, temporary-input, and replacement controls.
* Added deterministic whole-program reachability and link planning for native compilation, including `runmat compile --explain-link` and `--link-plan-json` output with exact symbol, runtime-family, target, catalog, capability, certainty, and retention-reason details.
* Added production closed-world native linking with exact catalog builtin symbols, conservative rejection of unknown or non-canonical targets, invocation-scoped no-fallback dispatch, selective archive extraction, platform dead stripping, and binary-to-link-plan verification, while preserving native-specialized runtime discovery.
...
* Added exact distributed package closure for remote jobs and tests: encrypted execution bundles now carry the existing frozen graph, alias visibility, base source revision, verified source bytes, and worker-rebasable handoff for path, Git, Server snapshot, and public or encrypted private registry sources, so drivers, caches, native workers, and browser workers preserve one identity without worker-side resolution or package credentials.
* Added complete remote cluster operability to the CLI and node agent: cluster commands now provide stable JSON output for automation, and customer or hosted nodes can install, inspect, update, and remove hardened systemd, launchd, or native Windows services from one validated secret-free configuration while preserving enrolled node credentials across service removal.
* Added durable content-blind remote execution across local processes, browser workers, customer-managed nodes, and hosted nodes with one portable scheduler and exact frozen package closure; encrypted jobs, distributed tests, coarse resource pools, opaque multi-instance relay, authoritative quota/billing, organization-admin policy and status views, optional locally held recovery keys, bounded legal-hold-aware retention cleanup, native node services, content-safe observability, and documented drain and recovery workflows now share the same public contracts without exposing workload plaintext to RunMat Server.

#### Changed
* Changed compiled execution artifacts to use exact versioned portable or native target identities and source-free package closures, so local, remote, and browser hosts reject incompatible products consistently while compiled remote jobs no longer transmit or reconstruct project source.
* Replaced the legacy bytecode JIT with bounded adaptive MIR-native execution: hot functions, loops, and eligible interactive inputs now use verified Native IR, guarded specialization, exact OSR, dependency-aware invalidation, transactional workspace publication, and shared runtime/placement semantics without replaying completed effects.
...
* Changed automatic GPU offload and VM fusion to use side-effect-free provider capability and representation feasibility checks before execution, with structured rejections and bounded correlated placement diagnostics spanning candidate selection, preparation, compilation, transfer, dispatch, kernel execution, synchronization, completion, and fallback without inventing unavailable provider timings; added deterministic pure-region discovery with liveness and future-consumer analysis, carrying stable region identities through bytecode, portable executable products, and Native IR.
* Changed local CPU/provider placement to compare complete risk-adjusted component costs after feasibility, including nested residency, coherency, cross-device transfer, cold or warm preparation, synchronization, downstream materialization, and absolute and relative decision margins; calibrated thresholds, fusion opportunities, and existing residency now act as priors rather than placement overrides, so transfer-dominated work falls back before provider execution while profitable resident chains remain accelerated.
* Changed execution placement to use bounded graph partitioning with execution-location-aware transfers, transactional fallback, exact session-owned decision invalidation, adaptive aggregate feedback with confidence and hysteresis, and admission against live host/provider memory, scratch, reclaimable allocations, queues, cancellation, and scheduler-owned CPU budgets; placement profiles are bounded, portable, explicit to persist, and exclude source text, paths, values, user identity, and wall-clock timestamps.
