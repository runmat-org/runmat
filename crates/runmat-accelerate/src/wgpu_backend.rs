...
impl DeviceMatrix for WgpuMatrix {
    ...
    fn as_buffer(&self) -> &dyn BufferHandle { &*self.buf }
}
...
impl AccelerateBackend for WgpuBackend {
    ...
    fn upload_matrix(&self, host: &Tensor) -> Result<Box<dyn DeviceMatrix>> {
        ...
        let buf = Arc::new(WgpuBuffer { len: host.data.len() });
        Ok(Box::new(WgpuMatrix { rows: host.rows, cols: host.cols, buf }))
    }
    ...
    fn elem_add(&self, a: &dyn DeviceMatrix, _b: &dyn DeviceMatrix) -> Result<Box<dyn DeviceMatrix>> {
        let buf = Arc::new(WgpuBuffer { len: a.as_buffer().len() });
        Ok(Box::new(WgpuMatrix { rows: a.rows(), cols: a.cols(), buf }))
    }
    fn elem_sub(&self, a: &dyn DeviceMatrix, _b: &dyn DeviceMatrix) -> Result<Box<dyn DeviceMatrix>> {
        let buf = Arc::new(WgpuBuffer { len: a.as_buffer().len() });
        Ok(Box::new(WgpuMatrix { rows: a.rows(), cols: a.cols(), buf }))
    }
    fn elem_mul(&self, a: &dyn DeviceMatrix, _b: &dyn DeviceMatrix) -> Result<Box<dyn DeviceMatrix>> {
        let buf = Arc::new(WgpuBuffer { len: a.as_buffer().len() });
        Ok(Box::new(WgpuMatrix { rows: a.rows(), cols: a.cols(), buf }))
    }
    fn elem_div(&self, a: &dyn DeviceMatrix, _b: &dyn DeviceMatrix) -> Result<Box<dyn DeviceMatrix>> {
        let buf = Arc::new(WgpuBuffer { len: a.as_buffer().len() });
        Ok(Box::new(WgpuMatrix { rows: a.rows(), cols: a.cols(), buf }))
    }
    fn elem_pow(&self, a: &dyn DeviceMatrix, _b: &dyn DeviceMatrix) -> Result<Box<dyn DeviceMatrix>> {
        let buf = Arc::new(WgpuBuffer { len: a.as_buffer().len() });
        Ok(Box::new(WgpuMatrix { rows: a.rows(), cols: a.cols(), buf }))
    }
    ...
    fn transpose(&self, a: &dyn DeviceMatrix) -> Result<Box<dyn DeviceMatrix>> {
        let buf = Arc::new(WgpuBuffer { len: a.as_buffer().len() });
        Ok(Box::new(WgpuMatrix { rows: a.cols(), cols: a.rows(), buf }))
    }
}
