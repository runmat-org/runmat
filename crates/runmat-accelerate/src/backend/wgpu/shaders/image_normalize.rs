...
fn main(
    ...
    var active_lanes = LANE_STRIDE;
    loop {
        if active_lanes <= 1u {
            break;
        }
        let next_active = (active_lanes + 1u) / 2u;
        if lane_index < next_active && lane_index + next_active < active_lanes {
            let dst = lane_index * BATCH_GROUPS;
            let src = (lane_index + next_active) * BATCH_GROUPS;
            for (var g = 0u; g < BATCH_GROUPS; g = g + 1u) {
...
var<workgroup> partial_sum: array<f32, LANE_STRIDE>;
var<workgroup> partial_sum_sq: array<f32, LANE_STRIDE>;
...
fn tensor_offset(sample: u32, batch: u32) -> u32 {
    return sample * params.batch_stride + batch;
}
...
fn main(
    ...
    let local_batch = wid.x;
    if local_batch >= params.batch_count {
        return;
    }
    let batch = params.batch_offset + local_batch;
    let plane = params.plane;
    if plane == 0u {
        return;
    }
    ...
    loop {
        ...
        loop {
            ...
            let value = input_tensor.data[tensor_offset(sample_idx, batch)];
            let y = value - sum_comp;
            let t = sum + y;
            sum_comp = (t - sum) - y;
            sum = t;
            let sq_val = value * value;
            let y_sq = sq_val - sum_sq_comp;
            let t_sq = sum_sq + y_sq;
            sum_sq_comp = (t_sq - sum_sq) - y_sq;
            sum_sq = t_sq;
            inner = inner + 1u;
        }
        idx = idx + lane_stride;
    ...
    partial_sum[lane_index] = sum;
    partial_sum_sq[lane_index] = sum_sq;
    workgroupBarrier();
    ...
    loop {
        ...
        let next_active = (active_lanes + 1u) / 2u;
        if lane_index < next_active && lane_index + next_active < active_lanes {
            partial_sum[lane_index] =
                partial_sum[lane_index] + partial_sum[lane_index + next_active];
            partial_sum_sq[lane_index] =
                partial_sum_sq[lane_index] + partial_sum_sq[lane_index + next_active];
        }
        workgroupBarrier();
        active_lanes = next_active;
    }
    ...
    if lane_index == 0u {
        ...
        let mean = partial_sum[0] * inv_plane;
        var variance = partial_sum_sq[0] * inv_plane - mean * mean;
        variance = max(variance, 0.0f);
        let sigma = sqrt(variance + params.epsilon);
        shared_mean = mean;
        shared_inv_sigma = select(0.0f, 1.0f / sigma, sigma > 0.0f);
    }
    ...
    loop {
        ...
        loop {
            ...
            let offset = tensor_offset(sample_idx, batch);
            var normalized =
                (input_tensor.data[offset] - shared_mean) * shared_inv_sigma;
            if apply_gain {
                normalized = normalized * params.gain;
            }
            if apply_bias {
                normalized = normalized + params.bias;
            }
            if apply_clamp_zero {
                normalized = max(normalized, 0.0f);
            }
            if apply_gamma {
                normalized = pow(normalized, params.gamma);
            }
            output_tensor.data[offset] = normalized;
            inner = inner + 1u;
        }
        write_idx = write_idx + lane_stride;
    }
