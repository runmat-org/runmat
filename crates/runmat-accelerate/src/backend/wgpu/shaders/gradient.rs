...
pub const GRADIENT_COORDINATES_SHADER_F64: &str = r#"
struct Tensor {
    data: array<f64>,
};
...
@group(0) @binding(1) var<storage, read_write> Output: Tensor;
@group(0) @binding(2) var<uniform> params: GradientParams;
@group(0) @binding(3) var<storage, read> Coordinates: Tensor;
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    if (k == 0u) {
        let right = center + params.stride_before;
        let denom = Coordinates.data[1u] - Coordinates.data[0u];
        Output.data[idx] = (Input.data[right] - Input.data[center]) / denom;
        return;
    }
    if (k + 1u == params.segment_len) {
        let left = center - params.stride_before;
        let denom = Coordinates.data[k] - Coordinates.data[k - 1u];
        Output.data[idx] = (Input.data[center] - Input.data[left]) / denom;
        return;
    }

    let left = center - params.stride_before;
    let right = center + params.stride_before;
    let denom = Coordinates.data[k + 1u] - Coordinates.data[k - 1u];
    Output.data[idx] = (Input.data[right] - Input.data[left]) / denom;
}
"#;

pub const GRADIENT_COORDINATES_SHADER_F32: &str = r#"
struct Tensor {
    data: array<f32>,
};
