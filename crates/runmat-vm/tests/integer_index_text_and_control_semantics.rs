...
fn execute_source_with_catalog(
    ...
    test_helpers::interpret(&bytecode)
}
...
fn compiled_inputname_bounds_and_comma_list_propagation_match_the_public_contract() {
    ...
    assert!(values.iter().any(|value| {
        matches!(value, runmat_builtins::Value::CharArray(chars) if chars.data.iter().collect::<String>() == "alpha")
    }));
    assert!(
        values
            .iter()
            .filter(|value| matches!(value, runmat_builtins::Value::CharArray(chars) if chars.data.is_empty()))
            .count()
            >= 2,
