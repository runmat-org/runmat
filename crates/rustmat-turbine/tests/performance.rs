...
fn test_compilation_performance() {
    gc_test_context(|| {
        if let Ok(mut engine) = TurbineEngine::new() {
            ...
            assert!(compilation_time < Duration::from_secs(1),
...
fn test_repeated_execution_performance() {
    gc_test_context(|| {
        if let Ok(mut engine) = TurbineEngine::new() {
            ...
            assert!(last_execution < first_execution * 5,
...
fn test_memory_usage_during_compilation() {
    gc_test_context(|| {
        if let Ok(mut engine) = TurbineEngine::new() {
            let initial_stats = rustmat_gc::gc_stats();
            let initial_memory = initial_stats.current_memory_usage.load(std::sync::atomic::Ordering::Relaxed);
            ...
            let final_stats = rustmat_gc::gc_stats();
            let final_memory = final_stats.current_memory_usage.load(std::sync::atomic::Ordering::Relaxed);
            
            // Memory usage should be reasonable (less than 10MB increase)
            let memory_increase = final_memory.saturating_sub(initial_memory);
            assert!(memory_increase < 10 * 1024 * 1024,
                   "Excessive memory usage: increased by {} bytes", memory_increase);
        }
    });
}
    ...
    gc_test_context(|| {
        if let Ok(mut engine) = TurbineEngine::new() {
            ...
            if compilation_times.len() >= 2 {
                ...
                assert!(last_time < first_time * 20,
    ...
    gc_test_context(|| {
        if let Ok(mut engine) = TurbineEngine::new() {
            ...
            for i in 0..100 {
                ...
                let _ = rustmat_gc::gc_allocate(value);
            }
...
fn test_peak_memory_usage() {
    gc_test_context(|| {
        if let Ok(mut engine) = TurbineEngine::new() {
            let initial_stats = rustmat_gc::gc_stats();
            let initial_peak = initial_stats.peak_memory_usage.load(std::sync::atomic::Ordering::Relaxed);
            ...
            let final_stats = rustmat_gc::gc_stats();
            let final_peak = final_stats.peak_memory_usage.load(std::sync::atomic::Ordering::Relaxed);
            
            // Peak memory usage should be reasonable (less than 50MB increase)
            let peak_increase = final_peak.saturating_sub(initial_peak);
            assert!(peak_increase < 50 * 1024 * 1024,
                   "Peak memory usage too high: increased by {} bytes", peak_increase);
        }
    });
}
...
fn test_cache_hit_ratio_performance() {
    gc_test_context(|| {
        if let Ok(mut engine) = TurbineEngine::new() {
            ...
            assert!(final_stats.cache_hit_rate >= 0.0 && final_stats.cache_hit_rate <= 1.0,
                    "Cache hit rate should be between 0.0 and 1.0, got: {}", final_stats.cache_hit_rate);
            
            // Engine should maintain cache capacity information
            assert!(final_stats.cache_capacity > 0, "Cache should have a positive capacity");
        }
    });
}