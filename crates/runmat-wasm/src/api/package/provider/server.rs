...
pub(super) async fn acquire(
    ...
    plan: &runmat_package::ServerProjectAcquisitionPlan,
) -> Result<runmat_package::ServerProjectPackageMount, String> {
    ...
    if let Some(expected) = &plan.expected {
        match shared::lease_expected(provider, &cache, &expected.tree_digest).await {
            Ok(Some(lease)) => {
                match runmat_package_cache::load_server_project_snapshot(&cache, expected.clone())
                    .await
                {
                    Ok(cached) => {
    ...
    if snapshot.is_none() {
        if !plan.allow_network {
            return Err(format!(
                "package cache miss for Server project `{}` while network access is disabled",
                plan.project
            ));
        }
        let plan_value = serde_wasm_bindgen::to_value(plan)
            .map_err(|error| format!("Server acquisition plan serialization failed: {error}"))?;
        let inventory_value =
            shared::call_provider(&provider.bindings, "fetchServerSnapshot", &plan_value).await?;
        let inventory: runmat_package_cache::ServerProjectTreeInventory =
            serde_wasm_bindgen::from_value(inventory_value)
                .map_err(|error| format!("Server snapshot inventory parse failed: {error}"))?;
        ...
        runmat_package::validate_server_project_acquisition(plan, &acquired.source)
            .map_err(|error| error.to_string())?;
        loop {
            let current = cache.snapshot().await.map_err(|error| error.to_string())?;
            let transaction = runmat_package_cache::cache_server_project_snapshot(
                current.revision,
                current.state,
                &acquired,
    ...
    Ok(runmat_package::ServerProjectPackageMount {
