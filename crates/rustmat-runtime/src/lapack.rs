...
pub fn lapack_solve_linear_system(a: &Matrix, b: &[f64]) -> Result<Vec<f64>, String> {
    ...
    if a.rows != b.len() {
        return Err(format!("Matrix rows {} must match RHS vector length {}", 
                          a.rows, b.len()));
    }
...
pub fn lapack_qr_decomposition(matrix: &Matrix) -> Result<QrDecomposition, String> {
    ...
    unsafe {
        lapack::dgeqrf(
            m, n, &mut a_copy, m,
            &mut tau, &mut work, lwork, &mut info
        );
    }
    
    if info != 0 {
        return Err(format!("LAPACK DGEQRF workspace query failed with info = {}", info));
    }
...
pub fn lapack_eigenvalues(matrix: &Matrix, compute_vectors: bool) -> Result<EigenDecomposition, String> {
    if !is_square(matrix) {
    ...
    unsafe {
        lapack::dsyev(
            jobz, uplo, n, &mut a_copy, n,
            &mut w, &mut work, lwork, &mut info
        );
    }
    
    if info != 0 {
        return Err(format!("LAPACK DSYEV workspace query failed with info = {}", info));
    }
...
pub fn lapack_determinant(matrix: &Matrix) -> Result<f64, String> {
    ...
    let mut swaps = 0;
    for i in 0..n {
        if lu.pivots[i] != (i + 1) as i32 { // LAPACK uses 1-based indexing
            swaps += 1;
        }
    }
...
pub fn lapack_matrix_inverse(matrix: &Matrix) -> Result<Matrix, String> {
    ...
    if info != 0 {
        return Err(format!("LAPACK DGETRI workspace query failed with info = {}", info));
    }
