...
fn execute_plot(mut figure: Figure) -> Result<String, String> {
    match get_plotting_mode() {
        PlottingMode::Interactive => {
            interactive_export(&mut figure)
        },
        PlottingMode::Static => {
            static_export(&mut figure, "plot.png")
        },
        PlottingMode::Jupyter => {
...
fn static_export(figure: &mut Figure, filename: &str) -> Result<String, String> {
    ...
    match figure.get_plot_mut(0) {
        Some(PlotElement::Line(line_plot)) => {
            ...
            rustmat_plot::plot_line(&x_data, &y_data, filename, rustmat_plot::PlotOptions::default())
        ...
        Some(PlotElement::Scatter(scatter_plot)) => {
            ...
            rustmat_plot::plot_scatter(&x_data, &y_data, filename, rustmat_plot::PlotOptions::default())
                .map_err(|e| format!("Plot export failed: {}", e))?;
        ...
        Some(PlotElement::Bar(bar_chart)) => {
            ...
            rustmat_plot::plot_bar(&bar_chart.labels, &values, filename, rustmat_plot::PlotOptions::default())
                .map_err(|e| format!("Plot export failed: {}", e))?;
        ...
        Some(PlotElement::Histogram(histogram)) => {
            ...
            rustmat_plot::plot_histogram(&data, histogram.bins, filename, rustmat_plot::PlotOptions::default())
                .map_err(|e| format!("Plot export failed: {}", e))?;
...
fn scatter3_builtin(x: Matrix, y: Matrix, z: Matrix) -> Result<String, String> {
    ...
    // Create 3D positions
    let positions: Vec<glam::Vec3> = x_data.iter()
        .zip(y_data.iter())
        .zip(z_data.iter())
    ...
    // For now, return a placeholder until 3D integration is complete
    Ok(format!("3D Point cloud created with {} points", point_cloud.len()))
}
