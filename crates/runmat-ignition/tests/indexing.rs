...
fn linear_index_and_end_keyword() {
    ...
    if let Value::Num(v1) = &vars[1] { assert_eq!(*v1, 1.0); } else { panic!("v1"); }
    if let Value::Num(v2) = &vars[2] { assert_eq!(*v2, 4.0); } else { panic!("v2"); }
}
...
fn logical_mask_indexing() {
    ...
    if let Value::Tensor(v) = &vars[2] { assert_eq!(v.data, vec![1.0, 2.0]); } else { panic!("v"); }
}
