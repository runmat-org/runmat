...
const CAPABILITY_NAMES: [&str; 25] = [
    "kron", "ldivide", "le", "length", "linprog", "linsolve", "linspace", "log", "log10", "log1p",
    "log2", "logical", "logspace", "lt", "lu", "minus", "mtimes", "ne", "not", "or", "plus",
    "power", "rdivide", "times", "xor",
];
...
fn linear_algebra_logarithm_and_logic_signatures_are_visible_to_lsp() {
    for (name, source) in [
        ...
        ("minus", "y=minus(int16(5),int16(2));"),
        ("mtimes", "y=mtimes(uint64(2),uint64([3 4]));"),
        ("ne", "y=ne(uint64(9007199254740993),9007199254740992);"),
        ("not", "y=not(uint8([0 1]));"),
        ("or", "y=or(uint8([0 1]),false);"),
        ("plus", "y=plus(uint16(1),uint16(2));"),
        ("power", "y=power(uint32(2),uint32(8));"),
        ("rdivide", "y=rdivide(int16(5),int16(2));"),
        ("times", "y=times(uint64(3),uint64(4));"),
        ("xor", "y=xor(uint8([0 1]),true);"),
    ] {
...
#[test]
fn core_operator_runmat_extensions_are_visible_in_lsp_metadata() {
    for (name, ids) in [
        ...
        ("or", &["or-complex-input", "or-character-input"][..]),
        ("xor", &["xor-complex-input", "xor-character-input"][..]),
    ] {
        let builtin = runmat_builtins::builtin_function_by_name(name).expect("registered builtin");
        let actual = builtin
            .extensions
