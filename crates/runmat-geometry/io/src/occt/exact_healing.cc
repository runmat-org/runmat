...
std::vector<ShapeDigest> persistent_inventory(const TopoDS_Shape& root,
  ...
  for (Standard_Integer index = 1; index <= shapes.Extent(); ++index) {
    const TopoDS_Shape canonical = shapes(index).Oriented(TopAbs_FORWARD);
    ...
    if (digest.size() != 32) {
      throw std::runtime_error("OCCT orientation identity digest has an invalid length");
    }
    ShapeDigest value;
