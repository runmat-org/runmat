...
fn metaclass_basic_and_qualified() {
    ...
    match &program.body[0] {
        Stmt::ExprStmt(Expr::MetaClass(s, _), _, _) => assert_eq!(s, "ClassName"),
...
fn metaclass_in_assignment_and_postfix_member() {
    ...
    match &program.body[0] {
        Stmt::ExprStmt(Expr::Member(obj, member, _), _, _) => {
            assert_eq!(member, "size");
            assert!(matches!(**obj, Expr::MetaClass(ref s, _) if s == "pkg.Class"));
        }
