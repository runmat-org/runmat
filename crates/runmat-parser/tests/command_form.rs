...
fn command_form_not_triggered_by_assignment_or_index() {
    ...
    match &program.body[0] { Stmt::Assign(name, _, _) => assert_eq!(name, "x"), _ => panic!("expected assign") }

    // Not command-form when followed by '('
    let program = parse("foo(1,2)").unwrap();
    match &program.body[0] { Stmt::ExprStmt(Expr::Index(_, _), _) | Stmt::ExprStmt(Expr::FuncCall(_, _), _) => {}, _ => panic!("expected call/index") }
}

#[test]
fn metaclass_qualified() {
    let program = parse("?pkg.sub.Class").unwrap();
    match &program.body[0] { Stmt::ExprStmt(Expr::MetaClass(s), _) => assert_eq!(s, "pkg.sub.Class"), _ => panic!("expected metaclass") }
}
