...
fn init_logging(level: LogLevel, verbose: bool) -> Result<()> {
    ...
    env_logger::Builder::from_default_env()
        ...
        .format(|buf, record| {
            ...
            match level {
                log::Level::Error => writeln!(buf, "\x1b[31m[{}] ERROR [{}]: {}\x1b[0m", timestamp, module, record.args()),
                log::Level::Warn => writeln!(buf, "\x1b[33m[{}] WARN  [{}]: {}\x1b[0m", timestamp, module, record.args()),
                log::Level::Info => writeln!(buf, "\x1b[32m[{}] INFO  [{}]: {}\x1b[0m", timestamp, module, record.args()),
                log::Level::Debug => writeln!(buf, "\x1b[36m[{}] DEBUG [{}]: {}\x1b[0m", timestamp, module, record.args()),
                log::Level::Trace => writeln!(buf, "\x1b[35m[{}] TRACE [{}]: {}\x1b[0m", timestamp, module, record.args()),
            }
        })
    ...
    if compatibility_only {
        ...
        match loader.load(&input) {
            Ok((_snapshot, stats)) => {
                ...
                println!("   🗜️  Compressed size: {}", format_size(stats.compressed_size as u64));
...
fn show_snapshot_info(input: PathBuf, detailed: bool, metrics: bool) -> Result<()> {
    ...
    println!("   Modified: {:?}", file_metadata.modified().unwrap_or(std::time::UNIX_EPOCH));
    println!();
    ...
    match SnapshotLoader::peek_header(&input) {
        Ok(header) => {
            ...
            println!("   Platform: {} {}", 
                     header.metadata.target_platform.os,
                     header.metadata.target_platform.arch);
            
            if header.data_info.compression.algorithm != rustmat_snapshot::format::CompressionAlgorithm::None {
                println!("   Compression: {:?}", header.data_info.compression.algorithm);
                println!("   Compression Ratio: {:.1}%", 
                         (1.0 - header.data_info.compressed_size as f64 / header.data_info.uncompressed_size as f64) * 100.0);
            }
            
            if detailed {
                println!();
                println!("🔧 Build Configuration:");
                println!("   Optimization: {}", header.metadata.build_config.optimization_level);
                ...
                println!("   CPU Features: {:?}", header.metadata.target_platform.cpu_features);
                println!("   Page Size: {}", header.metadata.target_platform.page_size);
                println!("   Cache Line Size: {}", header.metadata.target_platform.cache_line_size);
            }
            ...
            if metrics {
                ...
                println!("   Peak Memory Usage: {}", format_size(perf.peak_memory_usage as u64));
...
fn list_presets(detailed: bool) -> Result<()> {
    ...
    for preset in SnapshotPreset::all_presets() {
        ...
        if detailed {
            ...
            println!("      Debug Friendly: {}", if chars.debugging_friendly { "Yes" } else { "No" });
        }
...
fn benchmark_snapshot(input: PathBuf, iterations: usize, warmup: usize) -> Result<()> {
    ...
    let avg_time: std::time::Duration = load_times.iter().sum::<std::time::Duration>() / iterations as u32;
    ...
    if let Some(stats) = total_stats {
        ...
        println!("   Total Size: {}", format_size(stats.total_size as u64));
        println!("   Throughput: {:.1} MB/s", stats.loading_throughput() / 1_000_000.0);
        println!("   Compression Ratio: {:.1}%", stats.compression_efficiency() * 100.0);
        println!("   Builtins: {}", stats.builtin_count);
    }
