...
impl SnapshotLoader {
    ...
    pub async fn load_async<P: AsRef<Path>>(
        ...
        let file = tokio::fs::File::open(path)
            ...
            .map_err(|e| SnapshotError::Configuration { message: e.to_string() })?;
        ...
        let header: SnapshotHeader = bincode::deserialize(&file_contents[..header_size.min(file_contents.len())])
        ...
        let decompressed_data = if header.data_info.compression.algorithm != CompressionAlgorithm::None {
        ...
        // Deserialize snapshot
        let snapshot: Snapshot = bincode::deserialize(&decompressed_data)
            .map_err(|e| SnapshotError::Configuration { 
    ...
    fn open_snapshot_file(&mut self, path: &Path) -> SnapshotResult<FormatLoader> {
        ...
        let file = File::open(path)
            .with_context(|| format!("Failed to open snapshot file: {}", path.display()))
            .map_err(|e| crate::SnapshotError::Configuration { message: e.to_string() })?;
    ...
    fn load_snapshot_data(&mut self, format_loader: &FormatLoader) -> SnapshotResult<Vec<u8>> {
        ...
        let data = if matches!(
            ...
            self.compression.decompress(&compressed_data, &format_loader.header.data_info.compression)?
        };
    ...
    fn initialize_runtime_integration(&mut self, snapshot: &Snapshot) -> SnapshotResult<()> {
        ...
        log::debug!("Runtime integration initialized in {:?}", self.stats.initialization_time);
        
        Ok(())
    }
    
    /// Initialize builtin function dispatch table
    fn initialize_builtin_dispatch(
        &self,
        ...
        for function_meta in &registry.functions {
                ...
                .find(|b| b.name == function_meta.name)
            {
                ...
                log::warn!("Builtin function '{}' not found in current runtime", function_meta.name);
                // Use a placeholder function that returns an error
                dispatch_table.push(|_args| {
                    Err("Function not available in current runtime".to_string())
                });
            }
        }
        ...
        log::debug!("Initialized dispatch table with {} functions", registry.functions.len());
        Ok(())
    }
    ...
    fn read_header(&mut self) -> SnapshotResult<SnapshotHeader> {
        ...
        if use_mmap && self.mmap.is_some() {
            ...
            let header_size = u32::from_le_bytes([
                mmap_data[0], mmap_data[1], mmap_data[2], mmap_data[3]
            ]) as usize;
