...
fn test_custom_preset() {
    use rustmat_snapshot::{CacheEvictionPolicy, CompressionAlgorithm, SnapshotConfig};
...
fn test_preset_logical_consistency() {
    ...
    assert!(prod_config.compression_level >= 3 && prod_config.compression_level <= 7); // Balanced
}
...
fn test_compression_algorithm_choices() {
    ...
    let perf_config = SnapshotPreset::HighPerformance.config();
    assert!(matches!(perf_config.compression_algorithm, CompressionAlgorithm::Lz4));
    
    // Network-optimized should use high-compression algorithms
    let net_config = SnapshotPreset::NetworkOptimized.config();
    assert!(matches!(net_config.compression_algorithm, CompressionAlgorithm::Zstd));
    
    // Debug should not use compression
    let debug_config = SnapshotPreset::Debug.config();
    assert!(matches!(debug_config.compression_algorithm, CompressionAlgorithm::None));
}