...
fn test_snapshot_creation_and_loading() {
    gc_test_context(|| {
        ...
        assert!(result.is_ok(), "Snapshot loading failed: {:?}", result.err());
...
fn test_snapshot_presets() {
    gc_test_context(|| {
        ...
        for preset in SnapshotPreset::all_presets() {
            let snapshot_path = temp_dir.path().join(format!("{}.snapshot", preset.name().to_lowercase().replace('-', "_")));
            
            let config = preset.config();
            let builder = SnapshotBuilder::new(config.clone());
            
            let result = builder.build_and_save(&snapshot_path);
            assert!(result.is_ok(), "Failed to create snapshot with preset {}: {:?}", preset.name(), result.err());
            
            // Verify file exists and has reasonable size
            assert!(snapshot_path.exists());
            let metadata = fs::metadata(&snapshot_path).unwrap();
            assert!(metadata.len() > 0, "Snapshot file is empty for preset {}", preset.name());
            
            // Try to load it
            let mut loader = SnapshotLoader::new(config);
            let result = loader.load(&snapshot_path);
            assert!(result.is_ok(), "Failed to load snapshot with preset {}: {:?}", preset.name(), result.err());
        }
    });
}
