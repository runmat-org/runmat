...
fn test_platform_info() {
    ...
    // Test endianness
    assert!(matches!(platform.endianness, Endianness::Little | Endianness::Big));
}
...
fn test_compression_algorithm_equality() {
    ...
    assert_eq!(
        Zstd { dictionary: Option::<Vec<u8>>::None },
        Zstd { dictionary: Option::<Vec<u8>>::None }
    );
    assert_eq!(
        Zstd { dictionary: Some(vec![1, 2, 3]) },
        Zstd { dictionary: Some(vec![1, 2, 3]) }
    );
    assert_ne!(
        Zstd { dictionary: Some(vec![1, 2, 3]) },
        Zstd { dictionary: Some(vec![4, 5, 6]) }
    );
}
...
fn test_snapshot_format_with_checksum() {
    ...
    let format_with_checksum = format
...
fn test_serialization_roundtrip() {
    ...
    assert_eq!(deserialized.metadata.rustmat_version, header.metadata.rustmat_version);
}
