...
fn benchmark_snapshot_loading(c: &mut Criterion) {
    ...
    gc_test_context(|| {
        for (name, preset) in &snapshots {
            let snapshot_path = temp_dir.path().join(format!("{}.snapshot", name.to_lowercase()));
            let config = preset.config();
            let builder = SnapshotBuilder::new(config);
    ...
    for (name, preset, snapshot_path) in snapshot_paths {
        ...
        group.bench_with_input(BenchmarkId::new("load", name), &(preset, snapshot_path), |b, (preset, path)| {
            b.iter(|| {
                gc_test_context(|| {
                    let config = preset.config();
...
fn benchmark_compression_algorithms(c: &mut Criterion) {
    ...
    let test_data = vec![
        ...
        ("large_text", "The quick brown fox jumps over the lazy dog. ".repeat(1000).into_bytes()),
        ("repetitive", vec![42u8; 10000]),
        ("random", (0..10000).map(|i| (i * 17 % 256) as u8).collect()),
    ];
    
    let algorithms = vec![
        ("uncompressed", SnapshotConfig {
            compression_enabled: false,
            ..SnapshotConfig::default()
        }),
        ("lz4_fast", SnapshotConfig {
            compression_enabled: true,
            compression_algorithm: rustmat_snapshot::CompressionAlgorithm::Lz4,
            compression_level: 1,
            ..SnapshotConfig::default()
        }),
        ("lz4_balanced", SnapshotConfig {
            compression_enabled: true,
            compression_algorithm: rustmat_snapshot::CompressionAlgorithm::Lz4,
            compression_level: 6,
            ..SnapshotConfig::default()
...
fn benchmark_memory_usage(c: &mut Criterion) {
    ...
    for (name, config) in configs {
        group.bench_with_input(BenchmarkId::new("memory_footprint", name), &config, |b, config| {
            b.iter(|| {
                gc_test_context(|| {
                    let temp_dir = tempdir().unwrap();
