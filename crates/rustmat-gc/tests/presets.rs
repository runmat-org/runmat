use rustmat_gc::{GcConfig, gc_configure, gc_stats, gc_test_context};
use rustmat_gc::{gc_collect_minor, gc_collect_major, gc_allocate};
...
fn test_statistics_with_different_presets() {
    gc_test_context(|| {
        ...
        let stats = gc_stats();
        let allocations = stats.total_allocations.load(std::sync::atomic::Ordering::Relaxed);
        ...
        let minor_collections = post_collection_stats.minor_collections.load(std::sync::atomic::Ordering::Relaxed);
...
fn test_parallel_collection_config() {
    gc_test_context(|| {
        let mut config = GcConfig::default();
        config.parallel_collection = true;
        config.max_gc_threads = 4;
...
fn test_pointer_compression_config() {
    gc_test_context(|| {
        let mut config = GcConfig::default();
        config.pointer_compression = true;
...
fn test_min_max_heap_size() {
    gc_test_context(|| {
        let mut config = GcConfig::default();
        config.min_heap_size = 16 * 1024 * 1024; // 16MB
        config.max_heap_size = 256 * 1024 * 1024; // 256MB
...
fn test_collection_timeout() {
    gc_test_context(|| {
        let mut config = GcConfig::default();
        config.collection_timeout = std::time::Duration::from_millis(100);
...
fn test_invalid_config_edge_cases() {
    gc_test_context(|| {
        ...
        config.young_generation_size = 0;
        assert!(config.validate().is_err());
