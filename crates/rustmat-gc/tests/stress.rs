...
fn test_massive_allocation_cycle() {
    ...
    for cycle in 0..10 {
        ...
        println!("Cycle {}: allocated 1000 objects, collected {}", cycle, collected);
        ...
        let collected = gc_collect_major().expect("collection should succeed");
        println!("Cycle {}: after clear, collected {}", cycle, collected);
    }
}
...
fn test_large_matrix_stress() {
    ...
    for i in 0..100 {
        ...
        let ptr = gc_allocate(Value::Matrix(matrix))
            .expect("matrix allocation should succeed");
        matrices.push(ptr);
        
        // Trigger collection periodically
        if i % 10 == 9 {
            let collected = gc_collect_minor().expect("collection should succeed");
            println!("After {} large matrices, collected {} objects", i + 1, collected);
        }
    }
    
    // Final collection
    let collected = gc_collect_major().expect("final collection should succeed");
    println!("Final collection: {} objects", collected);
}
...
fn test_nested_cell_stress() {
    ...
    for depth in 1..=10 {
        ...
        if depth % 3 == 0 {
            let collected = gc_collect_minor().expect("collection should succeed");
            println!("After depth {}, collected {} objects", depth, collected);
        }
    }
    
    // Major collection to clean up
    let collected = gc_collect_major().expect("major collection should succeed");
    println!("Final nested collection: {} objects", collected);
}
...
fn test_gc_with_interpreter_integration() {
    ...
    for run in 0..50 {
        ...
        if run % 10 == 9 {
            let collected = gc_collect_minor().expect("collection should succeed");
            println!("After {} interpreter runs, collected {} objects", run + 1, collected);
        }
    }
}
...
fn test_gc_statistics_accuracy() {
    ...
    let final_collections = after_collect_stats.minor_collections.load(Ordering::Relaxed);
    
    assert_eq!(final_collections - initial_collections, 1);
    println!("Statistics test: allocated {}, collected {}", allocation_count, collected);
}
...
fn test_gc_under_memory_pressure() {
    ...
    for i in 0..10000 {
        ...
        let value = match i % 5 {
            ...
            2 => Value::Matrix(rustmat_builtins::Matrix::new(
                vec![i as f64; 100], 10, 10
            ).unwrap()),
            3 => Value::Cell(vec![Value::Num(i as f64), Value::Int(i as i32)]),
            _ => Value::Bool(i % 2 == 0),
        };
        ...
        if current_collections > collection_count {
            ...
            println!("Collection triggered after {} allocations (total collections: {})", 
                    i + 1, collection_count);
        }
    ...
    println!("Memory pressure test completed with {} collections", collection_count);
    assert!(collection_count > 0, "Should have triggered at least one collection");
}
...
fn test_gc_configuration_changes() {
    ...
    println!("Configuration test: aggressive collections: {}, final collections: {}", 
            aggressive_collections, final_collections);
    
    // Aggressive config should trigger more collections
    assert!(aggressive_collections >= 1, "Aggressive config should trigger collections");
}