...
use crate::{GcConfig, Result, GcError};
...
impl Generation {
    ...
    pub fn allocate(&self, size: usize) -> Result<()> {
        ...
        if current + size > self.max_size {
            return Err(GcError::OutOfMemory(
                format!("Generation {} cannot allocate {} bytes", self.number, size)
            ));
        }
    ...
    pub fn deallocate(&self, size: usize) {
        self.current_size.fetch_sub(size.min(self.current_size()), Ordering::Relaxed);
    }
    ...
    pub fn record_collection(&self, objects_collected: usize, survivors: usize) {
        ...
        *self.last_collection.lock() = Some(now);
        ...
        log::debug!("Generation {} collection: {} collected, {} survivors", 
                   self.number, objects_collected, survivors);
    }
    ...
    pub fn collection_frequency(&self) -> f64 {
        ...
        let duration = frequency.back().unwrap().duration_since(*frequency.front().unwrap());
    ...
    pub fn resize(&mut self, new_max_size: usize) -> Result<()> {
        if new_max_size < self.current_size() {
            return Err(GcError::ConfigError(
                format!("Cannot shrink generation {} below current size", self.number)
            ));
        }
        
        log::info!("Resizing generation {} from {} to {} bytes", 
                  self.number, self.max_size, new_max_size);
...
impl GenerationalHeap {
    ...
    pub fn should_collect_minor(&self) -> bool {
        self.young_generation().is_full(self.config.minor_gc_threshold)
    }
    ...
    pub fn total_utilization(&self) -> f64 {
        let total_used: usize = self.generations.iter()
            .map(|g| g.current_size())
            .sum();
    ...
    pub fn adapt_generation_sizes(&mut self) -> Result<()> {
        ...
        if young_gen_freq > 10.0 && young_gen_util > 0.8 {
            ...
            if new_size <= self.total_size_limit / 4 { // Don't let young gen be > 25% of total
...
mod tests {
    ...
    fn test_generational_heap() {
        ...
        heap.young_generation().allocate(
            (config.young_generation_size as f64 * 0.9) as usize
    ...
    fn test_heap_statistics() {
        ...
        heap.young_generation().allocate(1000).expect("should allocate");
    ...
    fn test_heap_total_utilization() {
        ...
        heap.generation(0).unwrap().allocate(1000).expect("should allocate");
        heap.generation(1).unwrap().allocate(2000).expect("should allocate");
