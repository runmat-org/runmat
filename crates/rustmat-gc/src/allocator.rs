...
use crate::{GcPtr, GcConfig, GcStats, Result, GcError};
...
impl Generation {
    fn new(number: usize, max_size: usize) -> Self {
        ...
        for &size_class in &[SizeClass::Small, SizeClass::Medium, SizeClass::Large] {
            blocks.insert(size_class, vec![MemoryBlock::new(size_class.allocation_size())]);
    ...
    fn allocate(&mut self, size: usize) -> Result<*mut u8> {
        ...
        if self.allocated_bytes.load(Ordering::Relaxed) + aligned_size > self.max_size {
            return Err(GcError::OutOfMemory(
                format!("Generation {} out of memory", self.number)
            ));
        }
        ...
        let new_block = MemoryBlock::new(
            std::cmp::max(size_class.allocation_size(), aligned_size * 2)
        );
...
impl GenerationalAllocator {
    ...
    pub fn total_usage(&self) -> f64 {
        let total_allocated: usize = self.generations.iter()
            .map(|g| g.allocated_bytes())
            .sum();
        let total_capacity: usize = self.generations.iter()
            .map(|g| g.max_size)
            .sum();
    ...
    pub fn reconfigure(&mut self, config: &GcConfig) -> Result<()> {
        ...
        for (i, gen) in self.generations.iter_mut().enumerate() {
            if gen.max_size != gen_size {
                log::info!("Resizing generation {} from {} to {} bytes", 
                          i, gen.max_size, gen_size);
    ...
    fn estimate_value_size(&self, value: &Value) -> usize {
        match value {
            ...
            Value::String(s) => {
                std::mem::size_of::<Value>() + s.len()
            }
            Value::Matrix(m) => {
                std::mem::size_of::<Value>() + 
                std::mem::size_of::<rustmat_builtins::Matrix>() +
                m.data.len() * std::mem::size_of::<f64>()
            }
            Value::Cell(cells) => {
                std::mem::size_of::<Value>() +
                cells.len() * std::mem::size_of::<Value>() +
                cells.iter().map(|v| self.estimate_value_size(v)).sum::<usize>()
            }
        }
    }
    ...
    pub fn stats(&self) -> AllocatorStats {
        AllocatorStats {
            ...
            generation_usage: self.generations.iter()
                .map(|g| g.allocated_bytes())
                .collect(),
...
mod tests {
    ...
    fn test_allocator_basic() {
        ...
        let ptr = allocator.allocate(value, &mut stats).expect("allocation should succeed");
