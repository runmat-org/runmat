...
use crate::{
    local_tet_flip::{
        ...
        two_to_three_face_flip_candidate, LocalTet, LocalTetFlipCandidate,
...
const MAX_MULTI_INTERIOR_REFILL_NODES: usize = 6;
const MAX_MULTI_INTERIOR_REFILL_CANDIDATES: usize = 512;
...
impl<'a> BoundaryExactCoverSearch<'a> {
    ...
    fn search_from_traced(
        ...
        let (forced_volume_m3, forced_indices) = match self.propagate_forced_interior_mates_traced(
            ...
            Ok(forced) => forced,
            Err(reason) => {
                self.record_dead_end(trace, selected, reason.reason());
                return None;
            }
        };
...
fn common_tet_edges(tets: [[u32; 4]; 3]) -> Vec<[u32; 2]> {
    let mut edge_counts = BTreeMap::<[u32; 2], usize>::new();
    for tet in tets {
...
fn tet_edges(node_ids: [u32; 4]) -> [[u32; 2]; 6] {
    [
...
mod tests {
    ...
    fn exact_cover_trace_reports_forced_mate_volume_overflow() {
        ...
        assert_eq!(result, Err(ForcedInteriorMateFailure::VolumeOverflow));
        assert_eq!(face_counts, initial_face_counts);
        assert!(selected.is_empty());
    }
