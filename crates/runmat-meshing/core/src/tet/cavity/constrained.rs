...
use crate::{
    predicate::{
        distance_squared, orient_tet_node_ids, point_in_closed_triangle_surface, tet_centroid,
        tet_circumsphere, tet_edge_aspect_ratio, tet_scaled_jacobian, tet_signed_volume,
        triangle_area, Point3, PointInClosedSurface, Triangle3,
    },
    tet::reconnect::{
        evaluate_local_tet_flip_quality, three_to_two_edge_flip_candidate,
...
pub fn constrained_cavity_from_refill_tet_component(
    ...
    let cavity = ConstrainedCavity {
        removed_tet_ids: (0..tets.len()).map(|index| index as u32).collect(),
        boundary_faces,
        protected_node_ids,
        target_volume_m3: tets.iter().map(|tet| tet.volume_m3).sum(),
    };
    validate_constrained_cavity(&cavity)?;
    Ok(cavity)
}

pub fn split_refill_tets_across_shared_face_at_barycentric(
    tets: &[ConstrainedCavityRefillTet],
    ...
    (Vec<ConstrainedCavityRefillTet>, ConstrainedCavityNode),
    ConstrainedCavityRefillTetSplitError,
> {
    ...
    {
        return Err(
            ConstrainedCavityRefillTetSplitError::InvalidBarycentricCoordinates { barycentric },
        );
    }
    let node_map = nodes
        .iter()
    ...
    for tet in tets {
        for node_id in tet.node_ids {
            if !node_map.contains_key(&node_id) {
                return Err(ConstrainedCavityRefillTetSplitError::MissingNode { node_id });
            }
        }
    }
    ...
    let incident_tet_indices = tets
        .iter()
        .enumerate()
        .filter_map(|(index, tet)| {
            tet_faces(tet.node_ids)
                .map(sorted_face)
                .contains(&target_face)
                .then_some(index)
        })
        .collect::<Vec<_>>();
    if incident_tet_indices.len() != 2 {
        return Err(ConstrainedCavityRefillTetSplitError::FaceIncidenceNotTwo {
            node_ids: target_face,
            incident_tet_count: incident_tet_indices.len(),
        });
    }
    ...
    let incident_tet_indices = incident_tet_indices.into_iter().collect::<BTreeSet<_>>();
    let mut split_tets = Vec::<ConstrainedCavityRefillTet>::new();
    for (index, tet) in tets.iter().enumerate() {
        if !incident_tet_indices.contains(&index) {
            split_tets.push(tet.clone());
            continue;
        }
        let opposite_node = tet
            .node_ids
            .into_iter()
            .find(|node_id| !target_face.contains(node_id))
            .expect("incident tet should have an opposite node");
        for child_node_ids in [
            ...
            match raw_refill_tet_with_rejection_reason(child_node_ids, points, options) {
                Ok(child) => split_tets.push(child),
                Err(reason) => {
                    return Err(ConstrainedCavityRefillTetSplitError::RejectedChildTet {
    ...
    Ok((split_tets, split_node))
}

pub fn flip_refill_tets_across_shared_face(
    tets: &[ConstrainedCavityRefillTet],
    nodes: &[ConstrainedCavityNode],
    face: [u32; 3],
    options: ConstrainedCavityRefillOptions,
) -> Result<Vec<ConstrainedCavityRefillTet>, ConstrainedCavityRefillTetFlipError> {
    let node_map = refill_component_node_map(tets, nodes)?;
    let target_face = sorted_face(face);
    for node_id in target_face {
        if !node_map.contains_key(&node_id) {
            return Err(ConstrainedCavityRefillTetFlipError::MissingNode { node_id });
        }
    }
    let incident_tet_indices = tets
        .iter()
        ...
        .collect::<Vec<_>>();
    if incident_tet_indices.len() != 2 {
        return Err(ConstrainedCavityRefillTetFlipError::FaceIncidenceNotTwo {
            node_ids: target_face,
            incident_tet_count: incident_tet_indices.len(),
        });
    }
    let left_index = incident_tet_indices[0];
    let right_index = incident_tet_indices[1];
    let flip = two_to_three_face_flip_candidate(
        LocalTet {
            tet_id: left_index as u32,
            node_ids: tets[left_index].node_ids,
        },
        LocalTet {
            tet_id: right_index as u32,
            node_ids: tets[right_index].node_ids,
        },
    )
    .map_err(
        |err| ConstrainedCavityRefillTetFlipError::InvalidFlipTopology {
            reason: local_tet_flip_error_reason(&err),
        },
    )?;
    refill_tets_from_flip_candidate(tets, &node_map, &flip, options)
}

pub fn flip_refill_tets_around_shared_edge(
    tets: &[ConstrainedCavityRefillTet],
    nodes: &[ConstrainedCavityNode],
    edge: [u32; 2],
    options: ConstrainedCavityRefillOptions,
    ...
    let incident_tet_indices = tets
        ...
        .enumerate()
        .filter_map(|(index, tet)| {
            (tet.node_ids.contains(&target_edge[0]) && tet.node_ids.contains(&target_edge[1]))
                .then_some(index)
        })
        .collect::<Vec<_>>();
    if incident_tet_indices.len() != 3 {
        return Err(ConstrainedCavityRefillTetFlipError::EdgeIncidenceNotThree {
    ...
    let flip = three_to_two_edge_flip_candidate(
        [
            LocalTet {
                tet_id: incident_tet_indices[0] as u32,
                node_ids: tets[incident_tet_indices[0]].node_ids,
            },
            LocalTet {
                tet_id: incident_tet_indices[1] as u32,
                node_ids: tets[incident_tet_indices[1]].node_ids,
            },
            LocalTet {
                tet_id: incident_tet_indices[2] as u32,
                node_ids: tets[incident_tet_indices[2]].node_ids,
            },
        ],
        target_edge,
    )
...
pub fn constrained_cavity_expanded_across_boundary_face(
    ...
    source_tets: &[CavityTet],
    boundary_face: [u32; 3],
) -> Result<ConstrainedCavity, ConstrainedCavityExpansionError> {
    constrained_cavity_expanded_across_boundary_faces(cavity, source_tets, &[boundary_face])
}
...
pub fn constrained_cavity_expanded_across_boundary_faces(
    ...
    let mut selected_tet_ids = cavity
        .removed_tet_ids
        .iter()
        .copied()
        .collect::<BTreeSet<_>>();
    for target in &target_faces {
        ...
        if face.outside_tet_ids.is_empty() {
            return Err(
                ConstrainedCavityExpansionError::BoundaryFaceHasNoOutsideTet { node_ids: *target },
            );
        }
        selected_tet_ids.extend(face.outside_tet_ids.iter().copied());
    }
    let tet_id_to_index = source_tets
        .iter()
        .enumerate()
    ...
    for step in 0..MAX_CONSTRAINED_CAVITY_EXPANSION_STEPS {
        let selected_indices = selected_tet_ids
            .iter()
            .map(|tet_id| {
                tet_id_to_index
                    .get(tet_id)
                    .copied()
                    .ok_or(ConstrainedCavityExpansionError::SourceTetIdNotFound { tet_id: *tet_id })
            })
        ...
        match validate_constrained_cavity(&expanded) {
            ...
            Err(ConstrainedCavityValidationError::NonManifoldBoundaryEdge { node_ids, .. }) => {
                ...
                for boundary in &expanded.boundary_faces {
                    ...
                    for tet_id in &boundary.outside_tet_ids {
                        added |= selected_tet_ids.insert(*tet_id);
                    }
                }
                if !added {
                    for tet in source_tets {
                        if selected_tet_ids.contains(&tet.tet_id) {
                            continue;
                        }
                        let touches_edge = tet_edges(tet.node_ids)
                            .into_iter()
                            .any(|edge| sorted_edge(edge) == node_ids);
                        if touches_edge {
                            added |= selected_tet_ids.insert(tet.tet_id);
                        }
                    }
                }
                if !added {
                    return Err(
                        ConstrainedCavityExpansionError::BoundaryEdgeHasNoOutsideTet { node_ids },
                    );
                }
            }
            Err(err) => {
...
pub fn constrained_cavity_expanded_across_boundary_faces_or_recovered_edge_star(
    ...
    let attempted_boundary_faces = boundary_faces
        ...
        .map(sorted_face)
        .collect::<Vec<_>>();
    match constrained_cavity_expanded_across_boundary_faces(cavity, source_tets, boundary_faces) {
        Ok(expanded) => Ok(ConstrainedCavityBoundaryEdgeRecovery {
            cavity: expanded,
            attempted_boundary_faces,
            recovered_edge: None,
        }),
        Err(ConstrainedCavityExpansionError::BoundaryEdgeHasNoOutsideTet { node_ids }) => {
            ...
            let before = cavity
                ...
                .copied()
                .collect::<BTreeSet<_>>();
            let added_tet_ids = expanded
                .removed_tet_ids
                .iter()
                .copied()
                .filter(|tet_id| !before.contains(tet_id))
                .collect::<Vec<_>>();
            Ok(ConstrainedCavityBoundaryEdgeRecovery {
                recovered_edge: Some(ConstrainedCavityBoundaryEdgeRecoveryStep {
                    node_ids,
                    added_tet_ids,
                    removed_tet_count_before: cavity.removed_tet_ids.len(),
                    removed_tet_count_after: expanded.removed_tet_ids.len(),
                }),
                cavity: expanded,
                attempted_boundary_faces,
            })
...
pub fn constrained_cavity_recovered_boundary_edge_star_excluding_nodes(
    ...
    let added_tet_ids = expanded
        ...
        .filter(|tet_id| !before.contains(tet_id))
        .collect::<Vec<_>>();
    let removed_tet_count_after = expanded.removed_tet_ids.len();
    Ok(ConstrainedCavityBoundaryEdgeRecovery {
        cavity: expanded,
...
pub fn constrained_cavity_expanded_across_boundary_edge_star_excluding_nodes(
    ...
    if !added {
        return Err(
            ConstrainedCavityExpansionError::BoundaryEdgeHasNoOutsideTet {
                node_ids: target_edge,
            },
        );
    }
...
pub fn constrained_cavity_expanded_across_first_valid_boundary_face(
    ...
    for boundary_face in boundary_faces {
        match constrained_cavity_expanded_across_boundary_face(cavity, source_tets, *boundary_face)
        {
            Ok(expanded) => return Ok(Some(expanded)),
            Err(ConstrainedCavityExpansionError::SourceTetIdNotFound { tet_id }) => {
                return Err(ConstrainedCavityExpansionError::SourceTetIdNotFound { tet_id });
            }
            Err(
                ConstrainedCavityExpansionError::BoundaryFaceNotFound { .. }
                | ConstrainedCavityExpansionError::BoundaryFaceHasNoOutsideTet { .. }
                | ConstrainedCavityExpansionError::BoundaryEdgeHasNoOutsideTet { .. }
        ...
        for second in (first + 1)..node_ids.len() {
            for third in (second + 1)..node_ids.len() {
                for fourth in (third + 1)..node_ids.len() {
                    ...
                    let tet_boundary_faces = tet_faces(tet_node_ids).map(sorted_face);
                    if !tet_boundary_faces
                        .iter()
                    ...
                    if raw_refill_tet_with_rejection_reason(tet_node_ids, points, relaxed_options)
                        .is_err()
                    {
                        continue;
                    }
                    for face in tet_boundary_faces {
...
fn non_manifold_boundary_edges(tets: &[CavityTet], selected: &BTreeSet<usize>) -> Vec<[u32; 2]> {
...
fn boundary_edge_defect_score(tets: &[CavityTet], selected: &BTreeSet<usize>) -> usize {
    let mut edge_counts = BTreeMap::<[u32; 2], usize>::new();
    for (_, owners) in boundary_face_owners(tets, selected) {
        if owners.len() != 1 {
            continue;
    ...
    if interior_candidate_nodes.is_empty() {
        if boundary_node_ids.len() == 4 {
            let Some(refill) = single_tet_refill_candidate(cavity, &boundary_node_map, options)
                ...
                record_refill_rejection(&mut rejected_by_reason, "single_tet_refill_rejected");
...
pub fn retriangulate_constrained_cavity_from_nodes(
        ...
        for second in (first + 1)..candidate_nodes.len() {
            for third in (second + 1)..candidate_nodes.len() {
                for fourth in (third + 1)..candidate_nodes.len() {
                    ...
                    let points = tet_node_ids.map(|node_id| node_map[&node_id]);
                    if point_in_closed_triangle_surface(
                        tet_centroid(points),
                        &boundary_triangles,
                        tolerance,
                    ...
                    if candidate_tets.len() > MAX_BOUNDARY_EXACT_COVER_CANDIDATES {
                        return Ok(None);
                    }
                }
            }
    ...
    let Some(mut refill) = exact_cover_refill_from_candidate_tets(cavity, &candidate_tets, options)
        .map_err(ConstrainedCavityRefillError::Validation)?
    else {
        return Ok(None);
    };
...
pub fn generate_constrained_cavity_component_steiner_nodes(
    cavity: &ConstrainedCavity,
    nodes: &[ConstrainedCavityNode],
    component_tets: &[ConstrainedCavityRefillTet],
    options: ConstrainedCavityRefillOptions,
    max_nodes: usize,
) -> Result<Vec<ConstrainedCavityNode>, ConstrainedCavityRefillError> {
    validate_refill_options(options)?;
    validate_constrained_cavity(cavity).map_err(ConstrainedCavityRefillError::Validation)?;
    if max_nodes == 0 || component_tets.is_empty() {
        return Ok(Vec::new());
    }
    let boundary_node_map = boundary_node_coordinates(cavity, nodes)?;
    let boundary_triangles = cavity_boundary_triangles(cavity, &boundary_node_map)?;
    ...
    let mut sorted_tets = component_tets.to_vec();
    sorted_tets.sort_by(|left, right| {
    ...
    for tet in sorted_tets {
        let tet_points = tet.node_ids.map(|node_id| node_map[&node_id]);
        let tet_centroid = tet_centroid(tet_points);
        component_steiner_candidate_points(tet.node_ids, tet_points, tet_centroid, cavity_centroid)
            .into_iter()
            .filter(|point| {
            ...
            .for_each(|point| {
                let score =
                    component_steiner_candidate_quality_score(tet.node_ids, tet_points, point);
...
pub fn generate_constrained_cavity_patch_steiner_nodes(
    ...
    for tet in tetrahedralize_points(&boundary_points) {
        let node_ids = tet.vertices.map(|index| boundary_points[index].node_id);
        let points = tet
            .vertices
        ...
        if let Ok(tet) = raw_refill_tet_with_rejection_reason(node_ids, points, options) {
...
pub fn generate_constrained_cavity_boundary_cap_nodes(
    ...
    for face in solid_empty_faces {
            ...
            local_cap_apex_candidates(face, surface_point, cavity_centroid, &boundary_node_map)
        {
            ...
            let Ok(tet) = raw_refill_tet_with_rejection_reason(
...
pub fn constrained_cavity_classified_solid_empty_boundary_faces(
    ...
    for face in &faces {
        ...
        if boundary_face.outside_tet_ids.is_empty() {
...
> {
    ...
    let mut expanded_removed_tet_ids = Vec::<u32>::new();
    if !classification.expandable_faces.is_empty() {
        ...
        expanded_removed_tet_ids = current_cavity
            .removed_tet_ids
            .iter()
            .copied()
            .filter(|tet_id| !before.contains(tet_id))
        ...
        for second in (first + 1)..boundary_node_ids.len() {
            for third in (second + 1)..boundary_node_ids.len() {
                for fourth in (third + 1)..boundary_node_ids.len() {
                    ...
                    let candidate_faces = tet_faces(tet_node_ids).map(sorted_face);
                    if !candidate_faces
                        .iter()
                        .any(|face| boundary_faces.contains(face))
                    {
                    ...
                    if raw_refill_tet_with_rejection_reason(tet_node_ids, points, options).is_ok() {
...
fn component_steiner_candidate_points(
    ...
    candidates.push(tet_centroid);
    candidates.push(tet_incenter(points));
    if let Some((center, _)) = tet_circumsphere(points, MeshingTolerance::default()) {
        candidates.push(center);
        for fraction in [0.25, 0.5, 0.75] {
            candidates.push(interpolate(tet_centroid, center, fraction));
        }
    }
    for face in tet_faces(node_ids) {
        let face_points = face.map(|node_id| {
            let index = node_ids
                ...
                .expect("tet face node should be in tet");
            points[index]
        });
        let face_centroid = [
        ...
        for fraction in [0.18, 0.33, 0.5, 0.67] {
            candidates.push(interpolate(face_centroid, tet_centroid, fraction));
        }
        for fraction in [0.12, 0.25, 0.4] {
            candidates.push(interpolate(face_centroid, cavity_centroid, fraction));
        }
    }
    for edge in tet_edges(node_ids) {
        let edge_points = edge.map(|node_id| {
            let index = node_ids
                .iter()
                .position(|candidate| *candidate == node_id)
                .expect("tet edge node should be in tet");
            points[index]
        });
        let midpoint = [
        ...
        for fraction in [0.2, 0.4, 0.6] {
            candidates.push(interpolate(midpoint, tet_centroid, fraction));
        }
    }
    candidates
}
...
fn component_steiner_candidate_quality_score(
    ...
    for face in tet_faces(node_ids) {
        ...
        let quality =
            tet_scaled_jacobian([face_points[0], face_points[1], face_points[2], candidate]);
        min_quality = min_quality.min(quality);
    }
    min_quality
}

fn tet_incenter(points: [Point3; 4]) -> Point3 {
    let weights = [
...
pub fn split_constrained_cavity_source_edge(
    ...
    for tet_id in &cavity.removed_tet_ids {
        if !source_tets.iter().any(|tet| tet.tet_id == *tet_id) {
            return Err(
                ConstrainedCavitySourceEdgeSplitError::MissingRemovedSourceTet { tet_id: *tet_id },
            );
        }
    }
    ...
    let selected_original_tet_ids = cavity
        .removed_tet_ids
        .iter()
        .copied()
        .collect::<BTreeSet<_>>();
    let mut selected_tet_ids = BTreeSet::<u32>::new();
    let mut split_source_tets = Vec::<CavityTet>::with_capacity(source_tets.len() + 8);
    let mut next_tet_id = source_tets
        .iter()
        .map(|tet| tet.tet_id)
        .max()
        .unwrap_or_default()
    ...
    for tet in source_tets {
        ...
        if !incident {
            if selected_original_tet_ids.contains(&tet.tet_id) {
                selected_tet_ids.insert(tet.tet_id);
            }
            split_source_tets.push(tet.clone());
            continue;
        }

        incident_count += 1;
        let opposite_nodes = tet
            .node_ids
            .into_iter()
            .filter(|node_id| !target_edge.contains(node_id))
            .collect::<Vec<_>>();
        if opposite_nodes.len() != 2 {
            return Err(ConstrainedCavitySourceEdgeSplitError::DegenerateSplitTet {
                tet_id: tet.tet_id,
            });
        }
        let child_node_ids = [
        ...
        for child in child_node_ids {
            ...
            let (oriented_node_ids, volume_m3) = orient_tet_node_ids(child, points);
            if volume_m3 <= 0.0 {
                return Err(ConstrainedCavitySourceEdgeSplitError::DegenerateSplitTet {
                    tet_id: tet.tet_id,
                });
            ...
            let child_tet = CavityTet {
                tet_id: next_tet_id,
                component_id: tet.component_id,
                node_ids: oriented_node_ids,
                source_surface_element_id: tet.source_surface_element_id,
                region_ids: tet.region_ids.clone(),
                volume_m3,
                aspect_ratio: tet_edge_aspect_ratio(oriented_points),
                exact_scaled_jacobian: tet_scaled_jacobian(oriented_points).abs(),
            };
            if selected_original_tet_ids.contains(&tet.tet_id) {
                selected_tet_ids.insert(child_tet.tet_id);
            }
            split_source_tets.push(child_tet);
            next_tet_id = next_tet_id.saturating_add(1);
        }
    }

    if incident_count == 0 {
        return Err(ConstrainedCavitySourceEdgeSplitError::NoIncidentSourceTet {
            node_ids: target_edge,
        });
    }

    let selected_indices = split_source_tets
        .iter()
        .enumerate()
        .filter_map(|(index, tet)| selected_tet_ids.contains(&tet.tet_id).then_some(index))
        .collect::<BTreeSet<_>>();
...
fn multi_interior_node_refill_candidate(
    ...
    for tet in tetrahedralize_points(&points) {
        ...
        if refill_tets.len() > MAX_MULTI_INTERIOR_REFILL_CANDIDATES {
...
fn scaled_worst_face_star_quality(
    ...
    let worst_tet = refill.tets.iter().min_by(|left, right| {
        left.exact_scaled_jacobian
            .total_cmp(&right.exact_scaled_jacobian)
    })?;
    let face_nodes = worst_tet
        .node_ids
        .into_iter()
...
fn boundary_edge_patch_split_node(
    edge: [u32; 2],
...
fn boundary_node_exact_cover_refill_candidate(
    ...
    let boundary_faces = cavity
        ...
        .map(|face| sorted_face(face.node_ids))
        .collect::<BTreeSet<_>>();
    let mut all_candidates = Vec::<ConstrainedCavityRefillTet>::new();
        ...
        for second in (first + 1)..node_ids.len() {
            for third in (second + 1)..node_ids.len() {
                for fourth in (third + 1)..node_ids.len() {
                    ...
                    let touches_boundary = tet_faces(tet_node_ids)
                        .map(sorted_face)
                        .iter()
                        .any(|face| boundary_faces.contains(face));
                    let points = tet_node_ids.map(|node_id| boundary_nodes[&node_id]);
                    ...
                    if let Ok(tet) =
                        raw_refill_tet_with_rejection_reason(tet_node_ids, points, options)
                    {
                        if touches_boundary && candidate_keys.insert(sorted_tet_nodes(tet.node_ids))
                        {
                            candidates.push(tet.clone());
                        }
                        all_candidates.push(tet);
                    }
                }
            }
        }
    ...
    if let Some(refill) = exact_cover_refill_from_candidate_tets(cavity, &candidates, options)? {
...
fn exact_cover_refill_from_on_demand_interior_mates(
    cavity: &ConstrainedCavity,
    mut candidates: Vec<ConstrainedCavityRefillTet>,
    all_candidates: Vec<ConstrainedCavityRefillTet>,
    options: ConstrainedCavityRefillOptions,
) -> Result<Option<ConstrainedCavityRefill>, ConstrainedCavityValidationError> {
    if candidates.is_empty() || candidates.len() > MAX_BOUNDARY_EXACT_COVER_CANDIDATES {
        return Ok(None);
    ...
    for _ in 0..64 {
        ...
        if let Some(selected) = selected {
            let selected_tets = selected
                .into_iter()
                .map(|index| candidates[index].clone())
                .collect::<Vec<_>>();
            return refill_from_tets(cavity, selected_tets, options.volume_relative_tolerance)
                .map(Some);
        }
        ...
        let open_interior_dead_end_faces = trace
            ...
            .flat_map(|dead_end| open_interior_faces_from_tet_node_ids(&dead_end.selected_tets))
        ...
        for dead_end in &future_mate_dead_ends {
            ...
            let selected_indices = dead_end
                .selected_tets
                .iter()
                .filter_map(|selected_tet| {
                    candidates.iter().position(|candidate| {
                        sorted_tet_nodes(candidate.node_ids) == sorted_tet_nodes(*selected_tet)
                    })
                })
                .collect::<Vec<_>>();
            let mut face_counts = BTreeMap::<[u32; 3], usize>::new();
            for selected_index in &selected_indices {
                for selected_face in
                    tet_faces(candidates[*selected_index].node_ids).map(sorted_face)
                {
        ...
        for mate_face in mate_faces {
            ...
            for index in indices {
                ...
                if candidate_keys.insert(sorted_tet_nodes(candidate.node_ids)) {
...
fn open_interior_faces_from_tet_node_ids(tets: &[[u32; 4]]) -> Vec<[u32; 3]> {
    let mut face_counts = BTreeMap::<[u32; 3], usize>::new();
    for tet in tets {
        for face in tet_faces(*tet).map(sorted_face) {
            *face_counts.entry(face).or_default() += 1;
        }
    }
    face_counts
...
#[cfg(test)]
fn exact_cover_trace_selected_tets_by_reason(
    trace: &BoundaryExactCoverTrace,
) -> BTreeMap<&'static str, Vec<[u32; 4]>> {
    let mut selected_tets_by_reason = BTreeMap::<&'static str, Vec<[u32; 4]>>::new();
    for dead_end in &trace.dead_ends {
        selected_tets_by_reason
            .entry(dead_end.reason)
            .or_insert_with(|| dead_end.selected_tets.clone());
    }
    selected_tets_by_reason
}
...
pub(crate) fn diagnostic_boundary_exact_cover(
    ...
    let mut diagnostic = BoundaryExactCoverDiagnostic {
        ...
        dead_end_face: None,
        dead_end_depth: 0,
        dead_end_selected_tets: Vec::new(),
    ...
    let boundary_faces = cavity
        ...
        .collect::<BTreeSet<_>>();
    let mut candidates = Vec::<ConstrainedCavityRefillTet>::new();
    let mut solid_candidates = Vec::<ConstrainedCavityRefillTet>::new();
    for first in 0..node_ids.len() {
        for second in (first + 1)..node_ids.len() {
            for third in (second + 1)..node_ids.len() {
                for fourth in (third + 1)..node_ids.len() {
    ...
    if let Some(dead_end) = trace.dead_end {
        ...
        diagnostic.dead_end_selected_tets = dead_end.selected_tets;
...
pub(crate) fn diagnostic_boundary_exact_cover_mates_for_face(
    ...
    let search =
        BoundaryExactCoverSearch::new(cavity, &candidates, options.volume_relative_tolerance);
    let selected_keys = selected_tet_node_ids
        .iter()
        .copied()
        .map(sorted_tet_nodes)
        .collect::<BTreeSet<_>>();
    let selected = candidates
        ...
        .filter_map(|(index, candidate)| {
            selected_keys
                .contains(&sorted_tet_nodes(candidate.node_ids))
                .then_some(index)
        })
        .collect::<Vec<_>>();
    let mut face_counts = BTreeMap::<[u32; 3], usize>::new();
...
pub(crate) fn diagnostic_boundary_exact_cover_face_candidate_sources(
    ...
    for fourth_node_id in cavity_boundary_node_ids(cavity) {
        ...
        match raw_refill_tet_with_rejection_reason(node_ids, points, relaxed_options) {
            Ok(tet) => {
                diagnostic.relaxed_pass_count += 1;
                diagnostic
                    .relaxed_candidate_node_ids
                    .push(sorted_tet_nodes(tet.node_ids));
            }
            Err(reason) => {
                *diagnostic
...
pub fn selected_exact_cover_face_count_blockers(
    ...
    let selected_keys = selected_tet_node_ids
        ...
        .map(sorted_tet_nodes)
        .collect::<BTreeSet<_>>();
    for selected_tet in selected_tet_node_ids {
        for face in tet_faces(*selected_tet).map(sorted_face) {
            *face_counts.entry(face).or_default() += 1;
        }
    }
    let mut selected_tets_by_face = BTreeMap::<[u32; 3], Vec<[u32; 4]>>::new();
    for selected_tet in selected_tet_node_ids {
        for face in tet_faces(*selected_tet).map(sorted_face) {
            selected_tets_by_face
                .entry(face)
                .or_default()
                .push(*selected_tet);
        }
        ...
        for second in (first + 1)..node_ids.len() {
            for third in (second + 1)..node_ids.len() {
                for fourth in (third + 1)..node_ids.len() {
                    ...
                    if selected_keys.contains(&sorted_tet_nodes(tet_node_ids)) {
                        continue;
                    }
                    let points = tet_node_ids.map(|node_id| boundary_node_map[&node_id]);
                    if point_in_closed_triangle_surface(
                    ...
                    candidate_count += 1;
                    let mut conflicting_faces = Vec::<[u32; 3]>::new();
                    let mut blocking_selected_tets = Vec::<[u32; 4]>::new();
                    for candidate_face in tet_faces(tet.node_ids).map(sorted_face) {
                        let count = face_counts.get(&candidate_face).copied().unwrap_or(0);
                        ...
                        if conflicts {
                            conflicting_faces.push(candidate_face);
                            if let Some(selected_tets) = selected_tets_by_face.get(&candidate_face)
                            {
                                blocking_selected_tets.extend(selected_tets.iter().copied());
                            }
                        }
                    }
                    if !conflicting_faces.is_empty() {
                        blocking_selected_tets.sort();
                        blocking_selected_tets.dedup();
                        blockers.push(BoundaryExactCoverFaceCountBlocker {
                            node_ids: tet.node_ids,
                            exact_scaled_jacobian: tet.exact_scaled_jacobian,
                            conflicting_faces,
                            blocking_selected_tets,
                        });
                    }
                }
            }
    ...
    Ok(BoundaryExactCoverFaceCountBlockers {
        target_face,
        selected_tet_count: selected_tet_node_ids.len(),
        candidate_count,
...
pub fn selected_exact_cover_saturated_component(
    ...
    let saturated_faces = selected_tets_by_face
        .iter()
        .filter_map(|(face, selected_tets)| {
            (!boundary_faces.contains(face) && selected_tets.len() >= 2).then_some(*face)
        })
        .collect::<BTreeSet<_>>();
    let mut component_faces = BTreeSet::<[u32; 3]>::new();
    let mut component_tets = BTreeSet::<[u32; 4]>::new();
    ...
    while let Some(face) = pending.pop() {
        ...
        let Some(selected_tets) = selected_tets_by_face.get(&face) else {
            continue;
        };
        for selected_tet in selected_tets {
            if component_tets.insert(*selected_tet) {
                for adjacent_face in tet_faces(*selected_tet).map(sorted_face) {
    ...
    BoundaryExactCoverSaturatedComponent {
        ...
        component_tet_count: component_tets.len(),
        component_faces: component_faces.into_iter().collect(),
        component_tets: component_tets.into_iter().collect(),
    }
}
...
pub(crate) fn diagnostic_boundary_exact_cover_with_interior_mate_closure(
        ...
        for second in (first + 1)..node_ids.len() {
            for third in (second + 1)..node_ids.len() {
                for fourth in (third + 1)..node_ids.len() {
                    ...
                    let touches_boundary = tet_faces(tet.node_ids)
                        .map(sorted_face)
                        .iter()
                        .any(|face| boundary_faces.contains(face));
                    if touches_boundary && candidate_keys.insert(sorted_tet_nodes(tet.node_ids)) {
    ...
    let Some(selected) = selected else {
        ...
        return Ok(BoundaryExactCoverInteriorMateClosureDiagnostic {
            ...
            dead_end_depth: dead_end
                ...
                .unwrap_or(0),
            dead_end_selected_tets: dead_end
                .as_ref()
                .map(|dead_end| dead_end.selected_tets.clone())
                .unwrap_or_default(),
            ...
            dead_end_selected_tets_by_reason: exact_cover_trace_selected_tets_by_reason(&trace),
            dead_end_selected_roles_by_reason: exact_cover_trace_selected_roles_by_reason(&trace),
            unforced_found_cover: false,
            unforced_selected_tet_count: 0,
    ...
    Ok(BoundaryExactCoverInteriorMateClosureDiagnostic {
        ...
        injected_candidate_count: candidates.len().saturating_sub(initial_candidate_count),
        found_cover: true,
        selected_tet_count: selected.len(),
        search_attempt_count: search.attempts,
        reason: "cover_found",
        dead_end_reason: "not_evaluated",
        dead_end_face: None,
        ...
        dead_end_reason_histogram: BTreeMap::new(),
        dead_end_faces_by_reason: BTreeMap::new(),
        dead_end_selected_tets_by_reason: BTreeMap::new(),
...
pub(crate) fn diagnostic_boundary_exact_cover_with_on_demand_interior_mates_excluding(
    ...
    let mut candidates = Vec::<ConstrainedCavityRefillTet>::new();
    let mut candidate_keys = BTreeSet::<[u32; 4]>::new();
    let excluded_keys = excluded_tet_node_ids
        .iter()
        .copied()
        .map(sorted_tet_nodes)
        .collect::<BTreeSet<_>>();
            ...
            for third in (second + 1)..node_ids.len() {
                for fourth in (third + 1)..node_ids.len() {
                    ...
                    let touches_boundary = tet_faces(tet.node_ids)
                        ...
                        .iter()
                        .any(|face| boundary_faces.contains(face));
                    let candidate_key = sorted_tet_nodes(tet.node_ids);
                    if touches_boundary
    ...
    for _ in 0..64 {
        ...
        if future_mate_dead_ends.is_empty() && no_candidate_dead_end_faces.is_empty() {
            ...
            return Ok(BoundaryExactCoverInteriorMateClosureDiagnostic {
                ...
                dead_end_selected_tets: dead_end.selected_tets,
        ...
        for mate_face in mate_faces {
            ...
            for index in indices {
                ...
                let candidate = &all_candidates[index];
                let candidate_key = sorted_tet_nodes(candidate.node_ids);
...
pub(crate) fn diagnostic_support_node_exact_cover(
    ...
    Ok(SupportNodeExactCoverDiagnostic {
        ...
        selected_tet_count: selected.as_ref().map(Vec::len).unwrap_or(0),
        search_attempt_count: search.attempts,
...
fn diagnostic_missing_face_shared_cap_stitch_with_link(
    ...
    for patch in &missing_face_patches {
        let faces = patch
            ...
            .map(|face_index| missing_faces[*face_index])
            .collect::<Vec<_>>();
        if let Some((coordinates_m, mut cap_tets)) = best_shared_patch_cap_for_faces(
            &faces,
            cavity_centroid,
            next_node_id,
            &boundary_node_map,
            ...
            diagnostic.capped_face_count += cap_tets.len();
            *diagnostic
                .patch_capped_face_count_histogram
                .entry(cap_tets.len())
                .or_default() += 1;
            candidate_tets.append(&mut cap_tets);
            next_node_id = next_node_id.saturating_add(1);
            continue;
        }
            ...
            diagnostic.uncapped_faces.extend(
                patch
                    ...
                    .filter(|face_index| {
                        ...
                        !candidate_tets[cap_tet_start..]
                            .iter()
                            .any(|tet| tet_faces(tet.node_ids).map(sorted_face).contains(&face))
                    })
                    .map(|face_index| missing_faces[*face_index]),
            );
            *diagnostic
...
#[cfg(test)]
fn best_shared_patch_cap_for_faces(
    faces: &[[u32; 3]],
    cavity_centroid: [f64; 3],
    apex_node_id: u32,
    node_coordinates: &BTreeMap<u32, [f64; 3]>,
    boundary_triangles: &[Triangle3],
    options: ConstrainedCavityRefillOptions,
) -> Option<([f64; 3], Vec<ConstrainedCavityRefillTet>)> {
    ...
    let mut best = None::<([f64; 3], Vec<ConstrainedCavityRefillTet>, f64)>;
    for point in candidate_points {
        let mut patch_tets = Vec::<ConstrainedCavityRefillTet>::with_capacity(faces.len());
        for face in faces {
            let tet_points = [
                node_coordinates[&face[0]],
                node_coordinates[&face[1]],
        ...
        if patch_tets.len() != faces.len() {
            continue;
        }
        let min_quality = patch_tets
            .iter()
            .map(|tet| tet.exact_scaled_jacobian)
            .fold(f64::INFINITY, f64::min);
        if best
            ...
            .is_none_or(|(_, _, best_quality)| min_quality > *best_quality)
        {
            best = Some((point, patch_tets, min_quality));
        }
    }
    best.map(|(point, patch_tets, _)| (point, patch_tets))
}
...
fn patch_steiner_candidate_points(
    surface_point: [f64; 3],
    cavity_centroid: [f64; 3],
    boundary_triangles: &[Triangle3],
) -> Vec<[f64; 3]> {
...
fn refill_component_node_map(
    tets: &[ConstrainedCavityRefillTet],
    nodes: &[ConstrainedCavityNode],
) -> Result<BTreeMap<u32, Point3>, ConstrainedCavityRefillTetFlipError> {
    let node_map = nodes
        .iter()
        .map(|node| (node.node_id, node.coordinates_m))
        .collect::<BTreeMap<_, _>>();
...
fn refill_tets_from_flip_candidate(
    tets: &[ConstrainedCavityRefillTet],
    node_map: &BTreeMap<u32, Point3>,
    flip: &LocalTetFlipCandidate,
    options: ConstrainedCavityRefillOptions,
) -> Result<Vec<ConstrainedCavityRefillTet>, ConstrainedCavityRefillTetFlipError> {
    let removed_indices = flip
        ...
        .collect::<BTreeSet<_>>();
    if removed_indices.iter().any(|index| *index >= tets.len()) {
        return Err(ConstrainedCavityRefillTetFlipError::InvalidFlipTopology {
            reason: "removed_tet_out_of_bounds",
        });
    }
    let mut candidate_tets = tets
        .iter()
        .enumerate()
        .filter_map(|(index, tet)| (!removed_indices.contains(&index)).then_some(tet.clone()))
        .collect::<Vec<_>>();
    let mut candidate_keys = candidate_tets
        .iter()
        .map(|tet| sorted_tet_nodes(tet.node_ids))
        .collect::<BTreeSet<_>>();
    for node_ids in &flip.created_tets {
        ...
        for (point, node_id) in points.iter_mut().zip(node_ids) {
            *point = *node_map
                .get(node_id)
                .ok_or(ConstrainedCavityRefillTetFlipError::MissingNode { node_id: *node_id })?;
        }
        match raw_refill_tet_with_rejection_reason(*node_ids, points, options) {
            Ok(tet) => candidate_tets.push(tet),
            Err(reason) => {
                return Err(ConstrainedCavityRefillTetFlipError::RejectedCreatedTet {
                    node_ids: *node_ids,
                    reason,
                });
            }
...
fn local_tet_flip_error_reason(error: &LocalTetFlipError) -> &'static str {
    match error {
        LocalTetFlipError::DegenerateTet { .. } => "degenerate_tet",
        LocalTetFlipError::NoSharedFace => "no_shared_face",
        LocalTetFlipError::NoSharedEdge => "no_shared_edge",
        LocalTetFlipError::InvalidEdgeRing => "invalid_edge_ring",
        LocalTetFlipError::InvalidQualityThresholds => "invalid_quality_thresholds",
        LocalTetFlipError::MissingNode { .. } => "missing_node",
        LocalTetFlipError::NonPositiveVolume { .. } => "non_positive_volume",
        LocalTetFlipError::VolumeBelowThreshold { .. } => "volume_below_threshold",
        LocalTetFlipError::ScaledJacobianBelowThreshold { .. } => "scaled_jacobian_below_threshold",
    }
}
...
fn improve_refill_with_local_flips(
    ...
    for left_index in 0..refill.tets.len() {
        for middle_index in (left_index + 1)..refill.tets.len() {
            for right_index in (middle_index + 1)..refill.tets.len() {
                let tets = [
                    LocalTet {
                        tet_id: left_index as u32,
                        node_ids: refill.tets[left_index].node_ids,
                    ...
                    LocalTet {
                        tet_id: middle_index as u32,
                        node_ids: refill.tets[middle_index].node_ids,
                    },
                    LocalTet {
                        tet_id: right_index as u32,
                        node_ids: refill.tets[right_index].node_ids,
                ...
                for edge in common_tet_edges(tets.map(|tet| tet.node_ids)) {
                    let Ok(flip) = three_to_two_edge_flip_candidate(tets, edge) else {
                        continue;
                    };
                    if evaluate_local_tet_flip_quality(&flip, &coordinates, thresholds).is_err() {
                        continue;
...
fn refill_from_local_flip_candidate(
    ...
    if removed_indices
        .iter()
        .any(|index| *index >= refill.tets.len())
    {
        return None;
    }
    ...
    let mut candidate =
        refill_from_tets(cavity, candidate_tets, options.volume_relative_tolerance).ok()?;
