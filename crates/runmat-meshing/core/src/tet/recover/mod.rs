...
use crate::contracts::{
    MeshingStage, ProtectedBoundaryComplex, StageEvidence, TetMesh, TopologyEntityId,
};

pub const MODULE_PURPOSE: &str = "source-edge, source-face, and material-interface recovery queues";

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum TetRecoveryError {
...
impl std::fmt::Display for TetRecoveryError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::InvalidProtectedBoundaryComplex => {
                write!(formatter, "Tet recovery requires a validated PLC")
            }
            Self::EmptyTetMesh => write!(formatter, "Tet recovery requires a non-empty Tet mesh"),
...
impl std::error::Error for TetRecoveryError {}

#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct TetRecoveryQueue {
    #[serde(default)]
    pub items: Vec<TetRecoveryQueueItem>,
    pub evidence: StageEvidence,
}

#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct TetRecoveryQueueItem {
    pub item_id: String,
    pub kind: TetRecoveryKind,
    pub status: TetRecoveryStatus,
    #[serde(default)]
...
pub fn build_recovery_queue_from_plc(
    ...
    tet_mesh: &TetMesh,
) -> Result<TetRecoveryQueue, TetRecoveryError> {
    if !plc.validation.valid_for_volume_meshing() {
        return Err(TetRecoveryError::InvalidProtectedBoundaryComplex);
    }
    if tet_mesh.nodes.is_empty() || tet_mesh.elements.is_empty() {
        return Err(TetRecoveryError::EmptyTetMesh);
    }

    let recovered_face_keys = tet_mesh
        .boundary_faces
        .iter()
        .map(|face| {
            (
        ...
        .collect::<BTreeSet<_>>();
    let recovered_boundary_edges = tet_mesh
        .boundary_faces
        .iter()
        .flat_map(|face| topology_face_edges(face.node_ids.clone()))
        .collect::<BTreeSet<_>>();
    let recovered_material_interfaces = tet_mesh
        .elements
        .iter()
        .map(|element| element.material_region_id.clone())
        .collect::<BTreeSet<_>>();

    let mut items = Vec::<TetRecoveryQueueItem>::new();
    for facet in &plc.facets {
        ...
        if !recovered_face_keys.contains(&face_key) {
            return Err(TetRecoveryError::MissingSourceFaceRecovery {
                face_id: facet.source_face_id.id.clone(),
            });
        }
        items.push(TetRecoveryQueueItem {
            item_id: format!("source_face:{}", facet.facet_id.id),
            kind: TetRecoveryKind::SourceFace,
            status: TetRecoveryStatus::Recovered,
    ...
    for material_interface_id in material_interfaces {
        if !recovered_material_interfaces.contains(&material_interface_id) {
            return Err(TetRecoveryError::MissingMaterialInterfaceRecovery {
                material_interface_id,
            });
        }
        items.push(TetRecoveryQueueItem {
            item_id: format!("material_interface:{material_interface_id}"),
            kind: TetRecoveryKind::MaterialInterface,
            status: TetRecoveryStatus::Recovered,
    ...
    evidence.entity_counts.insert(
        ...
        items
            .iter()
            .filter(|item| item.kind == TetRecoveryKind::SourceFace)
            .count(),
    );
    evidence.entity_counts.insert(
        ...
        items
            .iter()
            .filter(|item| item.kind == TetRecoveryKind::SourceEdge)
            .count(),
    );
    evidence.entity_counts.insert(
        ...
        items
            .iter()
            .filter(|item| item.kind == TetRecoveryKind::MaterialInterface)
            .count(),
    );

    Ok(TetRecoveryQueue { items, evidence })
}
...
mod tests {
    use super::*;
    use crate::contracts::{
        PlcFacet, PlcNode, PlcProtectedEdge, PlcValidationSummary, Tet4Element, TetBoundaryFace,
        TetMeshNode,
    };

    #[test]
    fn builds_recovery_queue_for_recovered_plc_constraints() {
        let queue = build_recovery_queue_from_plc(&single_facet_plc(), &single_facet_tet_mesh())
            .expect("matching Tet mesh should recover PLC constraints");
        ...
        assert!(queue
            .items
            .iter()
            .all(|item| item.status == TetRecoveryStatus::Recovered));
    }

    #[test]
    fn recovery_queue_rejects_missing_source_face() {
        let mut mesh = single_facet_tet_mesh();
        ...
        assert_eq!(
            ...
            Err(TetRecoveryError::MissingSourceFaceRecovery {
    ...
    fn recovery_queue_rejects_missing_source_edge() {
        ...
        assert_eq!(
            build_recovery_queue_from_plc(&plc, &single_facet_tet_mesh()),
            Err(TetRecoveryError::MissingSourceEdgeRecovery {
    ...
    fn recovery_queue_rejects_missing_material_interface() {
        ...
        assert_eq!(
            build_recovery_queue_from_plc(&single_facet_plc(), &mesh),
            Err(TetRecoveryError::MissingMaterialInterfaceRecovery {
    ...
    fn single_facet_tet_mesh() -> TetMesh {
        TetMesh {
            mesh_id: "single_facet_tet".to_string(),
            nodes: vec![
                tet_node(
                    entity(MeshingStage::ProtectedBoundaryComplex, "0"),
                    [0.0, 0.0, 0.0],
                ...
                tet_node(entity(MeshingStage::TetMesh, "3"), [0.0, 0.0, 1.0]),
            ],
            elements: vec![Tet4Element {
                element_id: entity(MeshingStage::TetMesh, "tet_1"),
                node_ids: [
                    entity(MeshingStage::ProtectedBoundaryComplex, "0"),
                    entity(MeshingStage::ProtectedBoundaryComplex, "1"),
                    entity(MeshingStage::ProtectedBoundaryComplex, "2"),
                    entity(MeshingStage::TetMesh, "3"),
                ],
                material_region_id: "solid_body".to_string(),
            }],
            boundary_faces: vec![TetBoundaryFace {
    ...
    fn tet_node(node_id: TopologyEntityId, coordinates_m: [f64; 3]) -> TetMeshNode {
        TetMeshNode {
            node_id,
            coordinates_m,
        }
