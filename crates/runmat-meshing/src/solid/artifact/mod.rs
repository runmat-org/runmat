...
use runmat_meshing_core::{
    contracts::{
        ...
        BoundaryElementKind, MeshEntityProvenance, MeshingStage, SourceEntityKind, SurfaceMesh,
        TopologyEntityId, VolumeElementKind,
    },
    quality::{
        predicate::{
...
use backend_summary::{
    build_backend_summary, BackendSummaryInput, SOLID_PLC_TETRAHEDRON_ALGORITHM,
};
...
pub(super) fn analysis_artifact_from_tetrahedron_mesh(
    ...
    let mut artifact = AnalysisMeshArtifact {
        ...
        backend: build_backend_summary(BackendSummaryInput {
            surface_element_count: surface.triangles.len(),
            tetrahedron_mesh: &tetrahedron_mesh,
            recovery_queue,
            initial_backend_quality: &initial_backend_quality,
            backend_quality,
        }),
