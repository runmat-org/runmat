...
pub(crate) fn sub(left: [f64; 3], right: [f64; 3]) -> [f64; 3] {
    [left[0] - right[0], left[1] - right[1], left[2] - right[2]]
}
...
pub(crate) fn tangent_angle(left: [f64; 3], right: [f64; 3]) -> f64 {
    dot(left, right).clamp(-1.0, 1.0).acos()
}

pub(crate) fn point_segment_distance(point: [f64; 3], left: [f64; 3], right: [f64; 3]) -> f64 {
...
pub(crate) fn metric_length(delta: [f64; 3], metric: MetricTensor3) -> f64 {
    let [x, y, z] = delta;
    (metric.xx * x * x
        + metric.yy * y * y
        + metric.zz * z * z
...
pub(crate) fn average_metric(left: MetricTensor3, right: MetricTensor3) -> MetricTensor3 {
    MetricTensor3 {
