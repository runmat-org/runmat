...
pub(super) fn sorted_tetrahedron_nodes(mut node_ids: [u32; 4]) -> [u32; 4] {
    node_ids.sort();
    node_ids
}
...
pub(super) fn common_tetrahedron_edges(tetrahedra: [[u32; 4]; 3]) -> Vec<[u32; 2]> {
    let mut edge_counts = BTreeMap::<[u32; 2], usize>::new();
    for tetrahedron in tetrahedra {
        for edge in tetrahedron_edges(tetrahedron) {
...
pub(super) fn tetrahedron_edges(node_ids: [u32; 4]) -> [[u32; 2]; 6] {
    [
...
pub(super) fn tetrahedron_faces(node_ids: [u32; 4]) -> [[u32; 3]; 4] {
    [
