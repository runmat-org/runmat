...
#[test]
fn exact_quality_repair_relocates_interior_seed() {
    let mut mesh = exact_quality_fixture([0.001, 0.001, 0.000001]);

    let report = repair_tetrahedron_mesh_exact_quality(
        &mut mesh,
        TetrahedronMeshExactQualityRepairOptions {
            exact_quality: TetrahedronExactQualityRepairOptions {
    ...
    assert_eq!(report.unrepaired_seed_count, 0);
    assert!(mesh.quality_optimized);
    assert_ne!(mesh.nodes[4].coordinates_m, [0.001, 0.001, 0.000001]);
    assert_eq!(
        mesh.evidence.entity_counts[TETRAHEDRON_EXACT_QUALITY_REPAIR_PASS_COUNT],
        1
    );
    assert_eq!(
        mesh.evidence.entity_counts[TETRAHEDRON_EXACT_QUALITY_SEED_STAR_RELOCATION_COUNT],
        1
    );
}

#[test]
fn exact_quality_repair_records_unrepaired_boundary_only_seed() {
    let mut mesh = boundary_only_exact_quality_fixture();
    ...
    assert_eq!(report.unrepaired_seed_count, 1);
    assert_eq!(report.rejected_by_reason["no_accepted_relocation"], 1);
    assert!(mesh.quality_optimized);
    assert_eq!(
        mesh.evidence.entity_counts[TETRAHEDRON_EXACT_QUALITY_UNREPAIRED_TOTAL_COUNT],
        1
    );
    assert_eq!(
        mesh.evidence.entity_counts[TETRAHEDRON_EXACT_QUALITY_UNREPAIRED_INTERIOR_SEED_COUNT],
        1
    );
}

#[test]
fn exact_quality_repair_records_unrepaired_seed_count_when_budget_is_zero() {
    let mut mesh = exact_quality_fixture([0.001, 0.001, 0.000001]);
    ...
    assert_eq!(report.relocated_seed_count, 0);
    assert!(report.unrepaired_seed_count > 0);
    assert!(!mesh.quality_optimized);
    assert_eq!(
        mesh.evidence.entity_counts[TETRAHEDRON_EXACT_QUALITY_UNREPAIRED_TOTAL_COUNT],
...
fn exact_quality_fixture(interior: Point3) -> TetrahedronMesh {
    TetrahedronMesh {
        mesh_id: "exact_quality_fixture".to_string(),
...
fn boundary_only_exact_quality_fixture() -> TetrahedronMesh {
    TetrahedronMesh {
        mesh_id: "boundary_only_exact_quality_fixture".to_string(),
        tetrahedron_generation_family: "unknown".to_string(),
        nodes: vec![
            node("0", [0.0, 0.0, 0.0]),
            node("1", [1.0, 0.0, 0.0]),
