...
#[test]
fn sliver_removal_accepts_interior_node_relocation() {
    let mut mesh = sliver_fixture([0.001, 0.001, 0.000001]);

    let report = remove_tetrahedron_mesh_slivers(
        &mut mesh,
        TetrahedronMeshSliverRemovalOptions {
            sliver: SliverRecoveryOptions {
                sliver_aspect_ratio: 20.0,
                min_exact_scaled_jacobian: 0.01,
                exact_quality_tolerance: 1.0e-12,
            },
            max_attempted_elements: 4,
            max_accepted_edits: 1,
            relaxation: 1.0,
        },
    );

    assert_eq!(report.attempted_element_count, 1);
    assert_eq!(report.accepted_edit_count, 1);
    assert_eq!(report.rejected_element_count, 0);
    assert!(mesh.quality_optimized);
    assert_ne!(mesh.nodes[4].coordinates_m, [0.001, 0.001, 0.000001]);
    assert_eq!(
        mesh.evidence.entity_counts[TETRAHEDRON_OPTIMIZATION_SLIVER_REMOVAL_ATTEMPT_COUNT],
        1
    );
    assert_eq!(
        mesh.evidence.entity_counts[TETRAHEDRON_OPTIMIZATION_SLIVER_REMOVAL_ACCEPTED_COUNT],
        1
    );
}

#[test]
fn sliver_removal_rejects_boundary_only_sliver() {
    let mut mesh = boundary_only_sliver_fixture();
    ...
    assert_eq!(report.attempted_element_count, 1);
    assert_eq!(report.accepted_edit_count, 0);
    assert_eq!(report.rejected_element_count, 1);
    assert_eq!(report.rejected_by_reason["no_accepted_relocation"], 1);
    assert!(mesh.quality_optimized);
}

#[test]
fn sliver_removal_reports_budget_limited_slivers() {
    let mut mesh = sliver_fixture([0.001, 0.001, 0.000001]);
    ...
    assert_eq!(report.attempted_element_count, 0);
    assert_eq!(report.accepted_edit_count, 0);
    assert!(report.budget_limited_element_count > 0);
    assert!(!mesh.quality_optimized);
    assert_eq!(
        mesh.evidence.entity_counts[TETRAHEDRON_OPTIMIZATION_SLIVER_REMOVAL_BUDGET_LIMIT_COUNT],
        report.budget_limited_element_count
    );
}

fn sliver_fixture(interior: Point3) -> TetrahedronMesh {
    TetrahedronMesh {
        mesh_id: "sliver_removal_fixture".to_string(),
...
fn boundary_only_sliver_fixture() -> TetrahedronMesh {
    TetrahedronMesh {
        mesh_id: "boundary_only_sliver_fixture".to_string(),
        tetrahedron_generation_family: "unknown".to_string(),
        nodes: vec![
            node("0", [0.0, 0.0, 0.0]),
            node("1", [1.0, 0.0, 0.0]),
