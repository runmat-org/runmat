...
use crate::protected_edges::{face_edges, source_edge_ids_for_face_edges};
...
pub(super) fn recover_boundary_diagonal_flip(
    plc: &ProtectedBoundaryComplex,
    tetrahedron_mesh: &mut TetrahedronMesh,
    recovered_diagonal: [TopologyEntityId; 2],
) -> Result<usize, BoundaryDiagonalRecoveryRejection> {
    let adjacent_facets = plc
        .facets
        .iter()
        .filter(|facet| face_contains_edge(facet.node_ids.clone(), recovered_diagonal.clone()))
        .collect::<Vec<_>>();
    if adjacent_facets.len() != 2 {
        return Err(BoundaryDiagonalRecoveryRejection::AdjacentFacetCount);
    }
    ...
    let opposite_nodes = adjacent_facets
        ...
        .filter_map(|facet| {
            facet
                .node_ids
                .iter()
                .find(|node_id| !recovered_diagonal.contains(node_id))
                .cloned()
        })
        .collect::<Vec<_>>();
    if opposite_nodes.len() != 2 || opposite_nodes[0] == opposite_nodes[1] {
        return Err(BoundaryDiagonalRecoveryRejection::AdjacentFacetTopology);
    }

    let current_boundary_face_keys =
        current_boundary_face_keys(&recovered_diagonal, &opposite_nodes);
    if !current_boundary_face_keys
        .iter()
        .all(|face_key| boundary_face_exists(tetrahedron_mesh, face_key))
    {
        return Err(BoundaryDiagonalRecoveryRejection::CurrentBoundaryFaces);
    }
    ...
    let left_element_index = *element_face_index
        ...
        .ok_or(BoundaryDiagonalRecoveryRejection::ElementTopology)?;
    let right_element_index = *element_face_index
        .get(&current_boundary_face_keys[1])
        .ok_or(BoundaryDiagonalRecoveryRejection::ElementTopology)?;
    if left_element_index == right_element_index {
        return Err(BoundaryDiagonalRecoveryRejection::ElementTopology);
    }
    ...
    if left_element.material_region_id != right_element.material_region_id {
        return Err(BoundaryDiagonalRecoveryRejection::MaterialRegionMismatch);
    }
    ...
    .ok_or(BoundaryDiagonalRecoveryRejection::QualityGate)?;
...
pub(super) enum BoundaryDiagonalRecoveryRejection {
    AdjacentFacetCount,
...
impl BoundaryDiagonalRecoveryRejection {
    pub(super) fn absent_source_edge_evidence_key(self) -> &'static str {
        match self {
    ...
    pub(super) fn source_face_evidence_key(self) -> &'static str {
        match self {
            Self::AdjacentFacetCount => {
            ...
            Self::ElementTopology => "rejected_source_face_diagonal_recovery_element_topology",
            Self::MaterialRegionMismatch => {
                "rejected_source_face_diagonal_recovery_material_region_mismatch"
            }
            Self::QualityGate => "rejected_source_face_diagonal_recovery_quality_gate",
        }
    }
}
...
fn current_boundary_face_keys(
    ...
    [
        sorted_topology_ids([
            recovered_diagonal[0].clone(),
            opposite_nodes[0].clone(),
            opposite_nodes[1].clone(),
        ]),
        sorted_topology_ids([
            recovered_diagonal[1].clone(),
            opposite_nodes[0].clone(),
            opposite_nodes[1].clone(),
        ]),
    ]
...
fn shared_support_node(
    ...
    let excluded = recovered_diagonal
        .iter()
