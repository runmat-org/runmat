...
pub(super) fn element_quality_for_nodes(
    ...
    for element in volume_elements {
        ...
        let volume_m3 = tetrahedron_volume(node_ids, nodes);
        if !volume_m3.is_finite() || volume_m3 <= 0.0 {
            return None;
        }
        let aspect_ratio = tetrahedron_aspect_ratio(node_ids, nodes);
        let exact_scaled_jacobian = tetrahedron_points(node_ids, nodes)
...
pub(super) fn orient_tetrahedron(mut node_ids: [u32; 4], nodes: &[AnalysisMeshNode]) -> [u32; 4] {
    if tetrahedron_volume(node_ids, nodes) < 0.0 {
        node_ids.swap(0, 1);
    }
    node_ids
}

pub(super) fn tetrahedron_volume(node_ids: [u32; 4], nodes: &[AnalysisMeshNode]) -> f64 {
    let Some([a, b, c, d]) = tetrahedron_points(node_ids, nodes) else {
        return 0.0;
    };
    dot(sub(b, a), cross(sub(c, a), sub(d, a))) / 6.0
}

pub(super) fn tetrahedron_aspect_ratio(node_ids: [u32; 4], nodes: &[AnalysisMeshNode]) -> f64 {
    let Some(points) = tetrahedron_points(node_ids, nodes) else {
        return f64::INFINITY;
    };
...
pub(super) fn tetrahedron_points(
    node_ids: [u32; 4],
    nodes: &[AnalysisMeshNode],
) -> Option<[[f64; 3]; 4]> {
    Some([
