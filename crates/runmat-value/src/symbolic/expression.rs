...
mod tests {
    ...
    fn substitutes_and_simplifies_symbols() {
        ...
        let expr =
            SymbolicExpr::function(SymbolicFunction::Cos, SymbolicExpr::add_expr(x.clone(), h));
        assert_eq!(
    ...
    #[test]
    fn differentiates_and_formats_symbolic_functions() {
        let x = SymbolicExpr::variable("x");
        let trig = SymbolicExpr::function(SymbolicFunction::Sin, x);
        assert_eq!(trig.derivative("x").to_string(), "cos(x)");

        let named = SymbolicExpr::function_reference("Y", vec!["X".to_string()]);
        assert_eq!(named.to_string(), "Y(X)");
        assert_eq!(named.derivative("X").to_string(), "diff(Y(X), X)");
        assert_eq!(
            named
                .substitute("X", &SymbolicExpr::constant(0.0))
                .to_string(),
            "Y(0)"
        );
    }
    ...
    #[test]
    fn formats_equations_negative_functions_and_power_bases() {
        let x = SymbolicExpr::variable("x");
        assert_eq!(
            SymbolicExpr::equation(
                SymbolicExpr::function_call("Y", vec![SymbolicExpr::constant(0.0)]),
                SymbolicExpr::constant(0.0),
            )
            .to_string(),
            "Y(0) == 0"
        ...
        assert_eq!(
            SymbolicExpr::neg_expr(SymbolicExpr::function(SymbolicFunction::Sin, x.clone()))
                .to_string(),
            "-sin(x)"
        );
        assert_eq!(
            SymbolicExpr::pow_expr(SymbolicExpr::neg_expr(x), SymbolicExpr::constant(2.0))
                .to_string(),
            "(-x)^2"
        );
    }
    ...
    #[test]
    fn preserves_undefined_constant_subexpressions() {
        let zero_product = SymbolicExpr::mul_expr(
            SymbolicExpr::constant(0.0),
            SymbolicExpr::div_expr(SymbolicExpr::constant(1.0), SymbolicExpr::constant(0.0)),
        );
        assert!(matches!(zero_product, SymbolicExpr::Mul(_, _)));
        assert!(zero_product.has_undefined_constant_subexpression());

        let singular =
            SymbolicExpr::pow_expr(SymbolicExpr::constant(0.0), SymbolicExpr::constant(-1.0));
        assert!(matches!(singular, SymbolicExpr::Pow(_, _)));
        assert!(singular.has_undefined_constant_subexpression());
    }

    #[test]
    fn exposes_only_non_nan_numeric_constants() {
        assert_eq!(
    ...
    fn oversized_rational_simplification_does_not_panic() {
        ...
        assert!(matches!(
            SymbolicExpr::add_expr(lhs, rhs),
            SymbolicExpr::Rational { .. }
        ));
    }

    #[test]
    fn heaviside_and_dirac_preserve_symbolic_semantics() {
        let x = SymbolicExpr::variable("x");
        let heaviside = SymbolicExpr::function(SymbolicFunction::Heaviside, x.clone());
        assert_eq!(heaviside.to_string(), "heaviside(x)");
        assert_eq!(
        ...
        assert_eq!(heaviside.derivative("x").to_string(), "dirac(x)");

        let dirac = SymbolicExpr::function(SymbolicFunction::Dirac, x);
        assert_eq!(dirac.derivative("x").to_string(), "dirac(1, x)");
        assert_eq!(
            dirac.derivative("x").derivative("x").to_string(),
            "dirac(2, x)"
        );
    }
}
