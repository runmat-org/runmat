...
fn left_scalar_and_transpose_on_device() {
    ...
    if let Value::GpuTensor(_) = r { } else { panic!("expected gpu result"); }
    let r_host = runmat_runtime::call_builtin("gather", &[r]).unwrap();
    if let Value::Tensor(tr) = r_host { assert_eq!(tr.data, vec![9.0, 8.0, 7.0, 6.0]); }

    // s ./ G
    let q = runmat_runtime::elementwise::elementwise_div(&Value::Num(8.0), &g).unwrap();
    if let Value::GpuTensor(_) = q { } else { panic!("expected gpu result"); }

    // transpose(G)
    let gt = runmat_runtime::transpose(g).unwrap();
    if let Value::GpuTensor(_) = gt { } else { panic!("expected gpu result"); }
    let gt_host = runmat_runtime::call_builtin("gather", &[gt]).unwrap();
    if let Value::Tensor(tt) = gt_host { assert_eq!(tt.shape, vec![2,2]); }
}
...
fn reductions_mean_min_max_on_device() {
    ...
    if let Value::GpuTensor(_) = m { } else { panic!("expected gpu handle"); }
    let m_host = runmat_runtime::call_builtin("gather", &[m]).unwrap();
    if let Value::Tensor(tm) = m_host { assert_eq!(tm.data, vec![2.5]); }
