...
fn logical_size_numel_ndims() {
	...
	if let Value::Tensor(t) = sz {
		...
		if let Value::Num(n) = ne { assert_eq!(prod as f64, n); } else { panic!(); }
	} else { panic!(); }
	let nd = runmat_runtime::call_builtin("ndims", &[v.clone()]).unwrap(); if let Value::Num(n) = nd { assert_eq!(n, 2.0); } else { panic!(); }
}
...
fn logical_from_numeric_and_stringarray() {
	...
	if let Value::LogicalArray(la) = l { assert_eq!(la.data, vec![0,1,1]); } else { panic!(); }
	let sa = runmat_builtins::StringArray::new(vec!["".to_string(), "a".to_string()], vec![2,1]).unwrap();
	let l2 = runmat_runtime::call_builtin("logical", &[Value::StringArray(sa)]).unwrap();
	if let Value::LogicalArray(la) = l2 { assert_eq!(la.data, vec![0,1]); } else { panic!(); }
}
