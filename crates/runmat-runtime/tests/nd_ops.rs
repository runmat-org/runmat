...
fn squeeze_basic() {
    ...
    match v { Value::Tensor(tt) => { assert_eq!(tt.shape, vec![2,2]); }, _ => panic!("expected tensor") }
}
...
fn permute_swap_dims() {
    ...
    if let Value::Tensor(p) = v { assert_eq!(p.shape, vec![3,2]); } else { panic!("expected tensor") }
}
...
fn cat_dim1() {
    ...
    let v = rt::call_builtin("cat", &[Value::Num(1.0), Value::Tensor(a), Value::Tensor(b)]).unwrap();
    if let Value::Tensor(t) = v { assert_eq!(t.shape, vec![4,2]); } else { panic!("expected tensor") }
}
...
fn cat_variadic_three_inputs() {
    ...
    let v = rt::call_builtin("cat", &[Value::Num(2.0), Value::Tensor(a), Value::Tensor(b), Value::Tensor(c)]).unwrap();
    if let Value::Tensor(t) = v { assert_eq!(t.shape, vec![2,3]); } else { panic!("expected tensor") }
}

#[test]
fn repmat_2d() {
    let a = runmat_builtins::Tensor::new(vec![1.0,2.0,3.0,4.0], vec![2,2]).unwrap();
    let v = rt::call_builtin("repmat", &[Value::Tensor(a), Value::Num(2.0), Value::Num(3.0)]).unwrap();
    if let Value::Tensor(t) = v { assert_eq!(t.shape, vec![4,6]); } else { panic!("expected tensor") }
}
...
fn repmat_nd_vector_form() {
    ...
    if let Value::Tensor(t) = v { assert_eq!(t.shape, vec![3, 8]); } else { panic!("expected tensor") }
}
...
fn meshgrid_basic() {
    ...
    if let Value::Tensor(t) = v { assert!(t.shape == vec![2,3] || t.shape == vec![3,2]); } else { panic!("expected tensor") }
}
...
fn diag_vector_to_matrix_and_back() {
    ...
    if let Value::Tensor(mt) = &m { assert_eq!(mt.shape, vec![3,3]); }
    let d = rt::call_builtin("diag", &[m]).unwrap();
    if let Value::Tensor(dt) = d { assert_eq!(dt.shape, vec![3,1]); assert!((dt.data[2]-3.0).abs()<1e-9); } else { panic!("expected tensor") }
}
...
fn triu_tril_shapes() {
    ...
    if let Value::Tensor(ut) = u { assert_eq!(ut.shape, vec![2,2]); } else { panic!("expected tensor") }
    if let Value::Tensor(lt) = l { assert_eq!(lt.shape, vec![2,2]); } else { panic!("expected tensor") }
}
