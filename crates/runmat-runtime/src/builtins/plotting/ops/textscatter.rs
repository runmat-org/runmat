...
use runmat_plot::plots::{Figure, NumericPlotData, Scatter3Plot, ScatterPlot, TextStyle};
...
mod tests {
    ...
    fn textscatter_coordinates_read_typed_integer_storage_exactly() {
        ...
        assert_eq!(
            x.storage(),
            &runmat_builtins::NumericStorage::I16(vec![1, 2])
        );
        assert_eq!(
            y.storage(),
            &runmat_builtins::NumericStorage::I16(vec![3, 4])
        );
        assert_eq!(
            z.unwrap().storage(),
            &runmat_builtins::NumericStorage::I16(vec![5, 6])
        );
    }
    ...
    fn textscatter_get_and_set_preserve_wide_integer_coordinate_storage() {
        ...
        let x = Tensor::from_numeric_storage(
            runmat_builtins::NumericStorage::U64(vec![wide, wide + 1]),
            vec![1, 2],
        )
        .unwrap();
        ...
        let replacement = Tensor::from_numeric_storage(
            runmat_builtins::NumericStorage::I64(vec![-(wide as i64), wide as i64]),
            vec![2, 1],
        )
        .unwrap();
        set_builtin(vec![
