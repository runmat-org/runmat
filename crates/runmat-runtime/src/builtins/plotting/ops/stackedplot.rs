...
use runmat_builtins::{
    ...
    CellArray, NumericDType, NumericStorage, ObjectInstance, StringArray, StructValue, Tensor,
    Value,
};
...
fn numeric_tensor_from_value(value: &Value, name: &str) -> BuiltinResult<Tensor> {
    match value {
        ...
        Value::Int(runmat_builtins::IntValue::I64(value)) => {
            Tensor::from_numeric_storage(NumericStorage::I64(vec![*value]), vec![1, 1])
                .map_err(stacked_err)
        }
        Value::Int(runmat_builtins::IntValue::U64(value)) => {
        ...
        Value::Tensor(tensor) => (0..tensor.len())
            .map(|offset| {
                ...
                let index = match scalar {
                    runmat_builtins::NumericScalar::F64(value) => {
                        if !value.is_finite()
                            || value < 1.0
                            || value.fract() != 0.0
...
mod tests {
    ...
    fn stackedplot_graphics_properties_preserve_wide_integer_source_storage() {
        ...
        let wide = 9_007_199_254_740_993_u64;
        let x = Tensor::new_integer(
            runmat_builtins::IntegerStorage::U64(vec![wide, wide + 2]),
            vec![1, 2],
        )
        .expect("XData");
        let y = Tensor::new_integer(
            runmat_builtins::IntegerStorage::I16(vec![-3, 4, 5, 6]),
            vec![2, 2],
        )
        .expect("YData");
        let handle =
        ...
        assert_eq!(
            x.integer_storage(),
            Some(&runmat_builtins::IntegerStorage::U64(vec![wide, wide + 2]))
        );
        assert_eq!(
            y.integer_storage(),
            Some(&runmat_builtins::IntegerStorage::I16(vec![-3, 4, 5, 6]))
        );
    ...
    fn stackedplot_automatic_integer_residency_gathers_exactly_in_compatibility_mode() {
        crate::builtins::common::test_support::with_test_provider(|provider| {
            ...
            let source = Tensor::new_integer(
                runmat_builtins::IntegerStorage::U64(vec![wide, wide + 1]),
                vec![1, 2],
            )
            .unwrap();
            ...
            assert_eq!(
                y.integer_storage(),
                Some(&runmat_builtins::IntegerStorage::U64(vec![wide, wide + 1]))
            );
