...
const UINT16_MAX_F64: f64 = u16::MAX as f64;
...
pub const GPU_SPEC: BuiltinGpuSpec = BuiltinGpuSpec {
    ...
    notes: "No provider-native uint16 hook yet; gpuArray inputs gather to host for saturating conversion and are then re-uploaded when possible.",
};
...
fn conversion_error(type_name: &str) -> RuntimeError {
    builtin_error(format!(
        "uint16: conversion to uint16 from {type_name} is not possible"
    ))
}
...
async fn uint16_builtin(value: Value, rest: Vec<Value>) -> BuiltinResult<Value> {
    if !rest.is_empty() {
        return Err(builtin_error("uint16: too many input arguments"));
    }
    match value {
        Value::Num(n) => Ok(Value::Int(IntValue::U16(cast_scalar_to_uint16(n)))),
        ...
        Value::LogicalArray(array) => {
            let tensor = tensor::logical_to_tensor(&array)
                .map_err(|e| builtin_error(format!("uint16: {e}")))?;
            Ok(uint16_value_from_tensor(uint16_tensor_to_host(tensor)))
        }
        Value::CharArray(chars) => uint16_from_char_array(chars),
...
fn uint16_from_char_array(chars: CharArray) -> BuiltinResult<Value> {
    let data: Vec<f64> = chars
        .data
        .iter()
        .map(|&ch| cast_scalar_to_uint16(ch as u32 as f64) as f64)
        .collect();
    let tensor = Tensor::new(data, vec![chars.rows, chars.cols])
        .map_err(|e| builtin_error(format!("uint16: {e}")))?;
    Ok(uint16_value_from_tensor(tensor))
}

async fn uint16_from_gpu(handle: GpuTensorHandle) -> BuiltinResult<Value> {
    let converted = uint16_tensor_to_host(gpu_helpers::gather_tensor_async(&handle).await?);
    ...
    if let Some(provider) = runmat_accelerate_api::provider_for_handle(&handle) {
        ...
        match provider.upload(&view) {
            ...
            Err(err) => trace!("uint16: provider upload failed after gather ({err})"),
        }
    }
...
fn uint16_tensor_to_host(mut tensor: Tensor) -> Tensor {
...
fn uint16_value_from_tensor(tensor: Tensor) -> Value {
    if tensor.data.len() == 1 {
        Value::Int(IntValue::U16(cast_scalar_to_uint16(tensor.data[0])))
    } else {
        Value::Tensor(tensor)
    }
}

fn cast_scalar_to_uint16(value: f64) -> u16 {
    if value.is_nan() {
        return 0;
    }
    ...
    value.round().clamp(0.0, UINT16_MAX_F64) as u16
}
...
pub(crate) mod tests {
    ...
    fn uint16_errors_on_string_input() {
        ...
        assert!(err.message().contains("uint16"));
        ...
        test_support::with_test_provider(|provider| {
            ...
            match result {
                Value::GpuTensor(handle) => {
                    ...
                    assert_eq!(gathered.data, vec![0.0, 4.0, u16::MAX as f64]);
                }
