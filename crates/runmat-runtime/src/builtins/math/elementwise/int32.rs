...
async fn int32_builtin(value: Value, rest: Vec<Value>) -> BuiltinResult<Value> {
    ...
    match value {
        ...
        Value::SparseTensor(sparse) => {
            cast_sparse_value(IntegerTarget::I32, sparse).map_err(|cause| match cause {
                CastError::Unsupported(type_name) => conversion_error(&type_name),
                CastError::Internal(detail) => {
                    int32_error_with_detail(&INT32_ERROR_INTERNAL, detail)
                }
            })
        }
        ...
        value @ (Value::Complex(_, _) | Value::ComplexTensor(_)) => {
            cast_complex_value(value, IntegerTarget::I32).map_err(|cause| match cause {
                CastError::Unsupported(type_name) => conversion_error(&type_name),
                CastError::Internal(detail) => {
                    int32_error_with_detail(&INT32_ERROR_INTERNAL, detail)
                }
...
async fn int32_from_gpu(handle: GpuTensorHandle) -> BuiltinResult<Value> {
    let provider = runmat_accelerate_api::provider().ok_or_else(|| {
        int32_error_with_detail(&INT32_ERROR_INTERNAL, "no acceleration provider registered")
    })?;
    provider
        .cast_to_integer(&handle, runmat_accelerate_api::IntegerElementType::I32)
        .await
        .map(Value::GpuTensor)
        .map_err(|e| int32_error_with_detail(&INT32_ERROR_INTERNAL, e))
}
