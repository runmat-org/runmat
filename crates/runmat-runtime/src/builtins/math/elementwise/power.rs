...
async fn value_to_real_tensor_for_gpu(value: &Value) -> BuiltinResult<Option<Tensor>> {
    match value {
        ...
        Value::Int(i) => Ok(Some(
            Tensor::new_integer(IntegerStorage::from_scalar(i.clone()), vec![1, 1])
                .map_err(|e| builtin_error(format!("power: {e}")))?,
        )),
...
fn upload_tensor(tensor: Tensor) -> BuiltinResult<Value> {
    ...
    let handle = gpu_helpers::upload_tensor(provider, &tensor)
        .map_err(|e| power_error_with_detail(&POWER_ERROR_INTERNAL, &e))?;
...
pub(crate) mod tests {
    ...
    use runmat_builtins::{
        ComplexTensor, IntValue, IntegerStorage, ResolveContext, SymbolicArray, SymbolicExpr,
        Tensor, Type,
    };
    ...
    fn power_mixed_real_complex_single_paths_preserve_single() {
        let complex_base =
            ComplexTensor::from_f32(vec![(1.0, 1.0), (2.0, 0.0)], vec![1, 2]).unwrap();
