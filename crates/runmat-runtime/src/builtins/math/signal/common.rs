...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum GpuSpectralRange {
    Onesided,
    Twosided,
    Centered,
}
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum GpuSpectralFrameMode {
    Sliding { hop: usize },
    FoldedColumns { input_rows: usize },
}

#[derive(Clone, Debug)]
pub(crate) struct GpuSpectralRequest<'a> {
    ...
    pub frame_mode: GpuSpectralFrameMode,
    pub range: GpuSpectralRange,
    pub denominator: f64,
}

#[derive(Clone, Debug)]
pub(crate) struct GpuSpectralResult {
...
pub(crate) async fn gpu_uniform_spectral_estimate(
    ...
    let window = provider
        ...
        .map_err(|e| {
            signal_error(
                "signal",
                None,
                format!("signal: GPU window upload failed: {e}"),
            )
        })?;
    ...
    let framed_len = request
        ...
        .ok_or_else(|| signal_error("signal", None, "signal: GPU spectral frame too large"))?;
    let frame_shader = spectral_frame_shader(&request, precision)?;
    ...
    let framed = provider
        ...
        .map_err(|e| {
            signal_error(
                "signal",
                None,
                format!("signal: GPU spectral frame failed: {e}"),
            )
        })?;
    provider.free(&window).ok();
    runmat_accelerate_api::set_handle_storage(&framed, GpuTensorStorage::ComplexInterleaved);

    let spectrum = provider.fft_dim(&framed, None, 0).await.map_err(|e| {
        signal_error(
            "signal",
            None,
            format!("signal: GPU spectral FFT failed: {e}"),
        )
    })?;
    provider.free(&framed).ok();
    let rows = selected_frequency_len(request.nfft, request.range);
    let selected = if matches!(request.range, GpuSpectralRange::Twosided) {
        spectrum
    } else {
        let selected_len = rows
            .checked_mul(request.frame_count)
            .and_then(|len| len.checked_mul(2))
            .ok_or_else(|| signal_error("signal", None, "signal: GPU spectral output too large"))?;
        let shader = spectral_select_shader(request.nfft, rows, request.range, precision)?;
        let handle = provider
            .fused_elementwise(
                &shader,
            ...
            .map_err(|e| {
                signal_error(
                    "signal",
                    None,
                    format!("signal: GPU spectral select failed: {e}"),
                )
            })?;
        provider.free(&spectrum).ok();
    ...
    let ps_len = rows
        .checked_mul(request.frame_count)
        .ok_or_else(|| signal_error("signal", None, "signal: GPU spectral power too large"))?;
    ...
    let ps = provider
        ...
        .map_err(|e| {
            signal_error(
                "signal",
                None,
                format!("signal: GPU spectral power failed: {e}"),
            )
        })?;
...
fn spectral_frame_shader(
    ...
    let (ty, zero, cast) = shader_numeric_fragments(precision);
    let input_complex = u32::from(request.input_complex);
    let (mode, hop, input_rows) = match request.frame_mode {
        GpuSpectralFrameMode::Sliding { hop } => (0u32, hop, 0usize),
        GpuSpectralFrameMode::FoldedColumns { input_rows } => (2u32, 0usize, input_rows),
    };
...
fn spectral_select_shader(
    nfft: usize,
    rows: usize,
    range: GpuSpectralRange,
    precision: ProviderPrecision,
) -> BuiltinResult<String> {
    let (ty, _, _) = shader_numeric_fragments(precision);
    let mode = match range {
        GpuSpectralRange::Onesided => 0u32,
        GpuSpectralRange::Centered => 1u32,
        GpuSpectralRange::Twosided => 2u32,
    };
    let centered_shift = centered_shift(nfft);
    Ok(format!(
        r#"
struct Tensor {{
    data: array<{ty}>,
...
fn spectral_power_shader(
    ...
    let (ty, _, cast) = shader_numeric_fragments(precision);
    let double_one_sided = u32::from(matches!(range, GpuSpectralRange::Onesided));
...
fn shader_numeric_fragments(
    precision: ProviderPrecision,
) -> (&'static str, &'static str, &'static str) {
    match precision {
        ProviderPrecision::F64 => ("f64", "0.0", "f64"),
        ProviderPrecision::F32 => ("f32", "0.0", "f32"),
    }
}
