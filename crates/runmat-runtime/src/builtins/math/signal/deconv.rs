...
#[runmat_macros::register_doc_text(name = "deconv")]
...
references:
  - title: "MATLAB deconv documentation"
    url: "https://www.mathworks.com/help/matlab/ref/deconv.html"
gpu_support:
  elementwise: false
  reduction: false
  precisions: ["f32", "f64"]
...
Coefficients follow MATLAB conventions: the first entry stores the highest-order term and the last
entry stores the constant term. Both real and complex inputs are supported.
...
- Leading zeros are ignored during division, matching MATLAB’s behaviour. The outputs include just
  enough coefficients to represent the resulting polynomials (with at least one element for the zero
  polynomial).
...
1. Gathers the device operands into host memory.
2. Performs the polynomial long division on the CPU.
3. Re-uploads real-valued results to the provider so downstream fused kernels can remain
   device-resident (complex outputs stay on the host until providers add complex buffer uploads).

This fallback keeps behaviour identical to MATLAB. Gather the outputs explicitly only when you need
them on the host.
...
They do not change the represented polynomial. Dropping them keeps results compact and mirrors
MATLAB’s behaviour. The zero polynomial is returned as a single zero.
