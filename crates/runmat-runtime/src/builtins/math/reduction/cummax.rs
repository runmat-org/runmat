...
pub const INTEGER_CAPABILITIES: [BuiltinIntegerCapabilityDescriptor; 1] =
    [BuiltinIntegerCapabilityDescriptor {
        ...
        notes: "Cumulative maximum preserves integer class and exact shape in forward or reverse direction; current R2026a exposes one public value output and rejects GPU nanflag, so internal/provider indices remain private and RunMat GPU nanflag support is mode-gated.",
    }];
...
pub(crate) mod tests {
    ...
    fn cummax_complex_single_preserves_native_storage() {
        ...
        assert_eq!(
            ...
            &runmat_value::ComplexStorage::F32(vec![(3.0, 0.0), (3.0, 0.0), (3.0, 0.0)])
        );
    }
    ...
    fn cummax_integer_storage_and_reverse_indices_remain_exact() {
        let input = Tensor::new_integer(
            runmat_value::IntegerStorage::I64(vec![i64::MIN, -4, -5, -3]),
            vec![2, 2],
        )
        .unwrap();
        ...
        assert_eq!(
            ...
            Value::Tensor(
                Tensor::new_integer(
                    runmat_value::IntegerStorage::I64(vec![-4, -4, -3, -3]),
                    vec![2, 2]
                )
                .unwrap()
            )
