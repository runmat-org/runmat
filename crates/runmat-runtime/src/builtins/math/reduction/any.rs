...
## See Also
[sum](./sum), [prod](./prod), [mean](./mean), [gpuArray](./gpuarray), [gather](./gather)
...
fn any_error(message: impl Into<String>) -> RuntimeControlFlow {
...
fn any_host(value: Value, spec: ReductionSpec, nan_mode: ReductionNaN) -> BuiltinResult<Value> {
...
impl TruthTensor {
    ...
    fn reduce_dim(&self, dim: usize, nan_mode: ReductionNaN) -> BuiltinResult<Self> {
        if dim == 0 {
            return Err(any_error("any: dimension must be >= 1"));
        }
