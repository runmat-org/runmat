    ...
    match value {
        ...
        Value::ComplexTensor(tensor) => {
            ...
            let bits = match tensor.complex_storage() {
                runmat_builtins::ComplexStorage::Integer(storage) => (0..storage.len())
                    .map(|index| {
                        u8::from(
                            storage
                                .is_nonzero_at(index)
                    ...
                    .collect(),
                runmat_builtins::ComplexStorage::F64(values) => values
                    .iter()
                    .map(|&(real, imag)| u8::from(is_nonzero_complex(real, imag)))
                    .collect(),
                runmat_builtins::ComplexStorage::F32(values) => values
                    .iter()
                    .map(|&(real, imag)| {
