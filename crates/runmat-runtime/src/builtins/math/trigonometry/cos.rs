    ...
    match args.len() {
        ...
        2 => {
            if matches!(keyword_of(&args[0]).as_deref(), Some("like")) {
                ...
                Err(runtime_error_for("cos: unsupported option; only 'like' is accepted"))
            }
        }
...
fn convert_to_gpu(value: Value) -> BuiltinResult<Value> {
    let provider = runmat_accelerate_api::provider().ok_or_else(|| {
        runtime_error_for("cos: GPU output requested via 'like' but no acceleration provider is active")
    })?;
    match value {
...
fn convert_to_host_like(value: Value) -> BuiltinResult<Value> {
    match value {
        Value::GpuTensor(handle) => {
            ...
            gpu_helpers::gather_value(&proxy).map_err(|e| runtime_error_for(format!("cos: {e}")))
        }
        other => Ok(other),
    }
}
