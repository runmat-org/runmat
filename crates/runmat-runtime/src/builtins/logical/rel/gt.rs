...
const GT_ERROR_COMPLEX_UNSUPPORTED: BuiltinErrorDescriptor = BuiltinErrorDescriptor {
    code: "RM.GT.COMPLEX_UNSUPPORTED",
    identifier: Some("RunMat:gt:ComplexNotSupported"),
    when: "At least one operand is complex.",
    message: "gt: complex numbers are not supported",
};

const GT_ERRORS: [BuiltinErrorDescriptor; 3] = [
    GT_ERROR_INVALID_INPUT,
    GT_ERROR_SIZE_MISMATCH,
    GT_ERROR_COMPLEX_UNSUPPORTED,
];
...
fn gt_error(error: &'static BuiltinErrorDescriptor) -> RuntimeError {
    gt_error_with_message(error.message, error)
}

fn gt_error_with_message(
...
fn logical_result(data: Vec<u8>, shape: Vec<usize>) -> crate::BuiltinResult<Value> {
    if tensor::element_count(&shape) <= 1 && data.len() == 1 {
        ...
        LogicalArray::new(data, shape)
            ...
            .map_err(|e| gt_error_with_message(format!("gt: {e}"), &GT_ERROR_INVALID_INPUT))
    }
}
    ...
    async fn from_value(value: Value) -> crate::BuiltinResult<Self> {
        match value {
            ...
            Value::Complex(_, _) | Value::ComplexTensor(_) => {
                Err(gt_error(&GT_ERROR_COMPLEX_UNSUPPORTED))
            }
            unsupported => Err(gt_error_with_message(
...
fn numeric_gt(
    ...
    let shape = broadcast_shapes(BUILTIN_NAME, &lhs.shape, &rhs.shape)
        .map_err(|err| gt_error_with_message(err, &GT_ERROR_SIZE_MISMATCH))?;
...
pub(crate) mod tests {
    ...
    fn gt_complex_error() {
        ...
        assert_eq!(err.identifier(), GT_ERROR_COMPLEX_UNSUPPORTED.identifier);
    }
