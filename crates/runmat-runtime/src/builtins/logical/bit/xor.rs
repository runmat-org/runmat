...
const XOR_OUTPUT: [BuiltinParamDescriptor; 1] = [BuiltinParamDescriptor {
    ...
    default: None,
    description: "Logical element-wise exclusive disjunction result.",
}];

const XOR_INPUTS: [BuiltinParamDescriptor; 2] = [
...
const XOR_SIGNATURES: [BuiltinSignatureDescriptor; 1] = [BuiltinSignatureDescriptor {
    label: "tf = xor(A, B)",
    inputs: &XOR_INPUTS,
    outputs: &XOR_OUTPUT,
}];

const XOR_ERROR_INVALID_INPUT: BuiltinErrorDescriptor = BuiltinErrorDescriptor {
    code: "RM.XOR.INVALID_INPUT",
    identifier: Some("RunMat:xor:InvalidInput"),
    when: "An input is not logical, numeric, complex, character, or gpuArray with gatherable numeric data.",
    message: "xor: unsupported input type; expected logical, numeric, complex, or character data",
};

const XOR_ERROR_SIZE_MISMATCH: BuiltinErrorDescriptor = BuiltinErrorDescriptor {
    code: "RM.XOR.SIZE_MISMATCH",
    identifier: Some("RunMat:xor:SizeMismatch"),
    when: "Input shapes are not broadcast-compatible.",
    message: "xor: array sizes are not compatible for broadcasting",
};

const XOR_ERRORS: [BuiltinErrorDescriptor; 2] = [XOR_ERROR_INVALID_INPUT, XOR_ERROR_SIZE_MISMATCH];

pub const XOR_DESCRIPTOR: BuiltinDescriptor = BuiltinDescriptor {
    signatures: &XOR_SIGNATURES,
...
async fn xor_host(lhs: Value, rhs: Value) -> BuiltinResult<Value> {
    ...
    let shape = broadcast_shapes(BUILTIN_NAME, &left.shape, &right.shape)
        .map_err(|err| builtin_error_with_message(err, &XOR_ERROR_SIZE_MISMATCH))?;
...
fn logical_value(fn_name: &str, data: Vec<u8>, shape: Vec<usize>) -> BuiltinResult<Value> {
    if data.len() == 1 && tensor::element_count(&shape) == 1 {
        ...
        LogicalArray::new(data, shape)
            ...
            .map_err(|e| {
                builtin_error_with_message(format!("{fn_name}: {e}"), &XOR_ERROR_INVALID_INPUT)
            })
    }
}
    ...
    match value {
        ...
        Value::GpuTensor(handle) => {
            let tensor = gpu_helpers::gather_tensor_async(&handle)
                .await
                .map_err(|err| {
                    builtin_error_with_message(format!("{name}: {err}"), &XOR_ERROR_INVALID_INPUT)
                })?;
