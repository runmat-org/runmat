...
#[runmat_macros::register_doc_text(name = "union")]
...
- `union(A, B)` flattens the inputs column-major, removes duplicates, and returns the sorted union
  of their values.
- `[C, IA, IB] = union(A, B)` also returns index vectors where `IA` points to the elements in `A`
  that contribute to `C` and `IB` refers to the elements in `B` that only appear in `B`.
- `union(A, B, 'stable')` preserves the first-seen order: unique values from `A` appear first,
  followed by new values appearing in `B`.
...
- `union` is registered as a residency sink. When tensors live on a GPU and the active provider
  does not expose a dedicated `union` hook, the runtime gathers them to host memory and executes
  the CPU implementation to guarantee MATLAB-compatible output.
...
No. When a provider does not implement a GPU kernel, RunMat automatically gathers inputs to the host,
performs the union there, and returns host-resident outputs that match MATLAB semantics exactly.

### What if the inputs have different shapes?
Element-wise unions accept any shapes; both inputs are flattened column-major. Row-wise unions require
matching column counts and two-dimensional inputs.

### Can I mix data types?
No. Inputs must belong to the same class (numeric/logical, complex, char, or string). Mixing classes
...
Yes. Character arrays treat every character, including trailing spaces, as significant—matching MATLAB's
behaviour.
...
## See Also
[unique](./unique), [sort](./sort), [sortrows](./sortrows)

## Source & Feedback
- Source code: [`crates/runmat-runtime/src/builtins/array/sorting_sets/union.rs`](https://github.com/runmat-org/runmat/blob/main/crates/runmat-runtime/src/builtins/array/sorting_sets/union.rs)
