...
`intersect(A, B)` computes the set intersection of `A` and `B`. By default it returns the
distinct common values sorted ascending, along with optional index outputs that map each
result back to its originating position in the inputs.

## How does the `intersect` function behave in MATLAB / RunMat?
- `intersect(A, B)` flattens both inputs column-major, removes duplicates, and returns the
  sorted intersection.
- `[C, IA, IB] = intersect(A, B)` also returns index vectors so that `C = A(IA)` and
  `C = B(IB)`.
- `intersect(A, B, 'stable')` preserves the first appearance order from `A`.
- `intersect(A, B, 'rows')` treats each row as an element. Inputs must share the same number
  of columns.
- Character arrays, string arrays, logical arrays, complex values, and numerical types are
  fully supported. Mixed classes raise a descriptive error.
- Legacy flags such as `'legacy'` or `'R2012a'` are not supported in RunMat.

## `intersect` Function GPU Execution Behavior
`intersect` registers as a residency sink. When the active acceleration provider offers a
dedicated `intersect` hook, the set logic can execute directly on the device and return
GPU-resident results. Until such a hook lands, RunMat automatically gathers GPU tensors to
host memory, runs the CPU implementation, and materializes host-side outputs so behavior
matches MathWorks MATLAB exactly.
...
No. MATLAB and RunMat return each common value at most once. Use other logic (such as
logical indexing) if you need multiset behavior.

### What is the default ordering?
`intersect` sorts results ascending by default. Specify `'stable'` to preserve the input
order from `A`.

### Can I intersect rows that contain strings?
Yes. String arrays support both element and row intersections. When using `'rows'`, the
...
No. RunMat only implements the modern MATLAB semantics. Passing `'legacy'` or `'R2012a'`
raises an error.

## GPU residency in RunMat (Do I need `gpuArray`?)
You usually do **not** need to call `gpuArray` manually. RunMat's planner keeps tensors
resident on the GPU when profitable. If a provider lacks an `intersect` kernel the runtime
gathers automatically, so explicit residency management is rarely needed. Explicit calls to
`gpuArray` remain available for compatibility with MathWorks MATLAB workflows.

## See Also
[union](./union), [unique](./unique), [sort](./sort), [sortrows](./sortrows)

## Source & Feedback
- Source code: [`crates/runmat-runtime/src/builtins/array/sorting_sets/intersect.rs`](https://github.com/runmat-org/runmat/blob/main/crates/runmat-runtime/src/builtins/array/sorting_sets/intersect.rs)
...
fn intersect_error(message: impl Into<String>) -> RuntimeError {
    build_runtime_error(message).with_builtin("intersect").build()
}
...
fn parse_options(rest: &[Value]) -> crate::BuiltinResult<IntersectOptions> {
    ...
    for arg in rest {
        ...
        match lowered.as_str() {
            ...
            "sorted" => {
                if let Some(prev) = seen_order {
                    if prev != IntersectOrder::Sorted {
                        return Err(intersect_error("intersect: cannot combine 'sorted' with 'stable'"));
                    }
                }
            ...
            "legacy" | "r2012a" => {
                return Err(intersect_error("intersect: the 'legacy' behaviour is not supported"));
            }
            other => return Err(intersect_error(format!("intersect: unrecognised option '{other}'")).into()),
        }
    }
...
fn intersect_host(
    ...
    match (a, b) {
        ...
        (Value::StringArray(astring), Value::String(b)) => {
            let bstring =
                StringArray::new(vec![b], vec![1, 1]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
            intersect_string(astring, bstring, opts)
        }
        (Value::String(a), Value::StringArray(bstring)) => {
            let astring =
                StringArray::new(vec![a], vec![1, 1]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
            intersect_string(astring, bstring, opts)
        }
...
fn intersect_numeric_rows(
    ...
    if a.shape.len() != 2 || b.shape.len() != 2 {
        return Err(intersect_error("intersect: 'rows' option requires 2-D numeric matrices"));
    }
    if a.shape[1] != b.shape[1] {
        return Err(
            intersect_error("intersect: inputs must have the same number of columns when using 'rows'"),
        );
    }
...
fn intersect_complex_rows(
    ...
    if a.shape.len() != 2 || b.shape.len() != 2 {
        return Err(intersect_error("intersect: 'rows' option requires 2-D complex matrices"));
    }
    if a.shape[1] != b.shape[1] {
        return Err(
            intersect_error("intersect: inputs must have the same number of columns when using 'rows'"),
...
fn intersect_string_rows(
    ...
    if a.shape.len() != 2 || b.shape.len() != 2 {
        return Err(intersect_error("intersect: 'rows' option requires 2-D string arrays"));
    }
    if a.shape[1] != b.shape[1] {
        return Err(
            intersect_error("intersect: inputs must have the same number of columns when using 'rows'"),
...
fn assemble_numeric_intersect(
    ...
    let value_tensor =
        Tensor::new(values, vec![order.len(), 1]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
    let ia_tensor = Tensor::new(ia, vec![order.len(), 1]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
    let ib_tensor = Tensor::new(ib, vec![order.len(), 1]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
...
fn assemble_numeric_row_intersect(
    ...
    let value_tensor =
        Tensor::new(values, vec![rows_out, cols]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
    let ia_tensor = Tensor::new(ia, vec![rows_out, 1]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
    let ib_tensor = Tensor::new(ib, vec![rows_out, 1]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
...
fn assemble_complex_intersect(
    ...
    let value_tensor =
        ComplexTensor::new(values, vec![order.len(), 1]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
    let ia_tensor = Tensor::new(ia, vec![order.len(), 1]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
    let ib_tensor = Tensor::new(ib, vec![order.len(), 1]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
...
fn assemble_complex_row_intersect(
    ...
    let value_tensor =
        ComplexTensor::new(values, vec![rows_out, cols]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
    let ia_tensor = Tensor::new(ia, vec![rows_out, 1]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
    let ib_tensor = Tensor::new(ib, vec![rows_out, 1]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
...
fn assemble_char_intersect(
    ...
    let value_array =
        CharArray::new(values, order.len(), 1).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
    let ia_tensor = Tensor::new(ia, vec![order.len(), 1]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
    let ib_tensor = Tensor::new(ib, vec![order.len(), 1]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
...
fn assemble_char_row_intersect(
    ...
    let value_array =
        CharArray::new(values, rows_out, cols).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
    let ia_tensor = Tensor::new(ia, vec![rows_out, 1]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
    let ib_tensor = Tensor::new(ib, vec![rows_out, 1]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
...
fn assemble_string_intersect(
    ...
    let value_array =
        StringArray::new(values, vec![order.len(), 1]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
    let ia_tensor = Tensor::new(ia, vec![order.len(), 1]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
    let ib_tensor = Tensor::new(ib, vec![order.len(), 1]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
...
fn assemble_string_row_intersect(
    ...
    let value_array =
        StringArray::new(values, vec![rows_out, cols]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
    let ia_tensor = Tensor::new(ia, vec![rows_out, 1]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
    let ib_tensor = Tensor::new(ib, vec![rows_out, 1]).map_err(|e| intersect_error(format!("intersect: {e}")).into())?;
...
fn scalar_complex_tensor(re: f64, im: f64) -> crate::BuiltinResult<ComplexTensor> {
    ComplexTensor::new(vec![(re, im)], vec![1, 1])
        .map_err(|e| intersect_error(format!("intersect: {e}")).into())
}
...
fn tensor_to_complex_owned(name: &str, tensor: Tensor) -> crate::BuiltinResult<ComplexTensor> {
    ...
    ComplexTensor::new(complex, shape)
        .map_err(|e| intersect_error(format!("{name}: {e}")).into())
}
