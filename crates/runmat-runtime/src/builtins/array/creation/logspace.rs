...
fn merge_source_owners(
    ...
    match (lhs, rhs) {
        (Some(lhs), Some(rhs)) if !std::ptr::eq(lhs.owner, rhs.owner) => Err(build_runtime_error(
    ...
    match value {
        ...
        Value::GpuTensor(handle) => {
            ...
            let owner = gpu_helpers::exact_provider_for_handle(&handle).ok_or_else(|| {
                build_runtime_error("logspace: no exact owner for GPU scalar input")
                    .with_builtin("logspace")
                    .with_identifier("RunMat:gpu:ProviderOwnershipMismatch")
                    .with_gpu_gather_retry(crate::GpuGatherRetry::Never)
                    .build()
    ...
    if complex {
        ...
        if let Some(source) = source {
            ...
            if source.explicit {
                ...
                .with_builtin("logspace")
                .with_identifier("RunMat:logspace:GpuUploadFailed")
                .with_gpu_gather_retry(crate::GpuGatherRetry::Never)
                .build());
            }
        }
