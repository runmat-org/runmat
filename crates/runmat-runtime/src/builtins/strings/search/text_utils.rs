...
pub(crate) fn broadcast_shapes(
    ...
    for dim in 0..len {
        let da = if dim < a.len() { a[dim] } else { 1 };
        let db = if dim < b.len() { b[dim] } else { 1 };
        if da == db {
            shape.push(da);
        } else if da == 1 && db == 0 {
            shape.push(0);
...
pub(crate) fn broadcast_index(
    ...
    for dim in 0..out_shape.len() {
        let size_out = out_shape[dim];
        let coord = if size_out == 0 { 0 } else { index % size_out };
        if size_out != 0 {
            index /= size_out;
        }
        let size_in = if dim < in_shape.len() {
            in_shape[dim]
        } else {
            1
        };
        let mapped = if size_in == 1 || size_out == 0 {
            0
        } else {
            coord
        };
