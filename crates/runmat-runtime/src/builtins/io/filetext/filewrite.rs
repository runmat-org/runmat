...
fn gather_values(values: &[Value]) -> Result<Vec<Value>, String> {
    ...
    for value in values {
        out.push(gather_if_needed(value).map_err(|e| format!("filewrite: {e}"))?);
    }
    Ok(out)
}

fn parse_options(args: &[Value]) -> Result<FilewriteOptions, String> {
    if args.is_empty() {
...
fn is_keyword(value: &Value) -> Result<bool, String> {
    let text = keyword_name(value)?;
    Ok(text.eq_ignore_ascii_case("encoding") || text.eq_ignore_ascii_case("writemode"))
}

fn keyword_name(value: &Value) -> Result<String, String> {
    match value {
        Value::String(s) => Ok(s.clone()),
        Value::CharArray(ca) if ca.rows == 1 => Ok(ca.data.iter().collect()),
        Value::CharArray(_) => {
            Err("filewrite: keyword names must be 1-by-N character vectors".to_string())
        }
        Value::StringArray(sa) if sa.data.len() == 1 => Ok(sa.data[0].clone()),
        Value::StringArray(_) => {
            Err("filewrite: keyword inputs must be scalar string arrays".to_string())
        }
        other => Err(format!(
...
fn encoding_from_value(value: &Value) -> Result<FileEncoding, String> {
    ...
    match FileEncoding::from_label(&label) {
        ...
        None if label.trim().is_empty() => {
            Err("filewrite: encoding name must not be empty".to_string())
        }
        None => Err(format!("filewrite: unsupported encoding '{}'", label)),
    }
}

fn write_mode_from_value(value: &Value) -> Result<WriteMode, String> {
    let label = keyword_name(value)?;
    match WriteMode::from_label(&label) {
        Some(mode) => Ok(mode),
        None if label.trim().is_empty() => {
            Err("filewrite: write mode must not be empty".to_string())
        }
        None => Err(format!(
...
fn resolve_path(value: &Value) -> Result<PathBuf, String> {
    match value {
        ...
        Value::CharArray(_) => {
            Err("filewrite: filename must be a 1-by-N character vector".to_string())
        }
        Value::StringArray(sa) if sa.data.len() == 1 => normalize_path(&sa.data[0]),
        Value::StringArray(_) => {
            Err("filewrite: string array filename inputs must be scalar".to_string())
        }
        other => Err(format!(
...
fn normalize_path(raw: &str) -> Result<PathBuf, String> {
    if raw.is_empty() {
        return Err("filewrite: filename must not be empty".to_string());
    }
...
fn prepare_payload(data: &Value, encoding: FileEncoding) -> Result<Vec<u8>, String> {
...
fn extract_payload(data: &Value) -> Result<Payload, String> {
    match data {
        ...
        Value::Num(n) => {
            Ok(Payload::Bytes(vec![float_to_byte(*n).map_err(|msg| {
                format!("filewrite: numeric value {msg}")
            })?]))
        }
        Value::Int(i) => {
            Ok(Payload::Bytes(vec![int_value_to_byte(i).map_err(
                |msg| format!("filewrite: integer value {msg}"),
            )?]))
        }
        ...
        Value::Cell(_) => Err("filewrite: cell arrays are not supported inputs".into()),
        Value::GpuTensor(_) => {
            Err("filewrite: internal error: GPU tensor should be gathered".into())
        }
        Value::Complex(_, _) | Value::ComplexTensor(_) => {
            Err("filewrite: complex data must be converted to text before writing".into())
        }
        other => Err(format!(
...
fn tensor_to_bytes(tensor: &Tensor) -> Result<Vec<u8>, String> {
    ...
    for (idx, value) in tensor.data.iter().enumerate() {
        match float_to_byte(*value) {
            ...
            Err(msg) => {
                return Err(format!("filewrite: numeric element {} {msg}", idx));
            }
        }
    }
    Ok(out)
...
fn float_to_byte(value: f64) -> Result<u8, String> {
    ...
    if !(0..=255).contains(&int) {
        return Err(format!("value {value} is not in the range 0..255"));
    }
    Ok(int as u8)
}

fn int_value_to_byte(value: &IntValue) -> Result<u8, String> {
    match value {
...
fn signed_to_byte(value: i64) -> Result<u8, String> {
...
fn unsigned_to_byte(value: u64) -> Result<u8, String> {
    if value > 255 {
        return Err(format!("value {value} is not in the range 0..255"));
    }
    Ok(value as u8)
...
fn encode_text(chars: Vec<char>, encoding: FileEncoding) -> Result<Vec<u8>, String> {
    match encoding {
...
fn encode_ascii_chars(chars: &[char]) -> Result<Vec<u8>, String> {
...
fn encode_latin_chars(chars: &[char], encoding: FileEncoding) -> Result<Vec<u8>, String> {
    let mut out = Vec::with_capacity(chars.len());
    for &ch in chars {
        if ch as u32 > 0xFF {
            return Err(format!(
...
fn encode_bytes(bytes: Vec<u8>, encoding: FileEncoding) -> Result<Vec<u8>, String> {
...
fn write_bytes(path: &Path, payload: &[u8], mode: WriteMode) -> Result<usize, String> {
    ...
    let mut file = options
        .open(path)
        .map_err(|err| format!("filewrite: unable to open '{}': {}", path.display(), err))?;
    ...
    file.flush()
        .map_err(|err| format!("filewrite: unable to flush '{}': {}", path.display(), err))?;
...
pub(crate) mod tests {
    ...
    fn filewrite_errors_on_invalid_ascii() {
        ...
        .expect_err("should error for non-ascii");
        assert!(
    ...
    fn filewrite_errors_on_numeric_out_of_range() {
        ...
        .expect_err("should reject out of range");
        assert!(
    ...
    fn filewrite_rejects_ascii_numeric_bytes_above_range() {
        ...
        .expect_err("should reject ascii byte");
        assert!(
            err.contains("cannot be encoded as ASCII"),
            "unexpected error message: {err}"
        );
    ...
    fn filewrite_rejects_unknown_option() {
        ...
        .expect_err("should reject unknown option");
        assert!(
    ...
    fn filewrite_rejects_duplicate_encoding() {
        ...
        .expect_err("should reject duplicate encoding");
        assert!(
    ...
    fn filewrite_rejects_duplicate_writemode() {
        ...
        .expect_err("should reject duplicate writemode");
        assert!(
    ...
    fn filewrite_rejects_invalid_writemode_value() {
        ...
        .expect_err("should reject invalid writemode");
        assert!(
    ...
    fn filewrite_rejects_invalid_encoding_value() {
        ...
        .expect_err("should reject invalid encoding");
        assert!(
