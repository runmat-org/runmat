...
pub(crate) mod tests {
    ...
    fn rmfield_preserves_all_integer_classes_in_retained_fields() {
        let storages = [
            runmat_value::IntegerStorage::I8(vec![i8::MIN]),
            runmat_value::IntegerStorage::I16(vec![i16::MIN]),
            runmat_value::IntegerStorage::I32(vec![i32::MIN]),
            runmat_value::IntegerStorage::I64(vec![i64::MIN]),
            runmat_value::IntegerStorage::U8(vec![u8::MAX]),
            runmat_value::IntegerStorage::U16(vec![u16::MAX]),
            runmat_value::IntegerStorage::U32(vec![u32::MAX]),
            runmat_value::IntegerStorage::U64(vec![u64::MAX]),
        ];
        for storage in storages {
            let tensor = runmat_value::Tensor::new_integer(storage.clone(), vec![1, 1])
