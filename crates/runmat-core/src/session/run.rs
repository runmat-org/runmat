...
impl RunMatSession {
    ...
    /// unit into the interactive workspace.
    pub async fn invoke_executable(
        ...
        if control.is_cancelled() {
            return Err(RunError::Runtime(
                build_runtime_error("executable invocation was cancelled")
        ...
        if control.deadline_elapsed() {
            return Err(RunError::Runtime(
                build_runtime_error("executable invocation deadline elapsed")
                    .with_identifier("RunMat:ExecutionDeadline")
                    .build(),
            ));
        }
        ...
        let result = match invocation.target {
            crate::ProcedureTarget::Entrypoint => {
                if !invocation.arguments.is_empty() {
                    return Err(RunError::Runtime(
                        ...
                        .with_identifier("RunMat:ExecutableArity")
                        .build(),
                    ));
                }
                ...
                .await
                {
                    Ok(runmat_vm::InterpreterOutcome::Completed(values)) => Ok(values
                        .into_iter()
                        .last()
            ...
            crate::ProcedureTarget::Function(name) => {
                let function = unit.functions().resolve_name(&name).ok_or_else(|| {
                    build_runtime_error(format!(
                        "executable unit does not contain semantic procedure '{name}'"
                    ))
                    .with_identifier("RunMat:UndefinedExecutableProcedure")
                    .build()
                })?;
                runmat_vm::invoke_semantic_function_value(
                    function.0,
