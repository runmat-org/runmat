    ...
    match instr {
        ...
        | runmat_vm::Instr::StoreSliceExpr {
            ...
            for expr in range_start_exprs.iter_mut().flatten() {
                remap_external_end_expr(expr, remap);
            }
...
fn remap_external_end_expr(
    ...
    match expr {
        runmat_vm::EndExpr::ResolvedCall { identity, args, .. } => {
            ...
            for arg in args {
                remap_external_end_expr(arg, remap);
            }
        }
        ...
        | runmat_vm::EndExpr::Pow(lhs, rhs) => {
            remap_external_end_expr(lhs, remap);
            remap_external_end_expr(rhs, remap);
        }
        ...
        | runmat_vm::EndExpr::Fix(inner) => remap_external_end_expr(inner, remap),
...
impl RunMatSession {
    ...
    async fn execute_internal(
        ...
        self.preload_project_symbol_functions_for_execution(&lowering, &mut bytecode)?;
        #[cfg(target_arch = "wasm32")]
        let _ = &analysis;
        if self.verbose {
            debug!("AST: {ast:?}");
    ...
    #[cfg(not(target_arch = "wasm32"))]
    fn preload_project_symbol_functions_for_execution(
        &self,
        lowering: &LoweringResult,
        bytecode: &mut runmat_vm::Bytecode,
    ) -> std::result::Result<(), RunError> {
        let source_name = self.current_source_name().to_string();
        let mut symbol_sources = discover_project_symbol_sources(Some(source_name.as_str()));
        let required_symbols = self.required_external_symbols(lowering);
        ...
        for symbol in required_symbols {
            ...
            let candidates = match runmat_runtime::builtins::common::path_search::file_candidates(
                &symbol,
                &[".m"],
        ...
        for (path, symbols) in symbols_by_path {
            ...
            let source_text = match std::fs::read_to_string(&path) {
                ...
                Err(err) => {
                    warn!(
                        "Skipping project symbol preload for {}: failed to read source ({err})",
                        path.display()
                    );
                    continue;
                }
            ...
            let ast = match parse_with_options(&source_text, ParserOptions::new(self.compat_mode)) {
                ...
                Err(err) => {
                    warn!(
                        "Skipping project symbol preload for {}: parse failed ({err})",
                        path.display()
                    );
                    continue;
                }
            ...
            let lowering = match runmat_hir::lower(
                &ast,
                &LoweringContext::new(&HashMap::new())
                    .with_bound_functions(&self.function_registry.names)
                    .with_known_project_symbols(&known_project_symbols)
                ...
                Err(err) => {
                    warn!(
                        "Skipping project symbol preload for {}: lowering failed ({err})",
                        path.display()
                    );
                    continue;
                }
            ...
            let mir = match runmat_mir::lowering::lower_assembly(&lowering.assembly) {
                ...
                Err(err) => {
                    warn!(
                        "Skipping project symbol preload for {}: MIR lowering failed ({err})",
                        path.display()
                    );
                    continue;
                }
            ...
            let mut compiled_functions =
                match runmat_vm::compile_semantic_function_registry(&lowering.assembly, &mir) {
                    ...
                    Err(err) => {
                        warn!(
                            "Skipping project symbol preload for {}: VM compile failed ({err})",
                            path.display()
                        );
                        continue;
                    }
            ...
            for function in &mut remapped_functions {
                for instr in &mut function.instructions {
                    remap_external_function_instr(instr, &remap);
                }
            }
