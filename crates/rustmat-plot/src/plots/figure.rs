...
use crate::core::{RenderData, BoundingBox};
use crate::plots::{LinePlot, ScatterPlot, BarChart, Histogram};
use glam::Vec4;
...
impl Figure {
    ...
    pub fn statistics(&self) -> FigureStatistics {
        let plot_counts = self.plots.iter().fold(
            HashMap::new(),
            |mut acc, plot| {
        ...
        let total_memory: usize = self.plots.iter()
            .map(|plot| plot.estimated_memory_usage())
            .sum();
            
        let visible_count = self.plots.iter()
            .filter(|plot| plot.is_visible())
            .count();
...
mod tests {
    ...
    fn test_multiple_line_plots() {
        ...
        // Add first line plot
        let line1 = LinePlot::new(vec![0.0, 1.0, 2.0], vec![0.0, 1.0, 4.0]).unwrap()
            .with_label("Quadratic");
        let index1 = figure.add_line_plot(line1);
        
        // Add second line plot
        let line2 = LinePlot::new(vec![0.0, 1.0, 2.0], vec![0.0, 1.0, 2.0]).unwrap()
    ...
    fn test_mixed_plot_types() {
        ...
        let line = LinePlot::new(vec![0.0, 1.0, 2.0], vec![1.0, 2.0, 3.0]).unwrap()
            .with_label("Line");
        figure.add_line_plot(line);
        
        let scatter = ScatterPlot::new(vec![0.5, 1.5, 2.5], vec![1.5, 2.5, 3.5]).unwrap()
            .with_label("Scatter");
    ...
    fn test_matlab_compat_multiple_lines() {
        ...
        let data_sets = vec![
            (vec![0.0, 1.0, 2.0], vec![0.0, 1.0, 4.0], Some("Quadratic".to_string())),
            (vec![0.0, 1.0, 2.0], vec![0.0, 1.0, 2.0], Some("Linear".to_string())),
            (vec![0.0, 1.0, 2.0], vec![1.0, 1.0, 1.0], Some("Constant".to_string())),
        ];
