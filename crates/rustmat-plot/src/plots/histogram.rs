...
use crate::core::{Vertex, PipelineType, RenderData, Material, DrawCall, BoundingBox};
use glam::{Vec3, Vec4};
    ...
    fn create_histogram_geometry(&self) -> (Vec<Vertex>, Vec<u32>) {
        ...
        for (_i, (&height, edges)) in heights.iter().zip(self.bin_edges.windows(2)).enumerate() {
            ...
            indices.extend_from_slice(&[
                base_vertex_index, base_vertex_index + 1, base_vertex_index + 2, // Bottom-right triangle
                base_vertex_index, base_vertex_index + 2, base_vertex_index + 3, // Top-left triangle
            ]);
        }
...
mod tests {
    ...
    fn test_histogram_validation() {
        ...
        assert!(Histogram::with_bin_edges(vec![1.0, 2.0], vec![2.0, 1.0]).is_err()); // Not sorted
    }
    ...
    fn test_histogram_normalization() {
        ...
        let total_area: f64 = heights.iter()
