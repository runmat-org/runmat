...
impl ScatterPlot {
    ...
    pub fn generate_vertices(&mut self) -> &Vec<Vertex> {
        if self.dirty || self.vertices.is_none() {
            self.vertices = Some(vertex_utils::create_scatter_plot(&self.x_data, &self.y_data, self.color));
            self.dirty = false;
        }
...
pub mod matlab_compat {
    ...
    pub fn scatter_with_style(x: Vec<f64>, y: Vec<f64>, size: f32, color: &str) -> Result<ScatterPlot, String> {
...
mod tests {
    ...
    fn test_scatter_plot_styling() {
        ...
        let plot = ScatterPlot::new(x, y).unwrap()
