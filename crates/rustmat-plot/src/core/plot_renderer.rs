...
use crate::core::{WgpuRenderer, Camera, Scene};
...
impl PlotRenderer {
    ...
    pub fn render_to_viewport(
        ...
        for node in self.scene.get_visible_nodes() {
            if let Some(render_data) = &node.render_data {
                if !render_data.vertices.is_empty() {
                    ...
                    let vertex_buffer = self.wgpu_renderer.create_vertex_buffer(&render_data.vertices);
                    
                    // Debug: Count vertices being sent to GPU
                    if render_data.vertices.len() == 12 {
                        println!("CRITICAL: {} vertices -> GPU, draw calls: {}", 
                                render_data.vertices.len(), render_data.draw_calls.len());
                        for (i, call) in render_data.draw_calls.iter().enumerate() {
                            println!("  Call {}: offset={}, count={}", i, call.vertex_offset, call.vertex_count);
                        }
                    }
        ...
        self.wgpu_renderer.update_uniforms(view_proj_matrix, Mat4::IDENTITY);
    ...
    pub fn render(
        ...
        self.wgpu_renderer.update_uniforms(view_proj_matrix, model_matrix);
...
pub mod plot_utils {
    ...
    ) -> Vec<(f32, f32, f32, f32)> { // Vector of (x1, y1, x2, y2) line segments
