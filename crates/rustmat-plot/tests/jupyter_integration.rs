//! Comprehensive Jupyter integration tests for RustMat plotting
//!
//! Tests the complete Jupyter plotting pipeline including export systems,
//! widget generation, and protocol integration.

use rustmat_plot::jupyter::{JupyterBackend, OutputFormat, ExportSettings, Quality};
use rustmat_plot::plots::{Figure, LinePlot, ScatterPlot, BarChart, Histogram};
...
fn test_figure_display_formats() {
    ...
    assert!(svg.contains("</svg>"));
    assert!(svg.contains("RustMat Plot System"));
}
...
fn test_plot_type_display() {
    ...
    let surface_plot = rustmat_plot::plots::SurfacePlot::new(
        vec![-1.0, 0.0, 1.0],
        vec![-1.0, 0.0, 1.0],
...
fn test_jupyter_environment_detection() {
    use rustmat_plot::jupyter::utils;
...
fn test_widget_state_management() {
    use rustmat_plot::jupyter::WidgetState;
...
fn test_multiple_plot_types_in_figure() {
    ...
    assert!(html.contains("WebGL") || html.contains("RustMat"));
...
fn test_concurrent_backend_usage() {
    ...
    for i in 0..5 {
        ...
        let handle = thread::spawn(move || {
            let line_plot = LinePlot::new(vec![i as f64, i as f64 + 1.0], vec![i as f64 * 2.0, (i as f64 + 1.0) * 2.0]).unwrap();
            let mut figure = Figure::new();
            figure.add_line_plot(line_plot);
...
fn test_memory_usage_patterns() {
    ...
    let large_y: Vec<f64> = large_x.iter().map(|&x| (x * 2.0 * std::f64::consts::PI).sin()).collect();
...
fn test_config_serialization() {
    use rustmat_plot::jupyter::{ExportSettings, Quality};
...
fn test_edge_cases() {
    ...
    let negative_values = LinePlot::new(
        vec![-5.0, -3.0, -1.0],
        vec![-10.0, -6.0, -2.0],
    ).unwrap();
...
fn test_performance_benchmarks() {
    ...
    for i in 0..10 {
        let line_plot = LinePlot::new(
            ...
            vec![i as f64 * 2.0, (i as f64 + 1.0) * 2.0, (i as f64 + 2.0) * 2.0],
        ).unwrap();
        let mut figure = Figure::new();
        figure.add_line_plot(line_plot);
...
fn test_widget_isolation() {
    ...
    assert!(widget1.contains("Figure 1") || widget1.contains("rustmat_"));
    assert!(widget2.contains("Figure 2") || widget2.contains("rustmat_"));
}