...
fn include_segment(i: u32, style: u32) -> bool {
    switch (style) {
        case 0u: { return true; }
        case 1u: { return (i % 4u) < 2u; }
        case 2u: { return (i % 4u) == 0u; }
        case 3u: {
            let m = i % 6u;
            return (m < 2u) || (m == 3u);
        }
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    if (i == 0u && params.baseline_visible != 0u) {
        write_vertex(0u, vec3<f32>(params.min_x, params.baseline, 0.0), params.baseline_color);
        write_vertex(1u, vec3<f32>(params.max_x, params.baseline, 0.0), params.baseline_color);
    }

    if (!include_segment(i, params.line_style)) {
        return;
    }
