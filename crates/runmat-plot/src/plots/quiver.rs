...
impl QuiverPlot {
    pub fn new(x: Vec<f64>, y: Vec<f64>, u: Vec<f64>, v: Vec<f64>) -> Result<Self, String> {
        let n = x.len();
        if n == 0 || y.len()!=n || u.len()!=n || v.len()!=n { return Err("quiver: X,Y,U,V must have same non-zero length".to_string()); }
        Ok(Self { x, y, u, v, color: Vec4::new(0.0,0.0,0.0,1.0), line_width: 1.0, scale: 1.0, head_size: 0.1, label: None, visible: true, vertices: None, bounds: None, dirty: true })
    }
    pub fn with_style(mut self, color: Vec4, line_width: f32, scale: f32, head_size: f32) -> Self { self.color=color; self.line_width=line_width.max(0.5); self.scale=scale.max(0.0); self.head_size=head_size.max(0.0); self.dirty=true; self }
    ...
    pub fn generate_vertices(&mut self) -> &Vec<Vertex> {
        if self.dirty || self.vertices.is_none() {
            ...
            for i in 0..self.x.len() {
                let (x,y,u,v) = (self.x[i] as f32, self.y[i] as f32, self.u[i] as f32, self.v[i] as f32);
                if !x.is_finite() || !y.is_finite() || !u.is_finite() || !v.is_finite() { continue; }
                let dx = u * self.scale; let dy = v * self.scale;
                // Main shaft
                ...
                if len > 0.0 && self.head_size > 0.0 {
                    let hx = dx / len; let hy = dy / len;
                    // Perpendicular
                    let px = -hy; let py = hx;
                    let h = self.head_size.min(len*0.5);
                    let tipx = x+dx; let tipy = y+dy;
                    let leftx = tipx - h*hx + 0.5*h*px; let lefty = tipy - h*hy + 0.5*h*py;
                    let rightx = tipx - h*hx - 0.5*h*px; let righty = tipy - h*hy - 0.5*h*py;
    ...
    pub fn bounds(&mut self) -> BoundingBox {
        if self.dirty || self.bounds.is_none() {
            ...
            for i in 0..self.x.len() {
                let x = self.x[i] as f32; let y = self.y[i] as f32; let dx = (self.u[i] as f32)*self.scale; let dy = (self.v[i] as f32)*self.scale;
                if !x.is_finite() || !y.is_finite() || !dx.is_finite() || !dy.is_finite() { continue; }
                min.x = min.x.min(x.min(x+dx)); max.x = max.x.max(x.max(x+dx));
                min.y = min.y.min(y.min(y+dy)); max.y = max.y.max(y.max(y+dy));
            }
