...
impl ImagePlot {
    pub fn from_grayscale(
        ...
        Ok(Self { surface, label: None, visible: true })
    }
    ...
    pub fn from_color_grid(
        ...
        let cols = if color_grid.is_empty() { 0 } else { color_grid[0].len() };
    ...
    pub fn bounds(&mut self) -> BoundingBox { self.surface.bounds() }
    pub fn render_data(&mut self) -> RenderData {
        ...
        if let Some(grid) = &self.surface.color_grid {
            ...
            let (min_z, max_z) = if let Some((lo, hi)) = self.surface.color_limits {
                ...
                for row in &self.surface.z_data { for &z in row { if z.is_finite(){ lo = lo.min(z); hi = hi.max(z); } } }
                (lo, hi)
            };
            let zr = (max_z - min_z).max(f64::MIN_POSITIVE);
            for i in 0..x_len {
                for j in 0..y_len {
                    let z = self.surface.z_data[i][j];
        ...
        let vertices = vec![
            Vertex { position: [min.x, min.y, 0.0], color: color.to_array(), normal: [0.0,0.0,1.0], tex_coords: [0.0, 0.0] },
            Vertex { position: [max.x, min.y, 0.0], color: color.to_array(), normal: [0.0,0.0,1.0], tex_coords: [1.0, 0.0] },
            Vertex { position: [max.x, max.y, 0.0], color: color.to_array(), normal: [0.0,0.0,1.0], tex_coords: [1.0, 1.0] },
            Vertex { position: [min.x, max.y, 0.0], color: color.to_array(), normal: [0.0,0.0,1.0], tex_coords: [0.0, 1.0] },
        ];
        let indices = vec![0u32,1,2, 0,2,3];

        let material = crate::core::Material::default();
        let draw_call = crate::core::DrawCall { vertex_offset: 0, vertex_count: vertices.len(), index_offset: Some(0), index_count: Some(indices.len()), instance_count: 1 };
        RenderData {
            ...
            image: Some(ImageData::Rgba8 { width, height, data: rgba }),
        }
    }
    pub fn estimated_memory_usage(&self) -> usize { self.surface.estimated_memory_usage() }
}
