...
impl ErrorBar {
    pub fn new(x: Vec<f64>, y: Vec<f64>, err_low: Vec<f64>, err_high: Vec<f64>) -> Result<Self, String> {
        let n = x.len();
    ...
    pub fn generate_vertices(&mut self) -> &Vec<Vertex> {
        if self.dirty || self.vertices.is_none() {
            let (min_x, max_x) = self.x.iter().fold((f64::INFINITY, f64::NEG_INFINITY), |(a,b), &v| (a.min(v), b.max(v)));
            ...
            for i in 0..self.x.len() {
                ...
                if !xi.is_finite() || !yi.is_finite() || !y0.is_finite() || !y1.is_finite() { continue; }
                // Vertical line
    ...
    pub fn bounds(&mut self) -> BoundingBox {
        if self.dirty || self.bounds.is_none() {
            ...
            for i in 0..self.x.len() {
                ...
                let y0 = (self.y[i] - self.err_low[i]) as f32;
                let y1 = (self.y[i] + self.err_high[i]) as f32;
                if !xi.is_finite() || !y0.is_finite() || !y1.is_finite() { continue; }
                min.x = min.x.min(xi);
                max.x = max.x.max(xi);
