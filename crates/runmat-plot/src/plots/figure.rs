...
use crate::plots::{AreaPlot, BarChart, ErrorBar, ImagePlot, LinePlot, PieChart, QuiverPlot, ScatterPlot, StairsPlot, StemPlot, SurfacePlot};
...
impl Figure {
    ...
    /// Set log scale flags
    pub fn with_xlog(mut self, enabled: bool) -> Self { self.x_log = enabled; self }
    pub fn with_ylog(mut self, enabled: bool) -> Self { self.y_log = enabled; self }
    pub fn with_axis_equal(mut self, enabled: bool) -> Self { self.axis_equal = enabled; self }
    pub fn with_colormap(mut self, cmap: ColorMap) -> Self { self.colormap = cmap; self }
    pub fn with_colorbar(mut self, enabled: bool) -> Self { self.colorbar_enabled = enabled; self }
    pub fn with_color_limits(mut self, limits: Option<(f64, f64)>) -> Self { self.color_limits = limits; self }
    ...
    pub fn axes_grid(&self) -> (usize, usize) { (self.axes_rows, self.axes_cols) }

    /// Axes index mapping for plots (length equals number of plots)
    pub fn plot_axes_indices(&self) -> &[usize] { &self.plot_axes_indices }
    ...
    pub fn assign_plot_to_axes(&mut self, plot_index: usize, axes_index: usize) -> Result<(), String> {
        if plot_index >= self.plot_axes_indices.len() { return Err(format!("assign_plot_to_axes: index {plot_index} out of bounds")); }
    ...
    pub fn add_errorbar(&mut self, plot: ErrorBar) -> usize {
        self.plots.push(PlotElement::ErrorBar(plot));
        self.plot_axes_indices.push(0);
        self.dirty = true;
        self.plots.len() - 1
    }
    ...
    pub fn add_stairs_plot(&mut self, plot: StairsPlot) -> usize {
        self.plots.push(PlotElement::Stairs(plot));
        self.plot_axes_indices.push(0);
        self.dirty = true;
        self.plots.len() - 1
    }
    ...
    pub fn add_stem_plot(&mut self, plot: StemPlot) -> usize {
        self.plots.push(PlotElement::Stem(plot));
        self.plot_axes_indices.push(0);
        self.dirty = true;
        self.plots.len() - 1
    }
    ...
    pub fn add_area_plot(&mut self, plot: AreaPlot) -> usize {
        self.plots.push(PlotElement::Area(plot));
        self.plot_axes_indices.push(0);
        self.dirty = true;
        self.plots.len() - 1
    }
    ...
    pub fn add_image_plot(&mut self, plot: ImagePlot) -> usize {
        self.plots.push(PlotElement::Image(plot));
        self.plot_axes_indices.push(0);
        self.dirty = true;
        self.plots.len() - 1
    }
    ...
    pub fn add_quiver_plot(&mut self, plot: QuiverPlot) -> usize {
        self.plots.push(PlotElement::Quiver(plot));
        self.plot_axes_indices.push(0);
        self.dirty = true; self.plots.len()-1
    }

    pub fn add_pie_chart(&mut self, plot: PieChart) -> usize {
        self.plots.push(PlotElement::Pie(plot));
        self.plot_axes_indices.push(0);
        self.dirty = true; self.plots.len()-1
    }
    ...
    pub fn add_surface_plot(&mut self, plot: SurfacePlot) -> usize {
        self.plots.push(PlotElement::Surface(plot));
        self.plot_axes_indices.push(0);
        self.dirty = true;
        self.plots.len() - 1
    }
    ...
    pub fn render_data(&mut self) -> Vec<RenderData> {
        ...
        for p in self.plots.iter_mut() {
            ...
            if let PlotElement::Surface(s) = p {
                if self.color_limits.is_some() { s.set_color_limits(self.color_limits); }
            }
