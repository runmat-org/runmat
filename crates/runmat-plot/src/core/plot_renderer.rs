...
impl PlotRenderer {
    ...
    pub fn set_figure(&mut self, figure: Figure) {
        ...
        if self.axes_cameras.len() != num_axes {
            self.axes_cameras = (0..num_axes).map(|_| Self::create_default_camera()).collect();
        }
    ...
    fn add_figure_to_scene(&mut self, mut figure: Figure) {
        ...
        for (node_id_counter, render_data) in render_data_list.into_iter().enumerate() {
            let axes_index = axes_map.get(node_id_counter).copied().unwrap_or(0).min(rows * cols - 1);
            ...
            let _ = nid; let _ = axes_index; let _ = rows; let _ = cols;
        }
    ...
    pub fn render_to_viewport(
        ...
        let msaa_view_opt = if use_msaa {
            let tex = self.wgpu_renderer.device.create_texture(&wgpu::TextureDescriptor {
                label: Some("runmat_msaa_color_camera"),
                size: wgpu::Extent3d { width: self.wgpu_renderer.surface_config.width, height: self.wgpu_renderer.surface_config.height, depth_or_array_layers: 1 },
                mip_level_count: 1,
    ...
    pub fn render_direct_to_viewport(
        ...
        let mut render_items: Vec<(&crate::core::RenderData, wgpu::Buffer, Option<wgpu::Buffer>)> =
            Vec::new();
        let mut total_vertices = 0;
        ...
        let mut image_bind_groups: Vec<Option<wgpu::BindGroup>> = Vec::with_capacity(render_items.len());
        let mut point_style_bind_groups: Vec<Option<wgpu::BindGroup>> = Vec::with_capacity(render_items.len());
        for (render_data, _vb, _ib) in &render_items {
            if render_data.pipeline_type == crate::core::PipelineType::Textured {
                if let Some(crate::core::scene::ImageData::Rgba8 { width, height, data }) = &render_data.image {
                    let (_t, _v, img_bg) = self.wgpu_renderer.create_image_texture_and_bind_group(*width, *height, data);
                    image_bind_groups.push(Some(img_bg));
                } else { image_bind_groups.push(None); }
            } else { image_bind_groups.push(None); }
            ...
            } else { point_style_bind_groups.push(None); }
        }
        ...
        for (idx, (render_data, vertex_buffer, index_buffer)) in render_items.iter().enumerate() {
            match render_data.pipeline_type {
                ...
                crate::core::PipelineType::Triangles => {
                    ...
                    let pipeline = self.wgpu_renderer.direct_triangle_pipeline.as_ref().unwrap();
                    render_pass.set_pipeline(pipeline);
                ...
                crate::core::PipelineType::Textured => {
                    ...
                    let pipeline = self.wgpu_renderer.get_pipeline(crate::core::PipelineType::Textured);
                    render_pass.set_pipeline(pipeline);
                    render_pass.set_bind_group(0, &self.wgpu_renderer.direct_uniform_bind_group, &[]);
                    if let Some(ref bg) = image_bind_groups[idx] { render_pass.set_bind_group(1, bg, &[]); }
                    render_pass.set_vertex_buffer(0, vertex_buffer.slice(..));
                    if let Some(idx) = index_buffer {
                        render_pass.set_index_buffer(idx.slice(..), wgpu::IndexFormat::Uint32);
                        if let Some(indices) = &render_data.indices { render_pass.draw_indexed(0..indices.len() as u32, 0, 0..1); }
                    } else {
                        for dc in &render_data.draw_calls {
                            render_pass.draw(dc.vertex_offset as u32..(dc.vertex_offset + dc.vertex_count) as u32, 0..dc.instance_count as u32);
                        }
                    }
                }
                _ => {
                    ...
                    if render_data.indices.is_some() {
                        ...
                        if matches!(render_data.pipeline_type, crate::core::PipelineType::Points) {
                            ...
                            let pipeline = self.wgpu_renderer.direct_point_pipeline.as_ref().unwrap();
                            render_pass.set_pipeline(pipeline);
                            render_pass.set_bind_group(0, &self.wgpu_renderer.direct_uniform_bind_group, &[]);
                            if let Some(ref bg) = point_style_bind_groups[idx] { render_pass.set_bind_group(1, bg, &[]); }
                        }
                        for dc in &render_data.draw_calls {
                            render_pass.draw(
    ...
    pub fn render(
        ...
        for (i, (render_data, vertex_buffer, index_buffer)) in render_items.iter().enumerate() {
            ...
            render_pass.set_pipeline(pipeline);
                ...
                render_pass.set_bind_group(0, &self.wgpu_renderer.direct_uniform_bind_group, &[]);
                if let Some(ref img_bg) = image_bind_groups[i] { render_pass.set_bind_group(1, img_bg, &[]); }
            } else {
                let pipeline = self.wgpu_renderer.get_pipeline(render_data.pipeline_type);
                render_pass.set_pipeline(pipeline);
    ...
    pub fn render_camera_to_viewport(
        ...
        for node in self.scene.get_visible_nodes() {
            if let Some(render_data) = &node.render_data {
                if !render_data.vertices.is_empty() {
                    self.wgpu_renderer.ensure_pipeline(render_data.pipeline_type);
                    let vb = self.wgpu_renderer.create_vertex_buffer(&render_data.vertices);
                    let ib = if let Some(indices) = &render_data.indices {
                        Some(self.wgpu_renderer.create_index_buffer(indices))
                    } else { None };
                    total_vertices += render_data.vertices.len();
                    if let Some(indices) = &render_data.indices { total_triangles += indices.len()/3; }
        ...
        let mut point_buffers: Vec<Option<(wgpu::Buffer, usize)>> = Vec::with_capacity(render_items.len());
        ...
        for (render_data, _vb, _ib) in render_items.iter() {
            if render_data.pipeline_type == crate::core::PipelineType::Textured {
                if let Some(crate::core::scene::ImageData::Rgba8 { width, height, data }) = &render_data.image {
                    let (_t, _v, bg) = self.wgpu_renderer.create_image_texture_and_bind_group(*width, *height, data);
        ...
        if self.figure_show_grid {
            if let Some((l, r, b, t)) = self.view_bounds() {
                ...
                if x_step.is_finite() && x_step > 0.0 {
                    ...
                    let b_f = b as f32; let t_f = t as f32;
                    while (x as f64) <= r {
                ...
                if y_step.is_finite() && y_step > 0.0 {
                    ...
                    let l_f = l as f32; let r_f = r as f32;
                    while (y as f64) <= t {
        ...
        render_pass.set_viewport(sx as f32, sy as f32, sw.max(1) as f32, sh.max(1) as f32, 0.0, 1.0);
        ...
        if let Some(ref vb_grid) = grid_vb_opt {
            if let Some(ref pipeline) = self.wgpu_renderer.direct_line_pipeline {
                ...
                render_pass.draw(0..(vb_grid.size() / std::mem::size_of::<Vertex>() as u64) as u32, 0..1);
            }
        }
        ...
        let direct_tri_pipeline = if use_direct_for_triangles && bounds_opt.is_some() { self.wgpu_renderer.direct_triangle_pipeline.as_ref().map(|p| p as *const wgpu::RenderPipeline) } else { None };
        let direct_line_pipeline = if use_direct_for_lines && bounds_opt.is_some() { self.wgpu_renderer.direct_line_pipeline.as_ref().map(|p| p as *const wgpu::RenderPipeline) } else { None };
        let direct_point_pipeline = if bounds_opt.is_some() { self.wgpu_renderer.direct_point_pipeline.as_ref().map(|p| p as *const wgpu::RenderPipeline) } else { None };

        // Keep transient point buffers alive during this pass
        let mut __temp_point_buffers_cam: Vec<wgpu::Buffer> = Vec::new();
        for (idx, (render_data, vertex_buffer, index_buffer)) in render_items.iter().enumerate() {
            let is_triangles = matches!(render_data.pipeline_type, crate::core::PipelineType::Triangles);
            let is_lines = matches!(render_data.pipeline_type, crate::core::PipelineType::Lines);
            let is_points = matches!(render_data.pipeline_type, crate::core::PipelineType::Points);
            let is_textured = matches!(render_data.pipeline_type, crate::core::PipelineType::Textured);
            // Use direct mapping for lines/triangles/points for correct pixel-sized markers in GUI
            let use_direct = ((use_direct_for_triangles && is_triangles) || (use_direct_for_lines && is_lines) || is_points) && bounds_opt.is_some();

            if use_direct {
                // Safe because we only read pointers here within pass
                let pipeline_ref: &wgpu::RenderPipeline = unsafe {
                    if is_triangles { direct_tri_pipeline.unwrap().as_ref().unwrap() } else if is_lines { direct_line_pipeline.unwrap().as_ref().unwrap() } else { direct_point_pipeline.unwrap().as_ref().unwrap() }
                };
    ...
    pub fn render_axes_to_viewports(
        ...
        let mut axes_to_nodes: std::collections::HashMap<usize, Vec<crate::core::scene::NodeId>> = std::collections::HashMap::new();
        for node in self.scene.get_visible_nodes() { axes_to_nodes.entry(node.axes_index).or_default().push(node.id); }

        if self.axes_cameras.is_empty() { self.axes_cameras.push(Self::create_default_camera()); }

        // Pre-collect all node ids
        let all_ids: Vec<crate::core::scene::NodeId> = self.scene.get_visible_nodes().into_iter().map(|n| n.id).collect();

        for (ax_idx, viewport) in axes_viewports.iter().enumerate() {
            let ids_for_axes = axes_to_nodes.get(&ax_idx).cloned().unwrap_or_default();
            if ids_for_axes.is_empty() { continue; }
            ...
            for id in &all_ids {
                if !ids_for_axes.contains(id) {
                    if let Some(node) = self.scene.get_node_mut(*id) {
                        if node.visible { node.visible = false; hidden_ids.push(*id); }
                    }
                }
            }
            // Update camera and bounds
            if let Some(cam) = self.axes_cameras.get(ax_idx).cloned() { self.camera = cam; }
            let _ = self.calculate_data_bounds();

            // Render this axes into its viewport
            let cfg = PlotRenderConfig { width: viewport.2, height: viewport.3, msaa_samples, ..Default::default() };
            let _ = self.render_camera_to_viewport(encoder, target_view, *viewport, &cfg)?;

            // Restore hidden nodes visibility
            for id in hidden_ids { if let Some(node) = self.scene.get_node_mut(id) { node.visible = true; } }
        }
        Ok(())
    }
    ...
    pub fn overlay_show_grid(&self) -> bool { self.figure_show_grid }
    pub fn overlay_title(&self) -> Option<&String> { self.figure_title.as_ref() }
    pub fn overlay_x_label(&self) -> Option<&String> { self.figure_x_label.as_ref() }
    pub fn overlay_y_label(&self) -> Option<&String> { self.figure_y_label.as_ref() }
    pub fn overlay_show_legend(&self) -> bool { self.figure_show_legend }
    pub fn overlay_legend_entries(&self) -> &Vec<LegendEntry> { &self.legend_entries }
    pub fn overlay_x_log(&self) -> bool { self.figure_x_log }
    pub fn overlay_y_log(&self) -> bool { self.figure_y_log }
    pub fn overlay_colormap(&self) -> ColorMap { self.figure_colormap }
    pub fn overlay_colorbar_enabled(&self) -> bool { self.figure_colorbar_enabled }
    /// Subplot grid
    pub fn figure_axes_grid(&self) -> (usize, usize) { self.scene_axes_grid_fallback() }
    ...
    pub fn overlay_categorical_labels(&self) -> Option<(bool, &Vec<String>)> {
        if let (Some(is_x), Some(labels)) = (&self.figure_categorical_is_x, &self.figure_categorical_labels) {
            Some((*is_x, labels))
        } else {
            None
        }
    ...
    pub fn data_bounds(&self) -> Option<(f64, f64, f64, f64)> {
        ...
        base.map(|(bx_min, bx_max, by_min, by_max)| {
            ...
            if let Some((xl, xr)) = self.figure_x_limits { x_min = xl; x_max = xr; }
            if let Some((yl, yr)) = self.figure_y_limits { y_min = yl; y_max = yr; }
    ...
    pub fn export_figure_clone(&self) -> crate::plots::Figure {
        if let Some(f) = &self.last_figure { return f.clone(); }
