...
impl ValueArena {
    ...
    #[cfg(not(target_arch = "wasm32"))]
    fn activate_gc_roots(&mut self) -> JitResult<()> {
        ...
        self.root_id = Some(
            runmat_gc::gc_register_root(Box::new(root))
                .map_err(|error| JitError::Host(format!("register native GC roots: {error}")))?,
        );
        Ok(())
    }
...
impl ScopedValueRoots {
    pub(crate) fn register(values: Vec<Value>, description: &str) -> JitResult<Self> {
        if values.is_empty() {
            return Ok(Self { root_id: None });
        }
        let root = runmat_gc::GlobalRoot::new(values, description.to_string());
        let root_id = runmat_gc::gc_register_root(Box::new(root))
            .map_err(|error| JitError::Host(format!("register native GC roots: {error}")))?;
        Ok(Self {
