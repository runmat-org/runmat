...
pub(super) fn assign(
    ...
    let updated = match base {
        ...
        _ => Err(JitError::from(semantic_error(
            "SliceNonTensor",
...
async fn plan_for(base: &Value, selectors: &MaterializedSelectors) -> JitResult<IndexPlan> {
    let shape = value_shape(base);
    ...
    .await?;
    build_index_plan(&built, selectors.dims, &shape).map_err(JitError::from)
}
...
fn assign_logical(
    ...
    .map_err(|error| JitError::from(shape_error(error)))?;
...
fn read_with_plan(base: Value, plan: &IndexPlan) -> JitResult<Value> {
    match base {
        Value::Tensor(value) => {
            read_slice::read_tensor_slice_from_plan(&value, plan).map_err(JitError::from)
        }
        Value::ComplexTensor(value) => {
            read_slice::read_complex_slice_from_plan(&value, plan).map_err(JitError::from)
        }
        Value::SparseTensor(value) => {
            read_slice::read_sparse_slice_from_plan(&value, plan).map_err(JitError::from)
        }
        Value::GpuTensor(value) => {
            read_slice::read_gpu_slice_from_plan(&value, plan).map_err(JitError::from)
        }
        Value::StringArray(value) => {
            read_slice::gather_string_slice(&value, plan).map_err(JitError::from)
        }
...
fn gather_logical(value: &LogicalArray, plan: &IndexPlan) -> JitResult<Value> {
...
fn gather_char(value: &CharArray, plan: &IndexPlan) -> JitResult<Value> {
...
fn gather_object(value: &ObjectArray, plan: &IndexPlan) -> JitResult<Value> {
    if let [index] = plan.indices.as_slice() {
        return value.get_linear(*index as usize).cloned().ok_or_else(|| {
            JitError::from(semantic_error("IndexOutOfBounds", "Index out of bounds"))
        });
    }
    let indices = plan
        .indices
...
fn gather_symbolic(value: &SymbolicArray, plan: &IndexPlan) -> JitResult<Value> {
...
fn read_scalar(value: Value, plan: &IndexPlan) -> JitResult<Value> {
    if plan.indices.as_slice() == [0] {
        Ok(value)
    } else {
        Err(JitError::from(semantic_error(
            "IndexOutOfBounds",
            "Index out of bounds",
        )))
    }
...
fn gather<T: Clone>(data: &[T], plan: &IndexPlan, kind: &str) -> JitResult<Vec<T>> {
    plan.indices
        .iter()
        .map(|index| {
...
fn normalize_outputs(value: Value, requested_outputs: usize) -> JitResult<Vec<Value>> {
    match (requested_outputs, value) {
        ...
        (expected, value) => Err(JitError::from(semantic_error(
            "OutputArityMismatch",
