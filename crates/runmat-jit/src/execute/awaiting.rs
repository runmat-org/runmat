...
pub(super) async fn complete(
    ...
    continuation: u64,
    generation: u64,
) -> JitResult<AwaitCompletion> {
    let pending = state
        .pending_await
        .take()
        .ok_or_else(|| JitError::Host("native invocation has no pending await".into()))?;
...
fn execution_error(error: ExecutionServiceError) -> JitError {
