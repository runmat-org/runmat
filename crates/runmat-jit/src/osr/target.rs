...
use crate::{GenericExecutor, JitResult};
...
impl OsrTarget {
    ...
    pub(crate) fn entrypoint(&self) -> JitResult<runmat_runtime::native::NativeEntryPoint> {
        self.executor.compiled_entrypoint(self.function)
    }

    pub(crate) fn executor(&self) -> &GenericExecutor {
        &self.executor
    }
}
