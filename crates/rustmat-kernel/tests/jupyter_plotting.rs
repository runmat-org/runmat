//! Tests for Jupyter plotting integration in the RustMat kernel
//!
//! Comprehensive tests covering the plotting manager, protocol integration,
//! and display data generation.

use rustmat_kernel::jupyter_plotting::{
    JupyterPlottingManager, JupyterPlottingConfig,
};
use rustmat_plot::jupyter::OutputFormat;
...
fn test_display_data_creation() {
    ...
    assert!(display_data.transient.contains_key("rustmat_plot_id"));
    assert!(display_data.transient.contains_key("rustmat_version"));
    
    // Check HTML content
    if let Some(JsonValue::String(html)) = display_data.data.get("text/html") {
        assert!(html.contains("RustMat Interactive Plot"));
    } else {
...
fn test_plot_function_handling_scatter() {
    ...
    let result = manager.handle_plot_function("scatter", &[x_data, y_data]).unwrap();
...
fn test_metadata_in_display_data() {
    ...
    if let Some(JsonValue::String(version)) = display_data.transient.get("rustmat_version") {
        assert_eq!(version, "0.0.1");
    }
    
    if let Some(JsonValue::String(plot_id)) = display_data.transient.get("rustmat_plot_id") {
