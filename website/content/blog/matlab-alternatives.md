...
- **RunMat** runs MATLAB simulation scripts (e.g., ODEs, discrete-time models) at near-native speed. Monte Carlo simulations and batch parameter sweeps benefit from automatic GPU acceleration. Control systems packages (transfer functions, state-space analysis) are on the roadmap.
...
Engineering datasets routinely exceed 10 GB per project: wind tunnel measurements, sensor logs, simulation outputs that dwarf the code producing them. RunMat handles this with automatic sharding and manifest versioning; Python uses DVC and Zarr; MATLAB offers 5 GB of MATLAB Drive on the free tier.
